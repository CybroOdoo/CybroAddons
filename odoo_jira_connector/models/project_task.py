...
class ProjectTask(models.Model):
    """Extends project.task to integrate with Jira tasks."""
    ...
    def _compute_task_sprint_active(self):
        """Computes if a sprint is active for the task's project."""
        for rec in self:
            if rec.project_id.sprint_active:
                rec.task_sprint_active = True
            else:
                rec.task_sprint_active = False

    @api.model_create_multi
    def create(self, vals_list):
        """Creates tasks in Odoo and, if configured, exports them to Jira."""
        ...
        if self.env['ir.config_parameter'].sudo().get_param('odoo_jira_connector.automatic'):
            ...
            if jira_connection:
                ...
                for task in res:
                    if not task.task_id_jira and task.project_id.jira_project_key:
                        desc_text = html2plaintext(task.description) if task.description else ""
                        payload = json.dumps({
                            'fields': {
                                'project': {'key': task.project_id.jira_project_key},
                                'summary': task.name,
                        ...
                        try:
                            ...
                            if response.ok:
                                data = response.json()
                                task.task_id_jira = str(data.get('key'))
                                export_task_count = self.env['ir.config_parameter'].sudo().get_param('export_task_count')
                                if export_task_count:
                                    self.env['ir.config_parameter'].sudo().set_param(
                                        'export_task_count', int(export_task_count) + 1)
                        except Exception:
                            continue
            return res
        return res
    ...
    def unlink(self):
        """Deletes tasks in Odoo and, if configured, in Jira."""
            ...
            for task in self:
                ...
                if jira_connection:
                    ...
                    if task.task_id_jira:
                        requests.delete(
                            jira_url.rstrip('/') + '/rest/api/2/issue/' + task.task_id_jira,
                            headers=HEADERS, auth=(user, password),
                            timeout=10)
        return super(ProjectTask, self).unlink()

    def write(self, vals):
        """Updates task details in Odoo and, if configured, in Jira."""
        if self.env['ir.config_parameter'].sudo().get_param(
                'odoo_jira_connector.automatic'):
            jira_connection = self.env['ir.config_parameter'].sudo().get_param(
                'odoo_jira_connector.connection')
    ...
    def webhook_data_handle(self, jira_data, webhook_event):
        """Handles incoming webhook data from Jira, routing to appropriate functions."""
    ...
    def create_project(self, jira_data):
        """Creates a project in Odoo based on Jira webhook data."""
        jira_project = jira_data['project']
        existing_project = self.env['project.project'].sudo().search(
            [('project_id_jira', '=', int(jira_project['id']))])
        values = {
        ...
        if not existing_project:
            imported_project = self.env['project.project'].sudo().create(values)
            url = self.env['ir.config_parameter'].sudo().get_param('odoo_jira_connector.url')
            user = self.env['ir.config_parameter'].sudo().get_param('odoo_jira_connector.user_id_jira')
            password = self.env['ir.config_parameter'].sudo().get_param('odoo_jira_connector.api_token')
            auth = HTTPBasicAuth(user, password)
            headers = {"Accept": "application/json"}
            response = requests.request(
                "GET",
                url.rstrip('/') + "/rest/api/2/project/" + jira_project['key'],
                headers=headers,
                auth=auth,
                timeout=10
            )
    ...
    def update_project(self, jira_data):
        """Updates a project in Odoo based on Jira webhook data."""
        project_id = int(jira_data['project']['id'])
        existing_project = self.env['project.project'].sudo().search(
            [('project_id_jira', '=', project_id)])
        if existing_project and existing_project.name != jira_data['project']['name']:
            existing_project.write({'name': jira_data['project']['name']})
 
    def delete_project(self, jira_data):
        """Deletes a project in Odoo based on Jira webhook data."""
        project_id = int(jira_data['project']['id'])
        self.env['project.project'].sudo().search(
            [('project_id_jira', '=', project_id)]).unlink()

    def create_task(self, jira_data):
        """Creates a task in Odoo based on Jira webhook data."""
    ...
    def delete_task(self, jira_data):
        """Deletes a task in Odoo based on Jira webhook data."""
        task_key = jira_data['issue']['key']
        self.env['project.task'].sudo().search(
            [('task_id_jira', '=', task_key)]).unlink()

    def create_comment(self, jira_data):
        """Creates a comment in Odoo based on Jira webhook data."""
    ...
    def create_user(self, jira_data):
        ...
        existing_user = self.env['res.users'].search([('jira_user_key', '=', jira_data['user']['accountId'])])
    ...
    def board_configuration_change(self, jira_data):
        ...
        if jira_data['configuration'].get('location'):
            ...
            project = self.env['project.project'].search([('board_id_jira', '=', board_id_jira)])
            ...
            if jira_data['changelog']['items'][0]['to'] != 'None':
                ...
                if num_attachments > 0:
                    ...
                    image = base64.b64encode(
                        requests.get(src, auth=auth, timeout=10).content)
    ...
    def sprint_started(self, jira_data):
        ...
        if sprint_in_odoo:
            ...
            input_start_date = datetime.strptime(start_date, '%Y-%m-%dT%H:%M:%S.%fZ')
            jira_start_date = input_start_date.strftime('%Y-%m-%d %H:%M:%S')
            end_date = jira_data['sprint']['endDate']
            input_end_date = datetime.strptime(end_date, '%Y-%m-%dT%H:%M:%S.%fZ')
            jira_end_date = input_end_date.strftime('%Y-%m-%d %H:%M:%S')
