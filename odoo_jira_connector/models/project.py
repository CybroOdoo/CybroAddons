...
class ProjectProject(models.Model):
    ...
    board_id_jira = fields.Integer(string='Jira Board ID', readonly=True,
    ...
    def write(self, vals):
        ...
        if vals.get('name') and self.env['ir.config_parameter'].sudo().get_param('odoo_jira_connector.automatic'):
            jira_connection = self.env['ir.config_parameter'].sudo().get_param(
                'odoo_jira_connector.connection')
            if jira_connection:
                for project in self:
...
class ProjectTaskType(models.Model):
    """Extends project task stages with Jira identifiers."""
    _inherit = 'project.task.type'

    stages_jira_id = fields.Char(string='Jira Stage ID', help='Jira status ID.')
    jira_project_key = fields.Char(string='Jira Project Key',
                                   help='Jira project key for this stage.')
