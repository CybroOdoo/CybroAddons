...
class Uom(models.Model):
    ...
    products_uom = fields.Integer(compute='_compute_product_count',
                                  string="Products",
                                  help="Compute product count in UOM")
    purchase_uom_products = fields.Integer(compute='_compute_product_count',
    ...
    def _compute_product_count(self):
        """Function for computing the count of products with the corresponding
        UOM"""
        for record in self:
            product_template = self.env['product.template']
            record.products_uom = product_template.search_count(
                [('uom_id', '=', record.id)])
            record.purchase_uom_products = product_template.search_count(
                [('uom_po_id', '=', record.id)])
