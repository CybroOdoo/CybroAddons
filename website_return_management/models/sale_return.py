...
class ReturnOrder(models.Model):
    ...
    @api.constrains('quantity', 'product_id', 'sale_order')
    def _check_return_quantity(self):
        ...
        for rec in self:
            ...
            if rec.quantity > remaining_qty:
                raise ValidationError(
                    "Return quantity (%.2f) exceeds remaining returnable quantity "
                    "(%.2f) for '%s'.\n"
                    "Total Delivered: %.2f | Already Returned: %.2f"
                    % (rec.quantity, remaining_qty,
                       rec.product_id.display_name,
                       delivered_qty, already_returned)
                )
    ...
    def return_confirm(self):
        ...
        if not self.source_pick:
            ...
            moves = stock_picks.mapped('move_ids').filtered(
        ...
        if moves:
            ...
            if return_pick:
                # normalize to id (Odoo 19 returns a record, not list)
                picking_id = return_pick.id if hasattr(return_pick, "id") else return_pick
                return_pick = self.env['stock.picking'].browse(picking_id)
    ...
    def onchange_sale_order(self):
        ...
        if delivery:
            product_ids = delivery.move_ids.mapped(
                'product_id').ids
