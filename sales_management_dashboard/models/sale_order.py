...
class SaleOrder(models.Model):
    ...
    def get_tile_domain(self, base_domain, filters):
        ...
        filters = self._build_global_domain(base_domain, filters)
    ...
    def get_sales_dashboard_data(self, filters=None):
        ...
        order_status_grouped = self.read_group(order_domain, ['state'], ['state'])
        ORDER_STATUS_LABELS = {
            'draft': 'Quotation',
            'sent': 'Quotation Sent',
            'sale': 'Sales Order',
        ...
        invoice_status_grouped = self.env['account.move'].read_group(invoice_domain, ['state'], ['state'])
        ...
        sale_orders = self.search_count(sale_orders_domain, limit=limit)

        quotations_domain = build_domain([('state', 'in', ['draft', 'sent'])], "quotation_tile_filter")
        quotations = self.search_count(quotations_domain, limit=limit)

        orders_to_invoice_domain = build_domain([('invoice_status', '=', 'to invoice')], "to_invoice_tile_filter")
        orders_to_invoice = self.search_count(orders_to_invoice_domain, limit=limit)

        orders_fully_invoiced_domain = build_domain([('invoice_status', '=', 'invoiced')], "invoiced_tile_filter")
        orders_fully_invoiced = self.search_count(orders_fully_invoiced_domain, limit=limit)
