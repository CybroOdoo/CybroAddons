...
class POSOrderInherit(models.Model):
    ...
    def create_from_ui(self, orders, draft=False):
        ...
        for line in res:
            if line['id']:
                id.append(line['id'])
        backend_order = self.search([('id', 'in', id)])
        if backend_order:
...
class ResConfigSettings(models.Model):
    ...
    customer_msg = fields.Boolean('POS Greetings', Help='Create an account if you ever create an account')
    auth_token = fields.Char('Auth Token', Help='Copy the token from your twilio console window adn paste here', required=True)
    account_sid = fields.Char('Account SID', required=True)
    twilio_number = fields.Char('Twilio Number', Help='The number provided by twilio used to send text messeges', required=True)
