...
class TestAccountMove(TransactionCase):
    ...
    def test_action_register_payment_returns_payment_wizard_action(self):
        invoices = self.env["account.move"].browse([
            self._create_out_invoice(self.partner_a, 120.0).id,
            self._create_out_invoice(self.partner_a, 80.0).id,
        ])
        ...
        self.assertEqual(action["context"]["active_ids"], invoices.ids)
