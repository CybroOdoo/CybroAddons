...
class SellerSignup(AuthSignupHome):
    ...
    def home_page(self, **post):
        ...
        button """
        params = request.env['res.config.settings'].sudo().get_values()
        bcome_seller = request.env['ir.config_parameter'].sudo().get_param('multi_vendor_marketplace.bcome_seller')
        # ir.config_parameter returns strings, so we check for 'True'
        params['bcome_seller'] = bcome_seller in ['True', '1', True]
        
        is_seller = request.env.user.has_group('multi_vendor_marketplace.multi_vendor_marketplace_seller')
