...
class SaleOrder(models.Model):
    ...
    due_date_order = fields.Date(string="Due Date",
                                 help='Set a due date for the Sale Order')
    ...
    def action_send_mail_sale_order(self):
        ...
        set_date_s = int(values['set_date_sales'])
        date = fields.Date.today()
        ref_s = 'sale_invoice_due_date_reminder.sale_order_due_mail_template'
        if values['reminder_sales'] is not False:
            for record in self.search([]):
                if (record.due_date_order and record.invoice_status !=
                        'invoiced'):
                    if (record.invoice_status == 'upselling' or
                            record.invoice_status == 'to invoice'):
                        if record.due_date_order == timedelta(set_date_s) + date:
                            mail_template = self.env.ref(ref_s)
                            mail_template.send_mail(record.id,
                                                    force_send=True)
                    elif record.invoice_status == 'no':
                        if record.invoice_count == 0:
                            if (record.due_date_order ==
                                    timedelta(set_date_s) + date):
                                mail_template = self.env.ref(ref_s)
                                mail_template.send_mail(record.id,
                                                        force_send=True)
                        else:
                            payment_list = record.invoice_ids.mapped(
                                'payment_state')
                            if (('not_paid' or 'in_payment' or 'partial' or
                                'reversed' or 'invoicing_legacy')
                                    in payment_list):
                                if (record.due_date_order ==
                                        timedelta(set_date_s) + date):
                                    mail_template = self.env.ref(ref_s)
