...
class AccountMove(models.Model):
    ...
    def action_send_mail_invoice(self):
        ...
        set_date = int(values['set_date_invoicing'])
        date = fields.Date.today()
        ref = 'sale_invoice_due_date_reminder.invoice_due_mail_template'
        if values['reminder_invoicing'] is not False:
            for record in self.search([]):
                if (record.invoice_date_due and record.move_type ==
                        'out_invoice'):
                    if record.state == 'draft':
                        if record.invoice_date_due == timedelta(set_date) + date:
                            mail_template = self.env.ref(ref)
                            mail_template.send_mail(record.id, force_send=True)
                    elif (record.state == 'posted' and
                          record.payment_state != 'paid' and
                          record.invoice_date_due == timedelta(set_date) + date):
                        mail_template = self.env.ref(ref)
                        mail_template.send_mail(record.id, force_send=True)
