...
class AutoDatabaseBackup(models.Model):
    """ Model for auto database backup"""
    ...
    db_name = fields.Char(string='Database Name', required=True,
                          help='Name of the database that needs to take backup')
    ...
    def _schedule_auto_backup(self):
        ...
        mail_template_success = \
            self.env.ref('auto_database_backup.'
        ...
        for rec in records:
            ...
            backup_filename = \
                "%s_%s.%s" % (rec.db_name, backup_time, rec.backup_format)
                ...
                try:
                    ...
                    if rec.auto_remove:
                        for filename in os.listdir(rec.backup_path):
                            ...
                            create_time = \
                                datetime.datetime.fromtimestamp(os.
                                                                path.
                                                                getctime(file))
                            backup_duration = \
                    ...
                    if rec.auto_remove:
                        ...
                        for f in files:
                            create_time = datetime.datetime. \
                                strptime(ftp_server.sendcmd('MDTM ' + f)[4:],
                                         "%Y%m%d%H%M%S")
                            diff_days = \
