...
class BiometricDeviceDetails(models.Model):
    ...
    def action_clear_attendance(self):
        """Method to clear record from the zk.machine.attendance model and
        from the device"""
        for info in self:
            try:
                    ...
                    for finger in fingers:
                        if finger.uid == use.uid:
                            ...
                            employee = self.env['hr.employee'].search(
                                [('device_id_num', '=', use.user_id),('company_id', '=', info.company_id.id)])
                            employee.write({
                                'device_id': self.id,
                            })
                        ...
                        for uid in user:
                            if uid.user_id == each.user_id:
                                get_user_id = self.env['hr.employee'].search(
                                    [('device_id_num', '=', each.user_id), ('company_id', '=', info.company_id.id)])
                                if get_user_id:
...
class ZKBioAttendance(Thread):
    ...
    def __init__(self, machine_ip, port_no, password, record):
        ...
        try:
            ...
            _logger.info("Connecting to biometric device at %s:%s...", machine_ip, port_no)
            ...
            _logger.exception("Exception occurred during ZKBioAttendance initialization connection: %s", e)
            raise UserError(_("Please Check the Connection: %s") % e)

    def run(self):
        """Function to run the Thread"""
        while not self.stop_event.is_set():
            try:
                # If connection was closed or lost, reconnect
                if not self.conn:
                    _logger.info("No active connection. Reconnecting to biometric device at %s:%s...", self.machine_ip, self.port_no)
                    zk_device = ZK(
                        self.machine_ip,
                        port=self.port_no,
                        timeout=5,
                    ...
                    self.conn = zk_device.connect()

                if self.conn and not self.conn.end_live_capture:
                    for attendance in self.conn.live_capture(2000):
                        if self.stop_event.is_set():
                            break
                        if attendance is not None:
                            
                            # A real event occurred. Disconnect first to free the device connection
                            try:
                                self.conn.disconnect()
                            except Exception as e:
                                _logger.warning("Error disconnecting live capture connection: %s", e)
                            self.conn = None

                            # Process the captured event directly using Odoo models
                            self._data_live_capture(attendance)
                            
                            # Break the inner generator loop to reconnect and restart live capture
                            break
                            
                time.sleep(10)
            except Exception as e:
                _logger.exception("Exception inside Live Capture Thread loop: %s", e)
                if self.conn:
                    try:
                        self.conn.disconnect()
                    except Exception:
    ...
    def stop(self):
        ...
        if self.conn:
            try:
                ...
                _logger.warning("Error disconnecting live capture connection on stop: %s", e)
            self.conn = None

    def _data_live_capture(self, attendance):
        """Process the real-time live capture attendance event"""
        with Registry(self.env.cr.dbname).cursor() as new_cr:
            new_env = api.Environment(new_cr, self.env.uid, self.env.context)
            try:
                zk_attendance = new_env['zk.machine.attendance']
                hr_attendance = new_env['hr.attendance']
                employee_model = new_env['hr.employee']
                record = self.record.with_env(new_env)
                
                # Convert timezone
                atten_time = attendance.timestamp
                local_tz = pytz.timezone(new_env.user.partner_id.tz or 'GMT')
                local_dt = local_tz.localize(atten_time, is_dst=None)
                utc_dt = local_dt.astimezone(pytz.utc)
                utc_dt = utc_dt.strftime("%Y-%m-%d %H:%M:%S")
                atten_time_str = fields.Datetime.to_string(datetime.datetime.strptime(utc_dt, "%Y-%m-%d %H:%M:%S"))
                
                # Search for Employee mapped to this device and company
                employee = employee_model.search([
                    ('device_id_num', '=', str(attendance.user_id)),
                ...
                if not employee:
                    employee = employee_model.create({
                        'device_id_num': str(attendance.user_id),
                        'device_id': record.id,
                        'name': f"Unknown User {attendance.user_id}",
                        'company_id': record.company_id.id
                    })

                # Check if this exact punch is already recorded
                duplicate_atten_ids = zk_attendance.search([
                    ('device_id_num', '=', str(attendance.user_id)),
                ...
                if not duplicate_atten_ids:
                    # Create Attendance Analysis Record
                    zk_attendance.create({
                        'employee_id': employee.id,
                        'device_id_num': str(attendance.user_id),
                    ...
                    # Manage HR Attendance check-in / check-out
                    att_var = hr_attendance.search([
                        ('employee_id', '=', employee.id),
                        ('check_out', '=', False)
                    ])
                    ...
                    if attendance.punch == 0:  # Check-in
                        ...
                        if len(att_var) == 1:
                            ...
                            att_var1 = hr_attendance.search([('employee_id', '=', employee.id)])
                            if att_var1:
                                att_var1[-1].write({
                                    'check_out': atten_time_str,
                                    'out_mode': 'technical',
                ...
                new_cr.rollback()
                _logger.exception("Error executing direct save in live capture: %s", e)
