...
class SaleOrder(models.Model):
    ...
    is_revision_enabled = fields.Boolean(string="Is Revision Enabled",
    compute = "_compute_is_revision_enabled", help="Is setting revised enabled"
    )
    ...
    def _compute_is_revision_enabled(self):
        enabled = self.env['ir.config_parameter'].sudo().get_param('sale_quotation_revision.is_quotation_revision')
        for rec in self:
            rec.is_revision_enabled =bool(enabled)
    ...
    def get_related_orders(self):
        ...
        self.ensure_one()
        related_orders = self.rev_sale_ids.filtered(
            lambda r: r.state not in ["cancel", "sale"]
        )
        if self.org_sale_id:
            if self.org_sale_id.state not in ["cancel", "sale"]:
                related_orders |= self.org_sale_id
            org_related_orders = self.org_sale_id.rev_sale_ids.filtered(
                lambda r: r.state not in ["cancel", "sale"]
            )
