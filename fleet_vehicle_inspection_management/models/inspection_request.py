...
#    Author: Jigin K(<https://www.cybrosys.com>)
...
class InspectionRequests(models.Model):
    ...
    name = fields.Char(readonly=True, store=True, default='New',
                       help='Inspection Request name',
                       string='Inspection Request name', required=True)
    ...
    is_service_available = fields.Boolean('Service active',
                                    help='Active Service smart button',
                                    compute='_compute_is_service_available')
    is_fleet_available = fields.Boolean('Service active',
                                  help='Active Service smart button',
                                  compute='_compute_is_fleet_available')

    @api.model_create_multi
    def create(self, vals_list):
        """generate vehicle inspection sequence (supports multi-create)"""
        for vals in vals_list:
            if vals.get('name', 'New') == 'New':
                vals['name'] = self.env['ir.sequence'].next_by_code(
                    'inspection.request') or 'New'
        result = super(InspectionRequests, self).create(vals_list)
        return result

    @api.depends('is_service_available')
    def _compute_is_service_available(self):
    ...
    @api.depends('is_fleet_available')
    def _compute_is_fleet_available(self):
        """To set fleet active value"""
        if self.vehicle_id:
            self.is_fleet_available = True
        else:
            self.is_fleet_available = False
    ...
    def action_create_inspection_request(self):
        ...
        for lines in vehicle_inspection_lines:
            if not lines.next_inspection_date:  # ← ADD THIS GUARD
                continue
