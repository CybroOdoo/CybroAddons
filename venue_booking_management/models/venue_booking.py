...
class VenueBooking(models.Model):
    ...
    def create(self, vals_list):
        ...
        for values in vals_list:
            ...
            if not partner_id:
                raise UserError(_('Partner is required for booking'))
            ...
            # Set name
            date_value = values.get('date', fields.Date.today())
            values['name'] = '%s- %s' % (partner_name, date_value)

            # Set sequence
            if not values.get('ref'):
                values['ref'] = self.env['ir.sequence'].next_by_code(
                    'venue.booking.sequence') or '/'

        # Call super with the modified list
        return super().create(vals_list)

    def unlink(self):
        """Override unlink to prevent deletion if there is a posted invoice"""
        for rec in self:
            invoice = self.env['account.move'].search(
                [('invoice_origin', '=', rec.ref), ('state', '=', 'posted')])
            if invoice:
                raise UserError(
                    _('You cannot delete a booking that has a posted invoice.'))
        return super().unlink()
