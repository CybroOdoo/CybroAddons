...
export class OfflineSalesApp extends Component {
    ...
    exitOfflineTerminal() {
        ...
        window.location.href = "/odoo/sales";
    }
    ...
    async loadPendingOrders() {
        ...
        try {
            const orders = await rpc("/web/dataset/call_kw/sale.order/get_pending_sale_orders", {
                model: 'sale.order',
    ...
    payNow() {
        if (this.state.lastOrder) {
            ...
            // 3. Update history state visually
            let currentHistory = this.db.load('history', []);
            // Check if already in history (though shouldn't be with new flow) or just unshift
...
<script>
    ...
    async emailReceipt() {
        ...
        try {
            ...
            await rpc("/web/dataset/call_kw/sale.order/send_offline_receipt_mail", {
                model: 'sale.order',
    ...
    async syncOrders(silent = false, isManualFromTransactions = false) {
        ...
        if (!navigator.onLine) {
            ...
            if (isManualFromTransactions) {
                this.showPopup("You are currently offline. Please connect to the internet to sync.", "Offline", "error");
            }
            return;
        }
        ...
        if (!orders.length && !newPartners.length) {
            this.state.syncStatus = 'idle';
            if (isManualFromTransactions) {
                this.showPopup("No pending orders to sync at the moment.", "Info", "success");
            }
            return;
        }
        ...
        try {
            ...
            if (results.partners) {
                const successfulPartnerMaps = results.partners.filter(p => p.offline_id && p.id && !p.error);
                const successfullySyncedPartnerIds = successfulPartnerMaps.map(p => p.offline_id);
                ...
                successfulPartnerMaps.forEach(map => {
                    const o_partner = this.state.partners.find(p => p.id === map.offline_id);
                    if (o_partner) o_partner.id = map.id;
                    if (String(this.state.currentPartner) === String(map.offline_id)) this.state.currentPartner = map.id;

                    orders.forEach(o => {
                        if (o.partner_id === map.offline_id) o.partner_id = map.id;
                    });
                });

                if (successfulPartnerMaps.length) {
                    const cachedData = this.db.load('data', {});
                    cachedData.partners = this.state.partners;
                    this.db.save('data', cachedData);
                }
            ...
            const orderResults = results.orders || results;
            ...
            // === SHOW POPUP ONLY FOR MANUAL SYNC FROM TRANSACTIONS ===
            if (isManualFromTransactions) {
                if (failed.length === 0) {
                    this.showPopup(
                        `${successfullySyncedUids.length} order(s) successfully synced to backend!`,
                        "Sync Successful",
                        "success"
                    );
                } else {
                    this.showPopup(
                        `${successfullySyncedUids.length} order(s) synced successfully.\n${failed.length} failed to sync.`,
                        "Sync Completed",
            ...
            // Refresh pending orders if needed
            if (orders.some(o => o.backend_order_id)) {
                this.loadPendingOrders();
            }
            ...
            if (isManualFromTransactions) {
                this.showPopup("Sync failed. Please check your connection and try again.", "Sync Error", "error");
            }
        }
    }
