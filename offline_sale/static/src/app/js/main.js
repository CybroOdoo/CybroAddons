...
function _stripStandardOdooChrome() {
    ...
    document.querySelectorAll(selectors.join(",")).forEach((el) => {
        if (
            el.id !== "offline_sale_root" &&
            !el.contains(document.getElementById("offline_sale_root"))
        ) {
            el.remove();
        }
    });
...
async function startApp() {
    ...
    try {
        ...
        if (rootElement) {
            // Mount the Offline Sales root component.
            const app = await mountComponent(OfflineSalesApp, rootElement, {
                name: "Offline Sales",
            });

            // Remove the initial loading spinner if present.
            const loadingSpinner = rootElement.querySelector(".fa-circle-o-notch");
            if (loadingSpinner) {
                loadingSpinner.parentElement.parentElement.remove();
            }

            // Remove any backend UI that may have rendered.
            _stripStandardOdooChrome();

            /**
             * Observe DOM changes and continue removing backend chrome
             * while the Odoo WebClient finishes mounting.
             */
            ...
            // Stop observing after startup is complete.
