...
class MedicalReport(models.Model):
    ...
    report_type_id = fields.Many2one(
        ...
        domain=[('type', '=', 'service'), ('is_medical_test', '=', True)], required=True
    )
    ...
    is_invoiced = fields.Boolean(
        string='Invoiced',
        help="Indicates whether an invoice has been created for this report."
    )
    ...
    def action_create_invoice(self):
        ...
        for record in self:
            ...
            invoice = self.env['account.move'].create([{
                ...
                'partner_id': record.patient_id.owner_id.id,
                'currency_id': record.currency_id.id or self.env.company.currency_id.id,
                'medical_test_bill_invoice_id': record.id,
                'invoice_line_ids': lines
            }])
            record.medical_report_invoice_id = invoice.id
