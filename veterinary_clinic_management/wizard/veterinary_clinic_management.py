...
class VeterinaryClinicManagement(models.TransientModel):
    """
    This model defines a wizard for generating veterinary clinic management reports.
    The wizard allows users to select a service type (procedures, grooming, or training)
    and specify a date range for the report.
    """
    _name = 'veterinary.clinic.management'
    ...
    def action_pdf_report(self):
        ...
        if self.service_type == 'procedures':
            ...
            for record in records:
                data.append({
                    ...
                    'treatment': record.is_treated,
                    'is_admit': record.is_admitted,
                    'vaccination': record.is_vaccinated,
                    'status': record.state
                })
        ...
        return (self.env.ref('veterinary_clinic_management.action_veterinary_clinic_management_report').
