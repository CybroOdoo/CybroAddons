...
export class OdooDynamicDashboard extends Component {
    ...
    saveLayout(ev){
        ...
        self.orm.call('dashboard.block', 'get_save_layout', [[], data_list]).then(() => {
            self.render(true);
            self.notification.add("Layout saved", { type: "success" });
        });
    }
    ...
    async printPdf() {
        const dashboardEl = document.querySelector('.items');
        if (!dashboardEl) return;
        const tiles = dashboardEl.querySelectorAll('.resize-drag');
        const originalTransforms = [];
        tiles.forEach(tile => {
            originalTransforms.push(tile.style.transform);
            const x = parseFloat(tile.dataset.x || 0);
            const y = parseFloat(tile.dataset.y || 0);
        ...
        await html2pdf().set(opt).from(dashboardEl).save();

        tiles.forEach((tile, i) => {
            tile.style.position = '';
            tile.style.transform = originalTransforms[i];
            tile.style.left = '';
            tile.style.top = '';
        });
    }
