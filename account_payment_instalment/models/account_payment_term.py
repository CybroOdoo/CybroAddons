...
class AccountPaymentTermLine(models.Model):
    """ Inherit the model to add instalment fields """
    _inherit = "account.payment.term.line"

    value = fields.Selection([('balance', 'Balance'), ('percent', 'Percent'),
                              ('fixed', 'Fixed Amount'),
                              ('instalment', 'Instalment')], string='Type',
                             required=True, default='balance',
                             help="Select here the kind of valuation related "
                                  "to this payment term "
                                  "line.")
    period_type = fields.Selection(
        [('daily', 'Day(s)'), ('weekly', 'Week(s)'), ('monthly', 'Month(s)'),
         ('yearly', 'Year(s)'), ], string='Period Type', default='monthly')
    period_count = fields.Integer('Number of instalments', default=1)


class AccountPaymentTerm(models.Model):
    """ Inherit the model to override the functions """
    ...
    def _check_lines(self):
        ...
        if payment_term_lines and payment_term_lines[-1].value not in \
                ['balance', 'instalment']:
            raise ValidationError(
                _('A Payment Term should have its last line of type '
                  'Balance/Instalment.'))
        lines = self.line_ids.filtered(lambda r: r.value == 'balance') or []
        if len(lines) > 1:
            raise ValidationError(
                _('A Payment Term should have only one line of type '
                  'Balance.'))
        ...
        lines = self.line_ids.filtered(
            lambda r: r.value in ['balance', 'instalment']) or []
        if len(lines) > 1:
            raise ValidationError(
                _('A Payment Term should have only one of type Balance and '
                  'Instalment.'))
        lines = self.line_ids.filtered(lambda r: r.value == 'instalment') or []
        for line in lines:
            if line.period_count == 0:
                raise ValidationError(
                    _('A Payment Term of type Instalment should have number '
                      'of instalments more than 0.'))

    @api.one
    def compute(self, value, date_ref=False):
        """ Overiding the function to add additional codes of instalment"""
            ...
            if line.value != 'instalment' and amt:
                ...
                if line.option == 'day_after_invoice_date':
                    ...
                    next_date += relativedelta(day=31,
                                               months=1)  # Getting last day
                # of next month
                elif line.option == 'last_day_current_month':
                    next_date += relativedelta(day=31,
                                               months=0)  # Getting last day
                # of next month
                ...
                while count > 0:
                    ...
                    result.append(
                        (fields.Date.to_string(next_date), instalment_amount))
                    count -= 1
                amount -= amt
