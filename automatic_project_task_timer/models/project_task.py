...
class Task(models.Model):
                    ...
                    if time_line.date_start:
                        if time_line.date_end:
                            start_time = fields.Datetime.from_string(
                                time_line.date_start).replace(second=0,
                                                              microsecond=0)
                            end_time = fields.Datetime.from_string(
                                time_line.date_end).replace(second=0,
                                                            microsecond=0)

                            diff = end_time - start_time

                            # Calculate duration in minutes and hours
                            minutes_duration = round(
                                diff.total_seconds() / 60.0, 2)
                            hours_duration = round(
                                diff.total_seconds() / (60.0 * 60.0), 2)

                            # Format durations to two decimal places
                            time_line.timer_duration = f"{minutes_duration:05.2f}"
                            time_line.unit_amount = f"{hours_duration:05.2f}"
                        else:
                            time_line.unit_amount = "00.00"
                            time_line.timer_duration = "00.00"
                    else:
    ...
    def get_working_duration(self):
        ...
        self.ensure_one()
        duration = 0
        for time in self.timesheet_ids.filtered(lambda time: not time.date_end):
