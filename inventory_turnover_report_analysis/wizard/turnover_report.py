...
class TurnoverReport(models.TransientModel):
    ...
    def _date_comparison(self, data):
        ...
        for record in record_dict.values():
            record_date = record.get('last_count_date')
            if not record_date:
                continue
            if (
                    (not self.start_date and not self.end_date) or
                    (
                            self.start_date and self.end_date and self.start_date <= self.end_date
                            and self.start_date <= record_date <= self.end_date) or
                    (
                            self.start_date and not self.end_date and record_date >= self.start_date) or
                    (
                            not self.start_date and self.end_date and record_date <= self.end_date)
            ):
