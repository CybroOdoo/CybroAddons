...
class OnlineSubscription(http.Controller):
    ...
    def get_vehicle(self, **kwargs):
        ...
        state_id_val = kwargs.get('state_id', False)
        seating_capacity = kwargs.get('seating_capacity', 0)
        insurance_type_id_val = kwargs.get('insurance_type_id', False)
        state_record = False
        country_record = False
        if state_id_val:
            try:
                state_record = int(state_id_val)
                state = request.env['res.country.state'].sudo().browse(state_record)
                country_record = state.country_id.id if state.country_id else False
            except (ValueError, TypeError):
                state_record = False
        insurance_record = False
        if insurance_type_id_val:
            try:
                # insurance_type_id_val is the insurance.type id selected on page 1
                # Find the matching vehicle.insurance record for this vehicle
                ins = request.env['vehicle.insurance'].sudo().search(
                    [('vehicle_id', '=', vehicle_id.id),
                     ('insurance_type_id', '=', int(insurance_type_id_val))],
                    limit=1)
                insurance_record = ins.id if ins else vehicle_id.insurance
            except (ValueError, TypeError):
                insurance_record = vehicle_id.insurance
        else:
            insurance_record = vehicle_id.insurance
        subscribe = request.env['fleet.subscription'].sudo().create({
            'vehicle_id': vehicle_id.id,
            'customer_id': customer_id.id,
            'insurance_type_id': insurance_record,
            ...
            'city': city,
            'state_id': state_record or False,
            'country_id': country_record or False,
