...
publicWidget.registry.Map_1 = publicWidget.Widget.extend({
    ...
    async start() {
        ...
        const filter = this.el.querySelector('.s_map_color_filter') || this.el.querySelector('.s_map_color_filter_1');
        if (filter) {
            filter.before(iframeEl);
        } else {
            this.el.querySelector('.map_container').appendChild(iframeEl);
        }
    },
});
