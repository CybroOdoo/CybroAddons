...
class ImageCompressor(models.Model):
    ...
    def _get_attachments_domain(self, model_names):
        """
        Build and return the domain used to filter image attachments
    ...
    def _schedule_auto_compress(self):
        """
        Automatically compress image attachments based on active
        ...
        for record in i_c_rules:
            ...
            for rec in attach_ments:
                try:
                    if rec.raw:
                        img = ImageProcess(rec.raw, verify_resolution=False)
                    else:
                        if not rec.datas:
                            continue
                        img = ImageProcess(
