...
//    Extending Component
    ...
    const session_uid = session.storeData.Store.settings.user_id.id
        if (this.state.date.start_date && this.state.date.end_date) {
            var domain = [['user_id', '=', session_uid], ['state', '=', 'draft'], ['date_order', '>=', this.state.date.start_date], ['date_order', '<=', this.state.date.end_date]]
        }
        else if (this.state.date.start_date) {
            var domain = [['user_id', '=', session_uid], ['state', '=', 'draft'], ['date_order', '>=', this.state.date.start_date]]
        }
        else if (this.state.date.end_date) {
            var domain = [['user_id', '=', session_uid], ['state', '=', 'draft'], ['date_order', '<=', this.state.date.end_date]]
        }
        else {
            var domain = [['user_id', '=',session_uid ], ['state', '=', 'draft']]
        }
    ...
    const session_uid = session.storeData.Store.settings.user_id.id
        if (this.state.date.start_date && this.state.date.end_date) {
            var domain = [['user_id', '=', session_uid], ['state', '=', 'sale'], ['date_order', '>=', this.state.date.start_date], ['date_order', '<=', this.state.date.end_date]]
        }
        else if (this.state.date.start_date) {
            var domain = [['user_id', '=', session_uid], ['state', '=', 'sale'], ['date_order', '>=', this.state.date.start_date]]
        }
        else if (this.state.date.end_date) {
            var domain = [['user_id', '=', session_uid], ['state', '=', 'sale'], ['date_order', '<=', this.state.date.end_date]]
        }
        else {
            var domain = [['user_id', '=', session_uid], ['state', '=', 'sale']]
        }
        return this.action.doAction({
            type: "ir.actions.act_window",
            name: 'Sale Orders',
    ...
    const session_uid = session.storeData.Store.settings.user_id.id
        if (this.state.date.start_date && this.state.date.end_date) {
            var domain = [['user_id', '=', session_uid], ['state', '=', 'sent'], ['date_order', '>=', this.state.date.start_date], ['date_order', '<=', this.state.date.end_date]]
        }
        else if (this.state.date.start_date) {
            var domain = [['user_id', '=', session_uid], ['state', '=', 'sent'], ['date_order', '>=', this.state.date.start_date]]
        }
        else if (this.state.date.end_date) {
            var domain = [['user_id', '=', session_uid], ['state', '=', 'sent'], ['date_order', '<=', this.state.date.end_date]]
        }
        else {
            var domain = [['user_id', '=', session_uid], ['state', '=', 'sent']]
        }
        return this.action.doAction({
            type: "ir.actions.act_window",
    ...
    const session_uid = session.storeData.Store.settings.user_id.id
        var domain = [['user_id', '=', session_uid], ['state', '=', 'cancel']]
        return this.action.doAction({
            type: "ir.actions.act_window",
    ...
    const session_uid = session.storeData.Store.settings.user_id.id
        if (this.state.date.start_date && this.state.date.end_date) {
            var domain = [['user_id', '=', session_uid], ['invoice_status', '=', 'to invoice'], ['date_order', '>=', this.state.date.start_date], ['date_order', '<=', this.state.date.end_date]]
        }
        else if (this.state.date.start_date) {
            var domain = [['user_id', '=', session_uid], ['invoice_status', '=', 'to invoice'], ['date_order', '>=', this.state.date.start_date]]
        }
        else if (this.state.date.end_date) {
            var domain = [['user_id', '=', session_uid], ['invoice_status', '=', 'to invoice'], ['date_order', '<=', this.state.date.end_date]]
        }
        else {
            var domain = [['user_id', '=', session_uid], ['invoice_status', '=', 'to invoice']]
        }
        return this.action.doAction({
            type: 'ir.actions.act_window',
            name: 'To Invoice',
