...
class SaleOrder(models.Model):
    ...
    @api.depends('signature', 'settings_approval')
    def _compute_check_signature(self):
        """Check if signature is present when approval is enabled"""
        for rec in self:
            rec.check_signature = not rec.settings_approval or bool(rec.signature)
    ...
    def _compute_settings_approval(self):
        ...
        for rec in self:
            approval_enabled = rec.env['ir.config_parameter'].sudo().get_param(
                'all_in_one_sales_kit.sale_document_approve')
            rec.settings_approval = bool(approval_enabled)
    ...
    def action_confirm(self):
        ...
        if to_approve:
            approval_users = self.env['res.users'].search([
                ('group_ids', 'in', [self.env.ref('all_in_one_sales_kit.group_approval_manager').id])
            ])
            display_id = self.id
