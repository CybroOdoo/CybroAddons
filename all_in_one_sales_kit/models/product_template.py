...
class ProductTemplate(models.Model):
    ...
    def create(self, values):
        records_values = values if isinstance(values, list) else [values]

        for vals in records_values:
            if vals.get('is_pack'):
                if not vals.get('pack_products_ids'):
                    raise UserError(_('You need to add atleast one product in the Pack...!'))
