...
class IrAttachment(models.Model):
    ...
    def save_edited_image(self, attachment_id, myImage):
        ...
        image_b64 = myImage
        if myImage and "," in myImage:
            image_b64 = myImage.split(",", 1)[1]
        file.write({'datas': image_b64})
        return file

    @api.model
    def generate_qr_code(self, attach_id):
        """Generate qr code for attachment that allows anyone to download it."""
        base_url = self.env['ir.config_parameter'].sudo().get_param('web.base.url')
        if not base_url.endswith('/'):
            base_url += '/'
        data = {}
        # Use the standard /web/content URL so the QR code always resolves
        download_url = f"web/content/{attach_id}?download=true"
        if qrcode and base64:
            ...
            # Decode bytes → str so the value is JSON-serialisable
