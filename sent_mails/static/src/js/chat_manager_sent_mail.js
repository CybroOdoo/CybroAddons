...
function make_message (data) {
    ...
    Object.defineProperties(msg, {
        is_sent: property_descr("channel_sent"),
    ...
    if (_.contains(data.author_id, session.partner_id)) {
        msg.is_sent = true;
    }
    if (msg.model === 'mail.channel') {
        var real_channels = _.without(msg.channel_ids, 'channel_inbox', 'channel_starred', 'channel_sent');
...
function fetch_from_channel (channel, options) {
    ...
    var domain =
        ...
        (channel.id === "channel_sent") ? [['subtype_id', '=', 'Discussions']] :
