...
class ProductPriceList(models.Model):
    """Inherits the ProductPriceList class for adding new fields and methods"""
    ...
    def check_pricelist_condition(self):
        """Check if product price list condition based on the variant and fields"""
        if self.item_ids:
                ...
                if not field_id:
                    field_id = self.env['ir.model.fields'].sudo().create({
                        'name': 'x_' + str(name) + str(
                            self.id) + pricelist_name,
                        'field_description': pricelist_name,
                        'model_id': model_id,
                        'ttype': 'float',
                    })
                view = self.env['ir.ui.view'].search([('name', '=',
                                                       'product.dynamic.fields.%s' % pricelist_name),
                                                      ('model', '=', model)])
                if not view:
    ...
    def create(self, vals):
        """Super the create function for calling the created checklist method"""
    ...
    def write(self, vals):
        """Super the write function for calling the created checklist method"""
    ...
    def unlink(self):
        """Super the unlink function for updating the pricelist"""
        for rec in self.item_ids:
            ...
            self.env['ir.model.fields'].search(
                [('model_id', '=', model_id),
                 ('name', '=',
                  'x_' + str(name) + str(self.id) + pricelist_name)]).unlink()
        return super().unlink()
