...
class WebsiteSales(WebsiteSale):
    ...
    def shop(self, page=0, category=None, search='', min_price=0.0,
             max_price=0.0, ppg=False, brand=None, products_prices=None,
             **post):
        """ Defined a url for product brands inside the existing function
        and controller of WebsiteSale """
        res = super(WebsiteSales, self).shop(page=page, category=category,
                                             search=search, min_price=min_price,
                                             max_price=max_price, ppg=ppg,
                                             brand=brand,products_prices=products_prices,
                                             **post)
        ...
        if brand:
            ...
            pager_brand = request.website.pager(url=url,
                                                ...
                                                url_args=post)
            products_prices = lazy(lambda: products_brand._get_sales_prices(website))
