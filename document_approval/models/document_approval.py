...
class DocumentApproval(models.Model):
    ...
    @api.depends('approver_ids', 'team_id.team_lead_id', 'state')
    @api.depends_context('uid')
    def _compute_show_approve(self):
        """This method _compute_show_approve to compute the value to the field
        show_approve"""
        for rec in self:
            rec.show_approve = (self.env.uid in rec.approver_ids.ids) or \
                               (rec.team_id.team_lead_id.id == self.env.uid)
