                    ...
                    try:
                        ...
                        for col in ws.iter_rows(min_row=2, values_only=True):
                            ...
                            if col[1]:
                                ...
                                if col[2]:
                                    ...
                                    if col[4]:
                                        ...
                                        bom_data.update({'product_uom_id': variant.uom_id.id})
                            ...
                            if col[5] and last_bom_id:
                                ...
                                if col[6]:
                                    bom_line_dict.update({'product_qty': col[6]})
                                else:
                                    bom_line_dict.update({'product_qty': 1})
                                ...
                                if col[7]:
                                    uom = self.env['uom.uom'].search(
                                        [('name', '=', col[7])], limit=1)
                                    bom_line_dict.update({'product_uom_id': uom.id})
                                else:
                                    bom_line_dict.update({'product_uom_id': variant.uom_id.id})
                        ...
                        raise UserError(
                            _("Sorry, The Excel file you provided does not match our required format\nError: %s") % ustr(
                                e)
                        )
