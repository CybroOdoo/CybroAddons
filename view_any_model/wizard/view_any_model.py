...
class ViewAnyModel(models.TransientModel):
    ...
    view_type = fields.Selection(
        selection=[('list', 'List'), ('form', 'Form')], string='View Type',
        default='list', required=True, help='Select the Type of view')
    ...
    def action_view_model(self):
        """
        View the records of selected model in list or form view
        :return: list/form view of selected model
        """
        try:
            if self.view_type == 'form':
                if self.record_id < 1:
                    raise UserError("ID should be a positive integer")
                if not self.env[self.model_name].sudo().search(
                    ...
                    raise UserError("Enter ID of existing record")
            action = {
            ...
            raise UserError(f'You are not allowed to access {self.model_name} records.')
