...
class InheritContacts(models.Model):
    ...
    def _compute_partner_type(self):
        ...
        This method computes the 'partner_type' field based on certain conditions.
        """
        for rec in self:
            sale_order_count = getattr(rec, "sale_order_count", 0)
            purchase_order_count = getattr(rec, "purchase_order_count", 0)
                ...
                for rec in contact_sync:
                    ...
                    for i in range(
                        ...
                        if self.env['mailer.cloud.properties'].search(
                                [('id', '=', rec.contact_mapping_ids.mapped(
                                        'property_id')[i].id)]).mailer_cloud:
                            contact_details_dict['custom_fields'] = {
                                rec.contact_mapping_ids.mapped(
                                    'property_id.mailer_cloud')[i]:
                                    res.read([rec.contact_mapping_ids.mapped(
                                        'contact_fields')[i]])[0][
                                        rec.contact_mapping_ids.mapped(
                                            'contact_fields')[i]] or ' '}
                ...
                for rec in contact_sync.contact_mapping_ids:
                    if key == rec.contact_fields and rec.property_id.mailer_cloud:
                        update_dict['custom_fields'] = {
                            rec.property_id.mailer_cloud: round(vals) if type(
