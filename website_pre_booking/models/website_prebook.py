...
class WebsitePrebook(models.Model):
    ...
    _description = 'Website PreBooking'
    _rec_name = 'reference'
    ...
    def create(self, vals):
        ...
        if isinstance(vals, list):
            ...
            for rec, v in zip(recs, vals):
                if not rec.reference or rec.reference == ('New'):
                    rec.reference = v.get('reference') or self.env['ir.sequence'].next_by_code('website.prebook') or (
                    'New')
            return recs
        if not vals.get('reference') or vals.get('reference') == ('New'):
            vals['reference'] = self.env['ir.sequence'].next_by_code('website.prebook') or ('New')
