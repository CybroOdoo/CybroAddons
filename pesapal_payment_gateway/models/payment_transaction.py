...
class PaymentTransaction(models.Model):
    ...
    def _pesapal_submit_order_request(self, token, odoo_base_url, ipn_id):
        ...
        # Normalise: strip leading '+' or country code prefix
        ...
        # Use a unique UUID per submission so Pesapal never rejects it as
        # a duplicate (same order retried). We store the UUID in
        # provider_reference so the callback lookup uses it.
    ...
    def _get_tx_from_notification_data(self, provider_code, notification_data):
        ...
        # Look up by provider_reference (the UUID we sent as order id)
        tx = self.search(
            [('provider_reference', '=', order_merchant_ref),
             ('provider_code', '=', 'pesapal')], limit=1)
        if not tx:
            # Fallback: try matching by reference (legacy/first transaction)
            tx = self.search(
    ...
    def _process_notification_data(self, notification_data):
        ...
        if payment_status == 'Completed':
            ...
            # Payment is still pending/processing — set pending so Odoo
            # knows to show "payment is being processed" on /payment/status.
            # This is the correct time to call _set_pending (after the user
            # has completed the form on Pesapal, not during rendering).
            self._set_pending()
