...
class ResCompany(models.Model):
    ...
    def action_sync_products(self):
        if self.product_synced:
            return
        if not self.api_key:
            raise ValidationError('Please Enter an API Key')
        self.get_products()
        ...
        for product in self.env['product.template'].sudo().search(
            ...
            if not response.json()['success']:
                raise ValidationError(
                    response.json()['error'] + '. ' + response.json()[
                        'error_info'])
            pipedrive_id = str(response.json()['data']['id'])
            product.with_context(skip_pipedrive_sync=True).write({
    ...
    def create_webhook(self, event_action, url, event_object):
        ...
        if not base_url.startswith('https://'):
            raise ValidationError(
                "web.base.url must be HTTPS for Pipedrive webhooks"
            )

        payload = {
            "subscription_url": base_url + url,
        ...
        for webhook in existing:
            if (
                    webhook.get('event_action') == event_action and
                    webhook.get('event_object') == event_object and
                    webhook.get('subscription_url') == base_url + url
            ):
                return
        ...
        if not response.json().get('success'):
            raise ValidationError(
                response.json().get('error', 'Webhook creation failed')
            )
                ...
                if not response.json()['success']:
                    raise ValidationError(
                        response.json()['error'] + '. ' + response.json()[
                            'error_info'])
                pipedrive_id = str(response.json()['data']['id'])
                lead.with_context(skip_pipedrive_sync=True).write({
                    'pipedrive_reference': pipedrive_id
                })
                self.env['pipedrive.record'].sudo().create({
                    'pipedrive_reference': pipedrive_id,
