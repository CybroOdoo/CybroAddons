...
class PaymentPost(PaymentPostProcessing):
    ...
    def poll_status(self, **kwargs):
        ...
        if movie_booking_data and res['state'] == 'done':
            ...
            registration_id = movie_booking_data.get('registration_id')
            if registration_id:
                # Updating existing movie.registration record
                movie_ticket = request.env['movie.registration'].sudo().browse(int(registration_id))
                # Delete old seats if any
                movie_ticket.seat_ids.sudo().unlink()
                movie_ticket.sudo().write({
                ...
                # Creating NEW movie.registration record
