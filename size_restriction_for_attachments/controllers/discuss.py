...
class DiscussController(DiscussController):
    ...
    @http.route('/mail/attachment/upload', methods=['POST'],
                type='http', auth='public')
    def mail_attachment_upload(self, ufile, thread_id,
                               thread_model, is_pending=False, **kwargs):
        """ Override the function mail_attachment_upload() to include a check
        to see if the size of the attachment exceeds the user-assigned maximum
        size. If true, an error message is displayed."""
        ...
        if channel_partner.env.user.share:
            # Only generate the access token
            # if absolutely necessary (= not for internal user).
            vals['access_token'] = \
                channel_partner.env['ir.attachment']._generate_access_token()
        try:
            ...
            attachmentData = {'error': _("You are not allowed "
                                         "to upload an attachment here.")}
