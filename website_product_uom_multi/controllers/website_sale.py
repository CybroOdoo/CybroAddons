...
class WebsiteProductUom(Cart):
    ...
    @http.route(['/shop/cart/update_json'], type='jsonrpc', auth="public",
    ...
    def cart_update_json(
        ...
        # Convert uom_id to integer if it's a string
        if uom_id:
            uom_id = int(uom_id)

        # Get product to check UOM
        ...
        # IMPORTANT: Update the line with correct UOM if it wasn't applied
        if uom_id and values.get('line_id'):
            order_line = request.env['sale.order.line'].browse(values['line_id'])
            if order_line.product_uom_id.id != uom_id:
                uom = request.env['uom.uom'].browse(uom_id)
                # Recalculate price based on UOM
                order_line.write({
    ...
    class WebsiteProductVariant(Cart):
        """
        ...
        def cart_options_update_json(self, product_and_options, lang=None,
            ...
            if product_and_options:
                ...
                # Get UOM ID if present
                uom_id = main_product.get('uom_id')
                if uom_id:
                    uom_id = int(uom_id)
                ...
                # Ensure UOM is correctly set on the line
                if uom_id and values.get('line_id'):
                    order_line = request.env['sale.order.line'].browse(values['line_id'])
                    if order_line.product_uom_id.id != uom_id:
                        order_line.write({
