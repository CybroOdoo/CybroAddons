    ...
    def home(self, **kw):
        ...
        if group_id in partners.groups_id:
            if show_so_q:
                ...
                limit_so = int(number_so)
                sale_orders_limited = sale_order.search([
                    ('user_id', '=', user),
                    ('state', 'not in', ['draft', 'sent'])
                ], limit=limit_so) if limit_so > 0 else sale_order.browse()
            ...
            if show_project:
                ...
                limit_project = int(number_project)
                projects_limited = project.search([],
                                                  limit=limit_project) if limit_project > 0 else project.browse()
                tasks_limited = task.search([('project_id', '!=', False)], limit=limit_project) if limit_project > 0 else task.browse()

            if show_account:
                number_account = config_parameters.get_param(
                    'portal_dashboard.account_count', 0)
                limit_account = int(number_account)
                invoices_limited = account_move.search([
                    ('move_type', 'in', ('out_invoice', 'out_refund', 'in_invoice', 'in_refund', 'out_receipt', 'in_receipt')),
                    ('state', 'not in', ['draft', 'cancel'])
                ], limit=limit_account) if limit_account > 0 else account_move.browse()
            ...
            if show_project:
                ...
                projects_limited = project.search([],
                                                  limit=limit_project) if limit_project > 0 else project.browse()
                tasks_limited = task.search([('project_id', '!=', False)],
                                            limit=limit_project) if limit_project > 0 else task.browse()
