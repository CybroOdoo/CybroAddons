    ...
    def action_execute_query(self):
        ...
        try:
            if self.query_name:
                ...
                table_datas = ''
                for key in keys:
                    table_header += "<th class='p-3' style='border:1px solid black !important'>%s</th>" % key
                query_result = self._cr.fetchall()
                for query_res in query_result:
                    table_datas += "<tr>"
                    for res in query_res:
                        table_datas += "<td class='p-3' style='border:1px solid black !important'>{0}</td>".format(res)
                    table_datas += "</tr>"
                self.query_result = """<div style="overflow:auto;"><table class="table table-striped text-center table-border table-sm"
                             style="width:max-content";><thead>
                            <tr style='border:1px solid black !important;background: #71639e;color: white;'>
