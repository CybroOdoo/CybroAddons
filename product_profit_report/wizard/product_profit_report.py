...
class ProductProfitReport(models.TransientModel):
    """Model for the wizard to print the report"""
    ...
    from_date = fields.Date(
        string='From Date', default=_get_from_date,
        required=True, help="From which date report need to print"
    )
    to_date = fields.Date(
        string='To Date', default=fields.Date.context_today, required=True,
    ...
    product_id = fields.Many2one(
        'product.product', string='Product', help="Select the Product"
    )
    ...
    def action_print_pdf_report(self):
        ...
        data = {'form': self.read([])[0] if self else {}}
        return self.env.ref(
