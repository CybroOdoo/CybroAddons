...
class SaleOrderLine(models.Model):
    ...
    recipient_id = fields.Many2one('res.partner', string='Recipient',
                                   ...
                                   domain=['|',
                                           ('company_id', '=',
                                            lambda self: self.env.company.id),
                                           ('company_id', '=', False)])
    ...
    def _prepare_procurement_values(self, group_id=False):
        ...
        could be overridden in order to add other custom key that could be used
        in move/po creation."""
        ...
        if self.order_id.delivery_split and self.recipient_id:
            values.update({"partner_id": self.recipient_id.id})
        else:
            values.update({"partner_id": self.order_id.partner_id.id})
        return values
