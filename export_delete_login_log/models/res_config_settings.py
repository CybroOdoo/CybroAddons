...
class ResConfigSettings(models.TransientModel):
    ...
    delete_log_models_ids = fields.Many2many('ir.model',
                                             ...
                                             help="Select the models for which delete operations will be logged")
    have_api_key = fields.Boolean(string='Have API Key',
                                  config_parameter='export_delete_login_log'
                                                   '.have_api_key',
                                  help="Enable this option if you have an ipapi API key for fetching precise IP location data")
    ipapi_key = fields.Char(string='API Key',
                            config_parameter='export_delete_login_log'
                                             '.ipapi_key',
                            help="API key from ipapi used to fetch geographical information for login logs")

    def set_values(self):
        """Set values to delete_log_models_ids Many2many field"""
        res = super().set_values()
        if self.env.user in self.env.ref(
                'export_delete_login_log.group_export_log_manager').user_ids:
    ...
    def get_values(self):
        ...
        tracked_models = with_user.get_param(
            'export_delete_login_log.delete_log_models_ids')
        res.update(delete_log_models_ids=[Command.set(literal_eval(tracked_models))
