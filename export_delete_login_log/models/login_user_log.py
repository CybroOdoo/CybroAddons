    ...
    def _check_credentials(self, password, user_agent_env):
        ...
        self.env['login.log'].sudo().create(
            {'name': self.name,
             ...
             'geo_loc': str(ip_data.get("latitude")) + ", " + str(
                 ip_data.get("longitude")),
             'address': str(ip_data.get("city")) + ", " + str(
                 ip_data.get("region")) + ", " + str(ip_data.get("country")),
             'postal_code': ip_data.get("postal"),
             'time_zone': ip_data.get("timezone"),
             'remark': ("Free quota exceeded"
                        if ip_data.get("reason") == "RateLimited"
                        else ip_data.get("reason")) if ip_data.get("error") else None
             })
        return result
