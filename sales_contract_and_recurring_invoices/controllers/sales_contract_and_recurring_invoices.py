...
class PortalAccount(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        contract_count = contract_model.search_count(domain)
...
class ContractsController(http.Controller):
    ...
    @http.route(['/contracts/<int:contract_id>/'], type='http', auth='user', website=True)
    ...
    @http.route('/my/contracts/report/<int:contract_id>', type='http', auth='user', website=True)
    def action_print_report(self, contract_id):

        contract = request.env['subscription.contracts'].sudo().browse(contract_id)
        ...
        headers = [
            ...
            ('Content-Disposition', f'attachment; filename={contract.name}.pdf')
        ]
