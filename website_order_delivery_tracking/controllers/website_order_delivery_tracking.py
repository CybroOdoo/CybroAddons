...
class Tracking(http.Controller):
    ...
    def get_track_details(self):
        """Track data"""
        return request.render('website_order_delivery_tracking.trackingTemplate')

    @http.route(['/tracking/details/update'], type='jsonrpc', auth="user",
                website=True)
    ...
    def track_data_edit(self, **post):
        ...
        if track_data and post.get('api_key') == tracking_api:
            for track in track_data:
                track.write({
                    'tracking_status': post.get('tracking_status'),
                })
            return track_data[:1].tracking_status
        else:
            return []
