...
class FranchisePortal(http.Controller):
    ...
    def display_franchise_registration(self):
        ...
        domain = [('state', 'in', ('e_contract', 'f_signed', 'g_declined'))]
        # If not admin, restrict records to portal user
        if not request.env.user.has_group('base.group_system'):
            domain.append(
                ('dealer_portal_user', '=', request.env.user.id)
            )
        franchise_registration = request.env[
            'franchise.dealer'
        ].sudo().search(domain)
        dealer_dict = {
            'franchise_registration': franchise_registration,
            'page_name': 'franchise_registrations'
        }
        return request.render(
            "franchise_management.portal_my_franchise",
            dealer_dict
        )
    ...
    @http.route(['/my/franchise/<int:dealer_id>/accept'], type='jsonrpc', auth="public",
