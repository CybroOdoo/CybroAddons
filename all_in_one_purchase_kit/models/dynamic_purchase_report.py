...
class DynamicPurchaseReport(models.Model):
    ...
    def _get_report_sub_lines(self, data):
        ...
        if data.get('report_type') == 'report_by_order':
            ...
            for line in report_by_order:
                line['amount_formatted'] = '{:,.2f}'.format(
                    line.get('amount_total') or 0
                )
            ...
            for line in report_by_order_details:
                line['amount_formatted'] = '{:,.2f}'.format(
                    line.get('amount_total') or 0
                )
            ...
            for line in report_by_categories:
                line['amount_formatted'] = '{:,.2f}'.format(
                    line.get('amount_total') or 0
                )
            ...
            for line in report_by_purchase_representative:
                line['amount_formatted'] = '{:,.2f}'.format(
    ...
    def get_purchase_xlsx_report(self, data, response, report_data, dfr_data):
        ...
        if filters.get('report_type') == 'report_by_order':
            sheet.merge_range('B5:D5', 'Report Type: Report By Order', txt_l)
            ...
            for rec_data in report_data_main:
                ...
                sheet.write(row, col + 5, rec_data['amount_total'], amount_format)
        if filters.get('report_type') == 'report_by_order_detail':
            sheet.merge_range('B5:D5', 'Report Type: Report By Order Detail', txt_l)
            sheet.write('A7', 'Order', heading)
            sheet.write('B7', 'Date Order', heading)
            sheet.write('C7', 'Customer', heading)
            sheet.write('D7', 'Purchase Representative', heading)
            ...
            for rec_data in report_data_main:
                ...
                sheet.write(row, col + 8, rec_data['amount_total'], amount_format)
        if filters.get('report_type') == 'report_by_product':
            sheet.merge_range('B5:D5', 'Report Type: Report By Product', txt_l)
            ...
            for rec_data in report_data_main:
                ...
                sheet.write(row, col + 4, rec_data['amount_total'], amount_format)
        if filters.get('report_type') == 'report_by_categories':
            sheet.merge_range('B5:D5', 'Report Type: Report By Categories', txt_l)
            ...
            for rec_data in report_data_main:
                ...
                sheet.write(row, col + 2, rec_data['amount_total'], amount_format)
        if filters.get('report_type') == 'report_by_purchase_representative':
            sheet.merge_range('B5:D5', 'Report Type: Report By Purchase Representative', txt_l)
            ...
            for rec_data in report_data_main:
                ...
                sheet.write(row, col + 3, rec_data['amount'], amount_format)
        if filters.get('report_type') == 'report_by_state':
            sheet.merge_range('B5:D5', 'Report Type: Report By State', txt_l)
            ...
            for rec_data in report_data_main:
                ...
                if rec_data['state'] == 'draft':
                    sheet.write(row, col, 'RFQ', txt_l)
                elif rec_data['state'] == 'sent':
                    sheet.write(row, col, 'RFQ Sent', txt_l)
                elif rec_data['state'] == 'purchase':
                    sheet.write(row, col, 'Purchase Order', txt_l)
                elif rec_data['state'] == 'to approve':
                    sheet.write(row, col, 'To Approve', txt_l)
                elif rec_data['state'] == 'done':
                    sheet.write(row, col, 'Locked', txt_l)
                elif rec_data['state'] == 'cancel':
                    sheet.write(row, col, 'Cancelled', txt_l)
                sheet.write(row, col + 1, rec_data['order'], txt_l)
                sheet.write(row, col + 2, rec_data['qty'], txt_l)
                sheet.write(row, col + 3, rec_data['amount'], amount_format)
        workbook.close()
