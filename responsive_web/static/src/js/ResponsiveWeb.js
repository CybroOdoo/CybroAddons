...
import { Component, proxy, signal, onWillStart, onWillUnmount } from "@odoo/owl";
...
class WebResponsive extends Component {
    ...
    setup() {
        ...
        onWillStart(async () => {
            this.state.menus = this.menuService.getApps();
     ...
     */
     onKeyDown(event) {
        ...
        if (!this.state.showModal && !isTyping && /^[a-zA-Z0-9]$/.test(event.key)) {
            ...
            setTimeout(() => {
                const inputEl = typeof this.searchInput === 'function' ? this.searchInput() : null;
                if (inputEl) {
                    inputEl.focus();
                } else {
                    const rootEl = typeof this.root === 'function' ? this.root() : this.root?.el;
                    rootEl?.querySelector(".responsive_search_input")?.focus();
                }
            }, 100);
            return;
        }
