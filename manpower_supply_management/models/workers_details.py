...
class WorkersDetails(models.Model):
    ...
    def create(self, vals):
        ...
        partner = self.env['res.partner'].create({
            'name': vals[0]['name'],
            'phone': vals[0]['phone_number'],
            'email': vals[0]['email'],
            'image_1920': vals[0]['image_worker'],
        })
        vals[0]['related_partner_id'] = partner.id
        self.related_partner_id = vals[0]['related_partner_id']
    ...
    def get_details_amount(self, option):
        ...
        if option == 'daily':
            ...
            labour_supply = labour_supply.filtered(
                lambda
                    record: record.from_date and first_day <= record.from_date <= last_day)
        elif option == "yearly":
            labour_supply = labour_supply.filtered(
                lambda
                    record: record.from_date and first_month <= record.from_date <= last_month)
