...
class LabourSupplyReport(models.TransientModel):
    ...
    filter = fields.Selection(
        string="Filter", default="customer_wise",
    ...
    def action_print_pdf(self):
        ...
        if self.filter == 'customer_wise':
            ...
            for data in datas:
                data['from_date'] = data['from_date'].strftime(
                            '%d/%m/%Y')
                data['to_date'] = data['to_date'].strftime('%d/%m/%Y')
            data = {
                'form': self.read()[0],
                'datas': datas,
                'customer_id': self.customer_id.name,
                'from_date': self.from_date.strftime('%d/%m/%Y') if
                self.from_date else self.from_date,
                'to_date': self.to_date.strftime('%d/%m/%Y') if self.to_date
                else self.to_date
            }
            return self.env.ref(
