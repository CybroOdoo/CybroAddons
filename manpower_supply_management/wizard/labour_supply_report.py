...
class LabourSupplyReport(models.TransientModel):
    ...
    def action_print_pdf(self):
        ...
        if self.filter == 'customer_wise':
            ...
            WHERE 1=1"""
            if self.customer_id:
                query += f" AND res_partner.id = {self.customer_id.id:d}"
            if self.from_date:
                query += f" AND labour_supply.from_date >= '{self.from_date}'"
            if self.to_date:
        ...
        if self.filter == 'state_wise':
            query = "SELECT * FROM labour_supply WHERE 1=1"
            if self.state_id:
                query += f" AND state = '{self.state_id}'"
            if self.from_date:
                query += f" AND from_date >= '{self.from_date}'"
            if self.to_date:
                query += f" AND to_date <= '{self.to_date}'"
            
            self.env.cr.execute(query)
            datas = self.env.cr.dictfetchall()
            
            if not datas:
                raise UserError("No data found for the selected state and dates.")
