...
class VendorRFQ(models.Model):
        ...
        for vals in vals_list:
            if vals.get('name', 'New') == 'New':
                vals['name'] = self.env['ir.sequence'].next_by_code(
                    'vendor.rfq') or '/'
            orders |= super(VendorRFQ, self).create(vals)
        return orders
    ...
    def action_send_by_mail(self):
        ...
        template = self.env.ref(
            'vendor_portal_odoo.email_template_vendor_rfq_request')
        ...
        order = self.env['purchase.order'].sudo().create({
            ...
            'order_line': [
                (0, 0, {
                    ...
                    'allowed_uom_ids': [(6, 0, [self.product_id.uom_id.id])] or [(6, 0, self.product_id.uom_ids.ids)],
                    'price_unit': price,
