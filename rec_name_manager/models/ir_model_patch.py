...
class IrModelRecNamePatch(models.AbstractModel):
    ...
    def read(self, fields=None, load='_classic_read'):
        ...
        # Guard: the configured field must still exist on this model.
        # If a module was removed after the config was saved the field
        # may no longer exist — clear the stale cache and fall back.
        if field_name not in self._fields:
            _logger.warning('rec_name_manager: field %r no longer exists on %r. Stale cache cleared.',
    ...
    def name_get(self):
        ...
        for record in self:
            try:
                ...
                _logger.warning('rec_name_manager: name_get() error id=%s: %s', record.id, e)
                default = super(IrModelRecNamePatch, record).name_get()
                result.append(default[0] if default else (record.id, str(record.id)))
        return result
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        ...
        try:
            if name:
                if field_ttype == 'many2one':
                    # Cannot search into a many2one sub-field generically;
                    # fall back to searching the default _rec_name field.
