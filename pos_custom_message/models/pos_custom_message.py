...
class PosCustomMessage(models.Model):
    ...
    def _load_pos_data_domain(self, data, config):
        """Only load messages that are linked to the current POS config."""
        selected_message_ids = self.env['ir.config_parameter'].sudo().get_param('pos_custom_message.message_ids')
        domain = [('pos_config_ids', 'in', config.id)]
        if selected_message_ids:
            domain.append(('id', 'in', literal_eval(selected_message_ids)))
    ...
    def _load_pos_data_fields(self, config):
        """Fields to expose to the POS frontend."""
        return ['id', 'message_type', 'title', 'message_text',
                'execution_time', 'pos_config_ids']
