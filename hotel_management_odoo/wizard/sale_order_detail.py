...
class SaleOrderWizard(models.TransientModel):
    ...
    def _get_report_values(self, docids, data=None):
        """Pass custom data to the QWeb PDF template context."""
        return {
            'booking': data.get('booking', []) if data else [],
        }
...
class ReportSaleOrder(models.AbstractModel):
    """Report context model for Sale Order PDF.

    Odoo resolves the QWeb rendering context by looking for a model named
    'report.<report_name>', i.e. 'report.hotel_management_odoo.report_sale_order'.
