...
class PackersAndMoversManagement(http.Controller):
    ...
    def truck_booking(self):
        ...
        return http.request.render(
            'packers_and_movers_management.truck_booking_page',
            {'truck_ids': truck_ids,
             'goods_ids': goods_ids,
             'state_ids': state_ids,
    ...
    def booking_success_page(self, **post):
        ...
        existing_partner = request.env['res.partner'].sudo().search(
            [('mobile', '=', post.get('mobile_no'))], limit=1)
        if existing_partner:
        ...
        return request.render(
            'packers_and_movers_management.truck_booking_success_page',
    ...
    @http.route('/get_states/<model("res.country"):country>', type='json',
                auth='public', website=True)
    def get_states(self, country):
        """Function to get the selected country"""
        state_ids = request.env['res.country.state'].sudo().search(
            [('country_id', '=', country.id)])
        states_data = [{'id': state.id, 'name': state.name} for state in
                       state_ids]
        return states_data
