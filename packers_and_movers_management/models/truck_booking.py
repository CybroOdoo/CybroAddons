...
class TruckBooking(models.Model):
    ...
    company_id = fields.Many2one('res.company', string='Company',
                                 ...
                                 help='Select the company to which this record'
                                      'belongs.')
    ...
    distance = fields.Float(string='Distance(KM)', compute='_compute_distance',
    ...
    def action_view_invoice(self):
        """Smart button to view the Invoices for the truck_booking"""
        return {
            'type': 'ir.actions.act_window',
            'name': 'Invoice',
            'view_mode': 'tree,form',
    ...
    def get_booking_analysis(self):
        ...
        self.env.cr.execute('''select pr.name,sum(tb.amount)
        from truck_booking as tb
        inner join res_partner as pr on pr.id = tb.partner_id
        group by name''')
    ...
    def get_truck_analysis(self):
        ...
        self.env.cr.execute('''select fv.name,sum(tb.amount)
        from truck_booking as tb
        inner join fleet_vehicle_model as fv on fv.id = tb.truck_id
        group by name''')
        booking = self.env.cr.dictfetchall()
    ...
    def get_distance(self):
        """Function to return total distance on the basis of customer
        and truck"""
        self.env.cr.execute('''select pr.name,sum(tb.distance)
        from truck_booking as tb
        inner join res_partner as pr on pr.id = tb.partner_id
        group by name''')
        customer = self.env.cr.dictfetchall()
        ...
        self.env.cr.execute('''select fv.name,sum(tb.distance)
        from truck_booking as tb
        inner join fleet_vehicle_model as fv on fv.id = tb.truck_id
        group by name''')
    ...
    def get_weight(self):
        ...
        self.env.cr.execute('''select pr.name,sum(tb.weight)
        from truck_booking as tb
        inner join res_partner as pr on pr.id = tb.partner_id
        group by name''')
        customer = self.env.cr.dictfetchall()
        self.env.cr.execute('''select fv.name,sum(tb.weight)
        from truck_booking as tb
        inner join fleet_vehicle_model as fv on fv.id = tb.truck_id
        group by name''')
        truck = self.env.cr.dictfetchall()
    ...
    def get_select_filter(self, option):
        ...
        if option == 'year':
            create_date = '''create_date between (now() - interval '1 year')
            and now()'''
        elif option == 'month':
            create_date = '''create_date between (now() - interval '1 months')
            and now()'''
        elif option == 'week':
            create_date = '''create_date between (now() - interval '7 day')
            and now()'''
        elif option == 'day':
            create_date = '''create_date between (now() - interval '1 day')
            and now()'''
       ...
       order by count desc limit 10''' % create_date)
        customer = self.env.cr.dictfetchall()
        self.env.cr.execute('''select pr.name,sum(tb.amount) from
        truck_booking as tb
        inner join res_partner as pr on pr.id = tb.partner_id
