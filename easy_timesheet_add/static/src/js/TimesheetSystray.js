...
export class TimesheetSystray extends Component {
	...
	async setup() {
	    ...
	    this.state.Access = await user.hasGroup('hr_timesheet.group_hr_timesheet_user') ||
        ...
        if (this.state.Access) {
            ...
            this.state.project = await this.orm.searchRead('project.project', [["is_template", "=", false]], ['id', 'name'])
            this.state.Task = await this.orm.searchRead('project.task', [], ['id', 'name'])
	    }
	}
	currentDT(){
	...
	async SearchReadTimeSheet(){
        this.user = await this.orm.searchRead('res.users',
		        [["id", "=", user.userId]], ['employee_id'])
	    if (!this.user.length || !this.user[0].employee_id) {
	        return
	    }
