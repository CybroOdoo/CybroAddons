            ...
            if move.product_id.is_bom:
                products = self.env['pos.product.bom'].search(
                    [('product_id.id', '=',
                     ...
                     ('state', '=', 'confirm')])
                for product_bom in products:
                    for rec in product_bom.bom_line_ids:
                        pos_bom_move = self.env['stock.move'].create(
                            {
                                ...
                                'product_uom_qty': abs(sum(order_lines.mapped(
                                    'qty'))) * (rec.quantity / product_bom.quantity),
                        ...
                        bom_moves |= pos_bom_move
        if bom_moves:
            bom_confirmed = bom_moves._action_confirm()
            for bm in bom_confirmed:
                bm.quantity = bm.product_uom_qty
            bom_confirmed.picked = True
