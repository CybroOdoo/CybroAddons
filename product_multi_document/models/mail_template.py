...
class MailTemplate(models.Model):
    ...
    def _generate_template_attachments(self, res_ids, render_fields,
        ...
        The parent may not populate 'attachments' when the template has no
        attachment records, so we use setdefault to guarantee the key exists
        ...
        if self.render_model == 'purchase.order':
            ...
            if documents:
                res[res_ids[0]].setdefault('attachments', [])
