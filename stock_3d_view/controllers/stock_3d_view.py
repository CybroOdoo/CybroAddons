    ...
    def get_stock_count_data(self, loc_code):
        ...
        if loc_code:
            loc = request.env['stock.location'].sudo().search(
                [('unique_code', '=', loc_code)], limit=1)
            quantity = request.env['stock.quant'].sudo().search(
                [('location_id', 'child_of', loc.id)]).mapped('quantity')
        ...
        if loc_code:
            ...
            if products:
                for rec in products:
                    ...
                    if rec.lot_id:
                        name = f"{name} ({rec.lot_id.name})"
                    product_data[name] = product_data.get(name, 0.0) + rec.quantity

            product_list = [(name, qty) for name, qty in product_data.items()]
