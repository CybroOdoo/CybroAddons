...
class ResUsers(models.Model):
    ...
    def action_import(self):
        ...
        return {
            ...
            'params': {
                'title': _('Success'),
                'message': _('Importing successfully done.'),
                'type': 'success',
                'sticky': False,
            }
        }
    ...
    def action_export(self):
        ...
        self.get_member_id(header, self.user_name)
        for project in self.env['project.project'].search([]):
            ...
            # Ensure all stages exist on Trello
            for stage in project.type_ids.sorted(key=lambda s: s.sequence,
                                                 reverse=True):
                if stage.name not in [rec['name'] for rec in lists_on_board]:
                    list_ref = self.create_list(header, query,
                                                project.trello_reference,
                                                stage.name)['id']
                    # We need to refresh lists_on_board or just remember this new list
                    lists_on_board.append({'name': stage.name, 'id': list_ref})

            # Sync tasks
            tasks = self.env['project.task'].search([('project_id', '=', project.id)])
            for task in tasks:
                old_stage_ref = task.stage_reference
                # Find the matching Trello list for the current task stage
                trello_list_id = next((rec['id'] for rec in lists_on_board if rec['name'] == task.stage_id.name), None)

                if trello_list_id:
                    if task.stage_reference != trello_list_id:
                        task.write({'stage_reference': trello_list_id})

                if task.trello_reference:
                    # Move card if stage changed
                    if old_stage_ref and task.stage_reference != old_stage_ref:
                        url = f"https://api.trello.com/1/cards/{task.trello_reference}"
                        params = {
                            'key': self.api_key,
                            'token': self.token,
                            'idList': task.stage_reference,
                        }
                        requests.request("PUT", url, headers=header, params=params, timeout=10)
                else:
                    # Create new card if it doesn't exist on Trello
                    if task.stage_reference:
        ...
        return {
            ...
            'params': {
                'title': _('Success'),
                'message': _('Exporting successfully done.'),
                'type': 'success',
                'sticky': False,
            }
        }
