...
class TableReservation(models.Model):
            ...
            for record in self:
                ...
                if record.starting_at:
                    ...
                    for rec in reserved:
                        ...
                        end_time_new = datetime.strptime(self.ending_at, "%H:%M").time() if self.ending_at else start_time_new
    ...
    def create(self, vals_list):
        """Assign a sequence to each reservation in batch-safe create."""
    ...
    def _onchange_date(self):
        """Valid year validation for dates"""
        for rec in self:
            if rec.date:
                if rec.date < fields.Date.today() and rec.state == 'draft':
                    raise ValidationError(_("You can't select a past date."))
                if rec.date.year > 9999:
                    raise ValidationError(_("Invalid Year"))

    @api.constrains('lead_time')
    def _check_lead_time(self):
        for rec in self:
            if not (0.0 <= rec.lead_time <= 24.0):
                raise ValidationError(_("Lead time must be between 00:00 and 24:00."))
    ...
    def edit_reservations(self, booking_id, date, customer, start_time,
        ...
        if is_lead_time:
            if lead:
                if isinstance(lead, str):
                    ...
                    time_float = float(lead)
            else:
                time_float = float(default_lead_time or 0.0)
        ...
        if date and int(date.split('-')[0]) > 9999:
            raise ValidationError(_("Invalid Year"))
        reservation.update({
    ...
    def get_table_details(self, floor_id, date, start_time, end_time,
        ...
        is_lead_time = self.env['ir.config_parameter'].sudo().get_param(
            "table_reservation_on_website.is_lead_time")
        is_lead = str(is_lead_time).lower() in ('true', '1') if is_lead_time else False
        lead_time_param = self.env['ir.config_parameter'].sudo().get_param(
            "table_reservation_on_website.reservation_lead_time")
        lead_hours = float(lead_time_param or 0.0)
        if is_lead and lead_hours > 0 and date and start_time:
            try:
        ...
        if reservations:
            for rec in reservations:
                reservation_start = datetime.strptime(rec.starting_at, "%H:%M").time()
                reservation_end = datetime.strptime(rec.ending_at,
                                                    "%H:%M").time()
                if reservation_start < end_time_val and start_time_val < reservation_end:
                    for table in rec.booked_tables_ids:
                        table_inbetween.append(table.id)
        data_tables = []
        for rec in tables:
        ...
        if booked_table_id:
            for id in booked_table_id:
                if self.env['restaurant.table'].browse(id).floor_id.id == int(
                    ...
                    data_tables.append({
                        'id': id,
                        'name': self.env['restaurant.table'].browse(id).table_number
                    })
        return data_tables
