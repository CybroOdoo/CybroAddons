...
class PosConfig(models.Model):
    ...
    reservation_lead_time = fields.Float(
        string='Lead Time Value',
        compute="_compute_reservation_lead_time",
        help="The lead time value set in configuration")
    ...
    def _compute_reservation_lead_time(self):
        """ To get the lead time value from settings """
        lead_time = self.env['ir.config_parameter'].sudo().get_param(
            'table_reservation_on_website.reservation_lead_time', default=0.0)
        for record in self:
            record.reservation_lead_time = float(lead_time)
