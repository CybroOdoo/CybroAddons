...
class CustomerPortal(portal.CustomerPortal):
    """ Class to count total number of services created """
    def _prepare_home_portal_values(self, counters):
        """ Function to add total service created by the current user """
        values = super()._prepare_home_portal_values(counters)
        if "contact_count" in counters:
            values["contact_count"] = (request.env["service.booking"].sudo().search_count([
    ...
    def car_service_portal(self):
        """ Function to create the list view car service """
        service_booking = (
    ...
    @http.route(['/service_portal/<model("service.booking"):contract>'],
