    ...
    def _get_report_values(self, data):
        ...
        if isinstance(report_res, list):
            for row in report_res:
                if 'note' in row:
                    row['note'] = self.strip_html_tags(row['note'])
        elif isinstance(report_res, dict):
            if 'note' in report_res:
                report_res['note'] = self.strip_html_tags(report_res['note'])
        return {
            'doc_ids': self.ids,
            'docs': docs,
            'PURCHASE': report_res,
    ...
    def _get_report_sub_lines(self, data):
        ...
        if data.get('report_type') == 'report_by_order':
            ...
            if data.get('date_to'):
                query += """ and l.date_order <= '%s' """ % data.get('date_to')
            query += """group by l.user_id,res_users.partner_id,res_partner.name,
            l.partner_id,l.date_order,l.name,l.amount_total,l.note,
            ...
            where 1=1 """
            if data.get('date_from'):
                query += """and purchase_order.date_order >= '%s' """ % data.get(
                    'date_from')
            if data.get('date_to'):
                query += """ and purchase_order.date_order <= '%s' """ % data.get(
                    'date_to')
            ...
            self.env.cr.execute(query)
            report_by_purchase_representative = self.env.cr.dictfetchall()
            ...
            self.env.cr.execute(query)
            report_by_state = self.env.cr.dictfetchall()
