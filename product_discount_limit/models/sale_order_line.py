...
class SalesOrderLine(models.Model):
    ...
    def _onchange_discount(self):
        ...
        if not self.env.user.allow_discount:
            desc_limit = self.product_id.product_tmpl_id.desc_limit or self.product_id.categ_id.desc_limit
            if desc_limit and self.discount > desc_limit:
                raise exceptions.UserError(
                    _('You are not allowed to apply discount more than %s percentage '
                      'on this %s. Please contact your administrator',
                      desc_limit,
                      'product' if self.product_id.product_tmpl_id else 'category'))
