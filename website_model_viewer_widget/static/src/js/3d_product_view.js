        ...
        if (this.$(ev.target).data('type') == "3d") {
            ...
            rpc('/product/3d', {
                ...
                try {
                    ...
                    // Adding event listener to handle load errors
                    self.view3D.on("loadError", (e) => {
                        console.error("View3D load error:", e.message);
                        // Fallback to default if custom model fails to load
                        if (val !== `/model_viewer_widget/static/src/assets/3d.glb`) {
                            self.view3D.load(`/model_viewer_widget/static/src/assets/3d.glb`);
                        }
                    });
                } catch (err) {
                    console.error("Failed to initialize View3D:", err);
                }
            });
        } else {
