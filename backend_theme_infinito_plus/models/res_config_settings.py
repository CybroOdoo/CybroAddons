...
class ResConfigSettings(models.TransientModel):
    """Model for adding new fields in configuration settings.

    This model extends the configuration settings to include new fields.
    ...
    is_refresh = fields.Boolean(string='Refresh', help='Tree,Kanban Refresh '
                                                       'mode Enabled',
                                default=False)
    chatterbox_position = fields.Char(string="ChatBox position",
    ...
    def get_values(self):
        ...
        This method retrieves the current values for the configuration settings.

        Returns:
            dict: A dictionary containing the current values for the settings."""
        res = super(ResConfigSettings, self).get_values()
        get_param = self.env['ir.config_parameter'].sudo().get_param
        res['is_refresh'] = get_param('backend_theme_infinito_plus.is_refresh',
                                      default=False)
        res['chatterbox_position'] = get_param('backend_theme_infinito_plus'
                                            '.chatterbox_position')
    ...
    def set_values(self):
        ...
        This method updates the values of the configuration settings.
