...
class SignupConfiguration(models.Model):
    ...
    signup_enabled=fields.Boolean(string='Is Signup Enabled',
                                  compute='_compute_signup_permission')

    @api.depends('name')
    def _compute_signup_permission(self):
        """Compute method for Customer Account Selection"""
        for record in self:
            if self.env['ir.config_parameter'].sudo().get_param('auth_signup.invitation_scope') == 'b2c':
                record.signup_enabled = True
            else:
                record.signup_enabled = False
