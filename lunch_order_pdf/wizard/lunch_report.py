...
class LunchReport(models.TransientModel):
    ...
    product_ids = fields.Many2many(
        'lunch.product', string='Products', default=False,
        domain='[("active", "=", True)]', help='Lunch Products.')
    category_ids = fields.Many2many(
        'lunch.product.category', string='Categories', default=False,
        help='Lunch Product Categories.')
    company_ids = fields.Many2many('res.company', string='Companies',
                                   default=False, help='Companies')
    ...
    def action_print_report(self):
        ...
        return self.env.ref(
            'lunch_order_pdf.action_report_lunch_report').report_action(
            self, data=data)
