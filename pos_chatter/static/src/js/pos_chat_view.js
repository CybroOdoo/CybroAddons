...
export class ChatMsgView extends owl.Component {
    ...
    render_messages() {
        var self = this
        jsonrpc('/pos_systray/chat_message', {
    ...
    _onClickMessageSend() {
        ...
        if (input_value.value) {
            ...
            jsonrpc('/pos_chatter/send_message', {
                'data': data
            }).then(function() {
                input_value.value = ""
...
ChatMsgView.template = xml`
    ...
    <div style="width=100%;background:#875A7B;height:36px;border-top-left-radius: 10px;border-top-right-radius: 10px;">
        ...
        <div class="row">
        <div class="col-9">
            <input id="message_to_send" type="text" style="border:none;width: 265px;margin-left: 17px;outline:none;height: 25px;" placeholder="Message" t-on-keypress="keyEnter" /></div>
            <div class="col-3"><i style="margin-left:7px;cursor: pointer;" id="button_send" class="fa fa-paper-plane" t-att-value="state.id" t-on-click="_onClickMessageSend" /></div>
        </div>
    </div>
</div>`
