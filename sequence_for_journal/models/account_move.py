...
class AccountMove(models.Model):
    ...
    def _get_starting_sequence(self):
        """Overriding the method, this method get the initial sequence of a
        journal"""
        self.ensure_one()
        interpolated_prefix = self.journal_id.code
        interpolated_suffix = self.journal_id.sequence_id.suffix
        if self.journal_id.sequence_id:
            interpolated_prefix, interpolated_suffix = self.journal_id.\
                sequence_id._get_prefix_suffix(date=fields.Date.to_string(
                    self.date))
        if self.journal_id.type in ['sale', 'bank', 'cash'] and \
            ...
            starting_sequence = "%s/%s/000%d" % (
                ...
                self.journal_id.sequence_id.suffix:
            starting_sequence = "%s/%s/%s%s" % (
                interpolated_prefix, self.date.year,
                self.journal_id.step_size,
                interpolated_suffix)
        else:
        ...
        if (self.journal_id.payment_sequence and self.origin_payment_id) or \
