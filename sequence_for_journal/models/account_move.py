...
class AccountMove(models.Model):
    ...
    def _get_starting_sequence(self):
        """Overriding the methode, this method gets the initial sequence of a
                journal"""
        self.ensure_one()
        if self.journal_id.type in ['sale', 'bank', 'cash'] and \
            ...
            starting_sequence = "%s/%s/000%d" % (
                self.journal_id.code,
                self.date.year,
                self.journal_id.step_size_default)
    ...
    def _set_next_sequence(self):
        """Overriding, to get the next sequence number, can change the prefix
        and suffix of sequence number"""
        self.ensure_one()
        last_sequence = self._get_last_sequence()
        new = not last_sequence
        if new:
            last_sequence = self._get_last_sequence(
                relaxed=True) or self._get_starting_sequence()
