...
export class WarehouseDesigner extends Component {
    ...
    async _checkPermissions() {
        this.state.isAdmin = await user.hasGroup("stock.group_stock_manager");
    }
    ...
    async selectLayout(layoutId) {
        ...
        try {
            ...
            this.notification.add(_t("Failed to load layout data"), { type: "danger" });
        }
    }
    ...
    async saveLayout() {
        ...
        try {
            const result = await rpc("/warehouse_3d/save_positions", {
                layout_id: this.state.selectedLayoutId,
                positions,
                removed_ids: this.state.removedLocationIds,
            });

            await rpc("/warehouse_3d/save_map_objects", {
                layout_id: this.state.selectedLayoutId,
