...
class StockLocation(models.Model):
    ...
    is_empty = fields.Boolean(
        ...
        help='Whether this location currently holds no stock (no quants '
             'with a positive quantity). Added in this module for '
             'Odoo 17 compatibility, since the field is not present in '
             'core stock.location on this version.',
    )
    net_weight = fields.Float(
        string='Net Weight',
        compute='_compute_net_weight',
        help='Total weight of the stock currently stored in this location '
             '(sum of quant quantity x product weight). Added in this '
             'module for Odoo 17 compatibility, since the field is not '
             'present in core stock.location on this version.',
    )
    ...
    def _compute_is_empty(self):
        """A location is empty when it has no quants with qty > 0."""
        quants = self.env['stock.quant'].read_group(
            domain=[('location_id', 'in', self.ids), ('quantity', '>', 0)],
            fields=['location_id'],
            groupby=['location_id'],
        )
        non_empty_ids = {g['location_id'][0] for g in quants}
        for loc in self:
            loc.is_empty = loc.id not in non_empty_ids

    def _compute_net_weight(self):
        """Sum of quantity x product weight for all quants in the
        location."""
        ...
        weight_map = {}
        for quant in quants:
            weight_map[quant.location_id.id] = weight_map.get(
                quant.location_id.id, 0.0
            ) + (quant.quantity * quant.product_id.weight)
        for loc in self:
            loc.net_weight = weight_map.get(loc.id, 0.0)
