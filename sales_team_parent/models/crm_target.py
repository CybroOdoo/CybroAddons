...
class CrmTeam(models.Model):
    ...
    def action_your_target(self, cr, uid, context=None):
        tree_res = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'sales_team_parent', 'sales_target_tree_view')
        x = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'sales_team_parent', 'action_crm_sales_target_act')

        tree_id = tree_res and tree_res[1] or False
        form_res = self.pool.get('ir.model.data').get_object_reference(cr, uid, 'sales_team_parent', 'sales_target_form_view')
        form_id = form_res and form_res[1] or False

        user_obj = self.pool.get('res.users')

        u_id = user_obj.browse(cr, uid, uid, context=context)
        obj_target = self.pool.get('crm.target').search(cr, uid, [], context=context)
        ...
        if u_id.has_group('base.group_sale_salesman_all_leads') and u_id.has_group(
            ...
            obj_target_list = obj_target

        else:
            obj_target_list = []
            if obj_target:
                for obj in obj_target:
                    if self.pool.get('crm.target').browse(cr, uid, [obj]).user_id.id == uid:
