...
class ProjectCredential(models.Model):
    ...
    def _get_access_token(self):
        ...
        self.ensure_one()
        if not self.hangout_company_access_token or \
           not self.hangout_company_access_token_expiry or \
           self.hangout_company_access_token_expiry < fields.Datetime.now():
            self.action_google_task_company_refresh_token()
        return self.hangout_company_access_token
        ...
        if response.json() and response.json().get('access_token'):
            self.write({
                'hangout_company_access_token': response.json().get(
                    'access_token'),
                'hangout_company_access_token_expiry': fields.Datetime.now() + \
                    datetime.timedelta(seconds=response.json().get('expires_in', 3600))
            })
            return {
