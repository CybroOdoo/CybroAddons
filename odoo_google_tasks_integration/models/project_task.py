...
class ProjectTask(models.Model):
    ...
    def write(self, vals):
        ...
        if not self.env.context.get('skip_gtask_sync'):
            for task in self:
                if vals.get('is_add_in_gtask'):
                    task.with_context(skip_gtask_sync=True).action_sync_task_to_google()
        return res
    ...
    def action_sync_task_to_google(self):
        ...
        datetime_obj = datetime.combine(
            self.date_deadline or fields.Date.today(),
            time.min
        )
        ...
        if response.status_code == 200:
            ...
            message_data = _("Connection Not Successful! Error: %s") % response.text
