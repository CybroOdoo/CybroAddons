...
class ProjectGoogleTaskImportWizard(models.Model):
    ...
    task_ids = fields.Many2many('project.task',
                                string='Select Tasks to Export'
                                , help='Tasks to be exported to Google Task')

    def action_import_tasks(self):
        """Import tasks from Google Task.This method fetches tasks from the
         authenticated user's Google Tasks account and creates corresponding
         tasks in the local database. It then provides feedback on the import
         status."""
    ...
    def action_export_task(self):
        """Export tasks to Google Task. This method triggers the
        synchronization of tasks with Google Tasks for each task in the
         current record's task_ids.It displays a notification indicating the
