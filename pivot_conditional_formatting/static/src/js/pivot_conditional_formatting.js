...
patch(PivotRenderer.prototype, {
    ...
    async set_default_rules() {
        ...
        var model = this.model.metaData.resModel
        await this.orm.call("conditional.rules", "search_read", [], {
            domain: [['model_id', '=', model], ['view_id', '=', viewId]],
        }).then(function (res) {
            self.conditional_rules = res
        })
        var cells = this.tableRef.el.parentElement.querySelectorAll('td')
        cells.forEach(function (data) {
            data.style.backgroundColor = "#f8f9fa"
            data.style.color = "black"
        })
        this.tableRef.el.parentElement.querySelectorAll('.o_pivot.table-responsive table .selected_cell').forEach(cell => {
            cell.classList.remove('selected_cell')
        });
        this.tableRef.el.parentElement.parentElement.querySelectorAll('.conditional_button').forEach(el => {
            el.style.display = "none"
        });
        this.tableRef.el.parentElement.parentElement.querySelectorAll('.conditional_container').forEach(el => {
            el.style.display = "none"
        });
    ...
    async conditional_format_tab() {
        ...
        this.tableRef.el.parentElement.querySelectorAll("#condition_val")[0].value = ''
        this.tableRef.el.parentElement.querySelectorAll("#secondcondition_val")[0].value = ''
        if (this.tableRef.el.parentElement.querySelectorAll(
            '.validation-error')[0].style.display == "inline") {
            this.tableRef.el.parentElement.querySelectorAll(
    ...
    display_field() {
        ...
        var condition = this.tableRef.el.parentElement.querySelectorAll(
        ...
        if (condition == 'in between') {
            this.tableRef.el.parentElement.parentElement.querySelectorAll('#secondcondition_val').forEach(el => {
                el.style.display = "block"
            });
            this.tableRef.el.parentElement.parentElement.querySelectorAll('#value_label').forEach(el => {
                el.style.display = "block"
            });
            this.tableRef.el.parentElement.parentElement.querySelectorAll('#sub_input_container2').forEach(el => {
        ...
        if (condition === 'null') {
            this.tableRef.el.parentElement.parentElement.querySelectorAll('#condition_val').forEach(el => {
                el.style.display = "none"
            });
            this.tableRef.el.parentElement.parentElement.querySelectorAll('#value_label1').forEach(el => {
                el.style.display = "none"
            });
            this.tableRef.el.parentElement.parentElement.querySelectorAll('#sub_input_container1').forEach(el => {
                el.style.display = "none"
            });
        } else {
            this.tableRef.el.parentElement.parentElement.querySelectorAll('#condition_val').forEach(el => {
    ...
    set_rule() {
        ...
        var condition = this.tableRef.el.parentElement.querySelectorAll(
            '.condition_select')[0].value
        var color_val = this.tableRef.el.parentElement.querySelectorAll(
            '.colorpicker')[0].value
        var text_color_val = this.tableRef.el.parentElement.querySelectorAll(
            '.text_color')[0].value
        var cells = this.tableRef.el.parentElement.querySelectorAll(
            '.selected_cell')
        var condition_val = this.tableRef.el.parentElement.querySelectorAll(
            '#condition_val')[0].value
        var second_condition_val = this.tableRef.el.parentElement.querySelectorAll(
            '#secondcondition_val')[0].value
        this.tableRef.el.parentElement.querySelectorAll(
            '#condition_val')[0].value = ''
        this.tableRef.el.parentElement.querySelectorAll(
        ...
        if (condition == 'in between') {
            if (condition_val > second_condition_val) {
                this.tableRef.el.parentElement.parentElement.querySelectorAll('.validation-error').forEach(el => {
