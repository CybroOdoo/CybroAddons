...
class WebsiteForm(WebsiteForm):
    ...
    def _handle_website_form(self, model_name, **kwargs):
        """Handle the submission of a website form. This function processes
         the submitted form data based on the provided model name.Which will
         returns encoded response containing the result of the form
         submission"""
        if model_name == 'help.ticket':
            ...
            partner = request.env['res.partner'].sudo().search(
                [('name', '=', kwargs.get('customer_name')),
                 ('email', '=', kwargs.get('email_from'))], order='id asc',
                limit=1)
            rec_val['customer_id'] = partner.id if partner else None
            rec_val['public_ticket'] = not partner
            ...
            # generically Ex:crm.lead.probability which is a float between 0 and
            # 1 TODO: How to get the name of the erroneous field ?
