...
class ThemeMedicare(http.Controller):
    ...
    def about_page(self, **kw):
        return request.render('theme_medicare.about_page')
    ...
    def doctors_page(self, page=1, **kw):
        ...
        PPG = 8  # Doctors per page

        url = '/doctors'
        url_args = {}
        if department_id:
            url_args['department_id'] = department_id
        if search:
        ...
        doctors = request.env['medicare.doctor'].sudo().search(domain, limit=PPG, offset=pager['offset'])
    ...
    def appointment_page(self, **kw):
        ...
        selected_doctor_id = kw.get('doctor_id')
        selected_department_id = kw.get('department_id')

        if selected_doctor_id:
            try:
                selected_doctor_id = int(selected_doctor_id)
                doctor_record = request.env['medicare.doctor'].sudo().browse(selected_doctor_id)
                if doctor_record.exists() and doctor_record.department_id:
                    selected_department_id = doctor_record.department_id.id
            except ValueError:
                pass
