...
class SessionWebsite(Session):
    """Extended session controller for website-related operations"""

    @http.route('/web/session/logout_popup', type='http', auth="public",
                website=True)
    def logout_popup(self):
        """Render the logout popup template.
        Sets X-Frame-Options and CSP headers to allow embedding in the
        Website Builder iframe (Odoo's middleware sets DENY by default).
        """
        uid = request.env.uid
        ...
        response = request.render(
            'login_user_details_save.logout_popup_template', values)
...
class WebHome(Home):
    ...
    def web_login(self, redirect=None, **kw):
        """Extended web login controller for login-related operations."""
        ensure_db()
