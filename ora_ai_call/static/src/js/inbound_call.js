...
class VapiCalling extends Component {
    ...
    onMessage({detail: notifications}) {
      var channel_vise_notifications = notifications.filter(item => item.payload.channel === this.channel)
      if(channel_vise_notifications[0].payload){
        if (channel_vise_notifications[0].payload.value.message.status === "in-progress"){
            timerInterval = setInterval(this.updateTimer.bind(this), 1000);
        }
        if (channel_vise_notifications[0].payload.value.message.status === "ended"){
