...
class SubcontractInstallationTeamTask(models.Model):
    """Class for subcontract installation team task"""
    _name = "subcontract.installation.team.task"
    _description = "Subcontract Installation Team Task"

    ren_order_id = fields.Many2one('ren.order', help="REN order related to this installation task.")
    subcontractor_id = fields.Many2one('res.partner', related="ren_order_id.subcontractor_id")
    installation_stage = fields.Selection([
        ...
        track_visibility='always', help='installation task stage')
    ...
    priority = fields.Selection([('0', 'Low'), ('1', 'Medium'), ('2', 'High'), ('3', 'Very High')], string='Priority',
                                related="ren_order_id.priority", help='installation task priority')
    ...
    def _group_expand_installation_states(self, states, domain, order):
        """Method ensures all possible selection values for 'installation_stage'
