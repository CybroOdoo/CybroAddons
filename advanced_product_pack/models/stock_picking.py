...
class StockPicking(models.Model):
    ...
    def _create_move_from_pos_order_lines(self, lines):
        ...
        for line in lines:
            if line.product_id.is_bundle and line.product_id.bundle_line_ids:
                ...
                for pack_line in line.product_id.bundle_line_ids:
                    ...
                    move_vals.append({
                        'name': pack_line.product_id.display_name or line.name,
        ...
        # Standard Odoo logic for regular (non-bundle) lines
        def get_grouping_key(line):
            """Return grouping key for regular pos order lines."""
        ...
        # Create all moves (exploded components + regular products)
        moves = self.env['stock.move'].create(move_vals)
        confirmed_moves = moves._action_confirm()
        # Link to order lines where possible
        # Components won't link directly to the bundle line in Odoo's default 'lines_data'
        # but they will be processed as 'moves_to_assign' in _add_mls_related_to_order
        confirmed_moves._add_mls_related_to_order(lines, are_qties_done=True)
        # moves created from PoS are immediately 'picked'
