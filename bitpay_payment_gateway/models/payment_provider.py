...
class PaymentProvider(models.Model):
    ...
    def _setup_provider(self, provider_code):
        """Perform module-specific setup steps for the BitPay provider."""
        super()._setup_provider(provider_code)
        if provider_code != 'bitpay':
            return
        provider = self.search([('code', '=', 'bitpay')], limit=1)
        if provider and not provider.image_128:
            import base64
            from odoo.tools.misc import file_open
            for img_path in (
                'bitpay_payment_gateway/static/description/icon.png',
                'bitpay_payment_gateway/static/description/assets/images/bitpay.png',
            ):
                try:
                    with file_open(img_path, 'rb') as f:
                        provider.image_128 = base64.b64encode(f.read())
                    break
                except Exception:
                    continue
