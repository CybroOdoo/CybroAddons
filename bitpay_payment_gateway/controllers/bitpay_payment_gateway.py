...
class BitPayController(http.Controller):
    ...
    def bitpay_return(self, **data):
        ...
        try:
            tx_sudo = request.env['payment.transaction'].sudo()._get_tx_from_notification_data('bitpay', data)
    ...
    def bitpay_notification(self, **data):
        ...
        try:
            request.env['payment.transaction'].sudo()._handle_notification_data('bitpay', notification_data)
        except ValidationError:
...
class BitPayPaymentPostProcessing(PaymentPostProcessing):
    ...
    def _get_monitored_transaction(self):
        """Retrieve the currently monitored transaction record for the current session."""
        monitored_tx_id = self.get_monitored_transaction_id()
        if monitored_tx_id:
            return request.env['payment.transaction'].sudo().browse(monitored_tx_id).exists()
