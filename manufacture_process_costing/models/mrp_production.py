...
class MrpProduction(models.Model):
    ...
    def button_mark_done(self):
        """
        Override button_mark_done to update actual costs
        """
        res = super(MrpProduction, self).button_mark_done()
        for production in self:
            for material in production.material_cost_ids:
                if material.actual_quantity == 0:
                    material.write({'actual_quantity': material.planned_qty})

            for labour in production.labour_cost_ids:
                if labour.actual_minute == 0:
                    workorder = production.workorder_ids.filtered(
                        lambda w: w.name == labour.operation)
                    if workorder:
                        labour.write({
                            'actual_minute': sum(
                                workorder.mapped('duration'))})
                    else:
                        labour.write({'actual_minute': labour.planned_minute})

            for overhead in production.overhead_cost_ids:
                if overhead.actual_minute == 0:
                    workorder = production.workorder_ids.filtered(
                        lambda w: w.name == overhead.operation)
                    if workorder:
                        overhead.write({
                            'actual_minute': sum(
                                workorder.mapped('duration'))})
                        ...
                        overhead.write(
                            {'actual_minute': overhead.planned_minute})
        return res
        ...
        self.write({
            'labour_cost_ids': [
                (0, 0, {
                    ...
                    'labour_cost_id': rec.labour_cost_id.id,
        ...
        self.write({
            'overhead_cost_ids': [
                (0, 0, {
                    ...
                    'overhead_cost_id': rec.overhead_cost_id.id,
                    'operation': rec.operation,
                    'work_center_id': rec.work_center_id.id,
                    'planned_minute': rec.planned_minute,
                    'cost_minute': rec.cost_minute,
