    ...
    def _program_check_compute_points(self, programs):
        ...
        for program in programs:
            ...
            if not program.is_nominative:
                if not code_matched:
                    program_result['error'] = ("This program requires a "
                                               "code to be applied.")
                elif not minimum_amount_matched:
                    program_result[
                        'error'] = 'A minimum of %(amount)s %(currency)s should be purchased to get the reward', min(
                        program.rule_ids.mapped(
                            'minimum_amount')), program.currency_id.name
                elif not product_qty_matched:
                    program_result['error'] = ("You don't have the required "
                                               "product quantities on your "
                                               "sales order.")
            elif not self._allow_nominative_programs():
                program_result['error'] = ("This program is not available "
                                           "for public users.")
