...
class LanteroTrackOrder(http.Controller):
    ...
    def track_order(self, order_number=None, email=None, **kw):
        """
        Handles the order tracking functionality for the Lantero theme.
        Allows users to search for their order status using an order number and email.
        Calculates progress and status steps based on the sale order's state and delivery pickings.
        :param order_number: The unique reference of the sale order (e.g., S0001).
        :param email: The email address associated with the order.
        :param kw: Additional keyword arguments.
        :return: Rendered track-order page with order status details or error messages.
        """
        values = {
            'searched': False,
            'order': None,
