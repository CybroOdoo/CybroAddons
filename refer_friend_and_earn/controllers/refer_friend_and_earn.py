...
class WebsiteLogin(AuthSignupHome):
    ...
    def do_signup(self, qcontext, **post):
        ...
        referral code and thus want to find partner with that referral code,
         to add points"""
        res = super(WebsiteLogin, self).do_signup(qcontext, **post)
...
class ReferAndEarn(http.Controller):
    ...
    def refer_and_earn_popup(self, **post):
        ...
        mail_template.send_mail(request.env.user.partner_id.id,
...
class WebsiteSale(WebsiteSale):
    ...
    def refer_earn(self, **kw):
        ...
        if kw.get('points'):
            ...
            if points_rec:
                ...
                total_price = sum(
                    sale_order.order_line.mapped('price_subtotal'))
    ...
    def payment_confirmation(self, **post):
        ...
        points = request.env['sale.order'].browse(
            request.session.get('sale_last_order_id')).points_applied
