...
class ResPartner(models.Model):
    ...
    def _compute_email(self):
        ...
        for rec in self:
            send_count = rec.env['mail.mail'].search(
                [('email_from', 'ilike', self.email)])
            rec.send_email_count = len(send_count)
            received_count = rec.env['mail.mail'].search(
                ['|', ('email_to', '=', self.email),
                 ('recipient_ids', '=', self.email)])
            rec.received_email_count = len(received_count)
    ...
    def received_email_history(self):
        ...
        action = self.env.ref('mail.action_view_mail_mail')
        result = action.read()[0]
        result['domain'] = ['|', ('email_to', '=', self.email),
                            ('recipient_ids', '=', self.email)]
        return result
