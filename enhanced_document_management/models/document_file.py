...
class DocumentFile(models.Model):
    ...
    workspace_id = fields.Many2one('document.workspace',
                                   ...
                                   default=lambda self: self.env.ref(
                                       'enhanced_document_management.document_workspace_default',
    ...
    days = fields.Integer(string='Days', help="auto delete in days", default=1)
    trash = fields.Boolean(string='Trash', help="To specify deleted items")
    delete_date = fields.Date(string='Date Delete', compute="_compute_delete_date", readonly=True, store=True)
    ...
    @api.depends('days')
    def _compute_delete_date(self):
        """ Compute delete_date as today + days """
        for rec in self:
            if rec.days and rec.days > 0:
                rec.delete_date = date.today() + timedelta(days=rec.days)
            else:
                rec.delete_date = False
    ...
    def click_document_lock(self):
        ...
        return {
            ...
            'view_mode': 'form',
            'view_id': self.env.ref(
                'enhanced_document_management.document_lock_view_form').id,
            'target': 'new',
            'context': {
                'default_document_file_id': document_id,
            }
