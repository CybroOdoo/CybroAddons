...
class ProjectReportButton(models.TransientModel):
    ...
    def get_xlsx_report(self, data, response):
        ...
        sheet.merge_range('E13:H13', "Assigned", format2)
        sheet.merge_range('I13:K13', "Stage", format2)
        row_number = 13
        column_number = 0
        for val in vals:
            sheet.merge_range(row_number, column_number, row_number, column_number + 3, val['name'], format3)
            sheet.merge_range(row_number, column_number + 4, row_number, column_number + 7, val['user_id'], format3)
            sheet.merge_range(row_number, column_number + 8, row_number, column_number + 9, val['stage_id'], format3)
            row_number += 1
