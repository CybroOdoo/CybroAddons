...
class GenerateToken(CustomerPortal):
    ...
    @http.route('/generate/token', type='http', auth="public", website=True)
    def generate_token(self):
        ...
        action = request.env['ir.actions.actions']._for_xml_id(
            'odoo_queue_manager.queue_counter_action')['id']
        return request.render('odoo_queue_manager.generate_token', {
    ...
    def generate_token(self, **kwargs):
        """Function to show the token generation page with department selection."""
        department_rec = request.env['department'].sudo().search([])
        action = request.env['ir.actions.actions']._for_xml_id(
    ...
    def create_token(self, **post):
        """Function to create a new token based on user input and render the token detail page."""
        department_rec = request.env['department'].sudo().search([])
        action = request.env['ir.actions.actions']._for_xml_id(
            'odoo_queue_manager.queue_counter_action'
        )['id']
