...
#    Author: Surya Gayathry (odoo@cybrosys.com)
#
...
class XLSXReportController(http.Controller):
    ...
    @http.route('/get_operation_types', auth='public', type='jsonrpc')
    def get_operation_types(self):
        ...
        for key, value in operation_type_name.items():
            if value not in seen_values:
                unique_dict[
                    key] = value  # Add key-value pair to the new dictionary
                seen_values.add(value)
        filtered_dict = {key: value for key, value in no_transfer.items() if
                         value != 0}
        return filtered_dict, late, waiting, unique_dict, backorder

    @http.route('/get_the_top_products', auth='public', type='jsonrpc')
    ...
    @http.route('/get_stock_moves', auth='public', type='jsonrpc')
    def get_stock_moves(self):
        """rpc method of stock moves graph
            Returns location name and quantity"""
        company_id = request.env.company.id
    ...
    @http.route('/get_product_moves', auth='public', type='jsonrpc')
    def get_product_moves(self):
        """rpc method of product moves graph
            Returns product move product and quantity"""
        company_id = request.env.company.id
        ...
        for record in products_quantity:
            quantity.append(record.get('sum'))
    ...
    @http.route('/get_product_category', auth='public', type='jsonrpc')
    ...
    @http.route('/product_move_by_category', auth='public', type='jsonrpc')
    def product_move_by_category(self, args):
        """rpc method of product moves by category
            Returns category name and quantity"""
    ...
    @http.route('/get_locations', auth='public', type='jsonrpc')
    def get_locations(self):
        ...
        query = """
            ...
            WHERE m.state = 'done'
              AND m.company_id = %s
            GROUP BY src.usage, dest.usage, src.complete_name, dest.complete_name
        """

        request.env.cr.execute(query, (company_id,))
        results = request.env.cr.fetchall()

        value = {}
        for src_usage, dest_usage, src_name, dest_name, qty in results:
            if src_usage == 'supplier':
                value['Vendors'] = value.get('Vendors', 0) + qty
            if dest_usage == 'customer':
                value['Customers'] = value.get('Customers', 0) + qty
            if src_usage == 'inventory' or dest_usage == 'inventory':
                value['Inventory Adjustment'] = value.get('Inventory Adjustment', 0) + qty
            if dest_usage == 'internal':
                key = dest_name.split('/')[-1]
                value[key] = value.get(key, 0) + qty
        return {k: round(v, 2) for k, v in value.items() if v != 0}

    @http.route('/get_out_of_stock', auth='public', type='jsonrpc')
    ...
    @http.route('/get_dead_of_stock', auth='public', type='jsonrpc')
    ...
    @http.route('/top_products_last_ten', auth='public', type='jsonrpc')
    ...
    @http.route('/top_products_last_thirty', auth='public', type='jsonrpc')
    ...
    @http.route('/top_products_last_three_months', auth='public', type='jsonrpc')
    ...
    @http.route('/top_products_last_year', auth='public', type='jsonrpc')
    ...
    @http.route('/stock_move_last_ten_days', auth='public', type='jsonrpc')
    def stock_move_last_ten_days(self):
    ...
    @http.route('/this_month', auth='public', type='jsonrpc')
    def this_month(self):
    ...
    @http.route('/last_three_month', auth='public', type='jsonrpc')
    def last_three_month(self):
    ...
    @http.route('/last_year', auth='public', type='jsonrpc')
    def last_year(self):
    ...
    @http.route('/inventory_report', auth='public', type='jsonrpc')
