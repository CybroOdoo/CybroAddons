...
class StockMove(models.Model):
    ...
    def _onchange_product_id_set_barcode(self):
        """Auto-fill barcode when product is selected or scanned"""
        for rec in self:
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if not vals.get('barcode') and vals.get('product_id'):
                product = self.env['product.product'].browse(vals['product_id'])
                vals['barcode'] = product.barcode or False
        return super().create(vals_list)

    def write(self, vals):
        if 'product_id' in vals and 'barcode' not in vals:
            product = self.env['product.product'].browse(vals['product_id'])
            vals['barcode'] = product.barcode or False
    ...
    def _compute_cw_qty_done(self):
        ...
        for rec in self:
            if rec.product_id.catch_weight_ok:
                rec.cw_qty_done = rec.product_qty * rec.product_id.average_cw_qty
            else:
                rec.cw_qty_done = 0
            ...
            rec.update(
                {
                    ...
                    'cw_done': rec.quantity * rec.product_id.
                    average_cw_qty,
    ...
    def top_products_last_three_months(self):
        ...
        '''
        self.env.cr.execute(query, (company_id, start_three_months_ago, now))
        top_product = self.env.cr.dictfetchall()
        total_quantity = []
        product_name = []
        for record in top_product[:10]:
    ...
    def top_products_last_year(self):
        ...
        ORDER BY SUM(stock_move.product_uom_qty) DESC
        '''
        self.env.cr.execute(query, (company_id, start_last_year, end_last_year))
        top_product = self.env.cr.dictfetchall()
        total_quantity = []
        product_name = []
        for record in top_product[:10]:
    ...
    def stock_move_last_ten_days(self, post):
        ...
        query = ('''select stock_location.name,sum(stock_move_line.quantity) 
                ...
                stock_move_line.state = 'done' and stock_move_line.company_id =
                 ...
                 % company_id)
        self.env.cr.execute(query)
        location_quantity = self.env.cr.dictfetchall()
        quantity = []
        name = []
    ...
    def this_month(self, post):
        ...
        '''
        self.env.cr.execute(query, (company_id, start_this_month, end_this_month))
        location_quantity = self.env.cr.dictfetchall()
        quantity = []
        name = []
        for record in location_quantity:
