        ...
        if data.get('report_type') == 'report_by_transfers':
            ...
            query = '''
                SELECT 
                    ...
                    COALESCE((product_product.standard_price ->> '1')::float, 0.0) AS value_float,
                    product_category.name AS category,
                    COALESCE(SUM(stock_quant.quantity), 0.0) AS quantity
                FROM product_product
                INNER JOIN product_template 
                    ON product_product.product_tmpl_id = product_template.id
                LEFT JOIN stock_quant 
                ...
                WHERE stock_location.usage = 'internal' OR stock_location.id IS NULL
            '''
            filters = []
            params = []
            if data.get('date_from'):
                filters.append("product_product.create_date >= %s")
