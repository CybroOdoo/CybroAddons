...
class StockReport(models.TransientModel):
    """A transient model for generating a report on the current stock
    history."""
    _name = "wizard.stock.history"
    _description = "Current Stock History"

    warehouse_ids = fields.Many2many('stock.warehouse', 'wh_wiz_rel', 'wh',
                                     'wiz', string='Warehouse', required=True,
                                     help="Warehouse for stock")
    category_ids = fields.Many2many('product.category', 'categ_wiz_rel',
                                    'categ', 'wiz', string='Category',
                                    help="Product Category")

    def action_export_xlsx(self):
        """Prepares the data and triggers the generation of the XLSX report."""
        data = {
            'ids': self.ids,
            'model': self._name,
        ...
        return {
            ...
            'data': {'model': 'wizard.stock.history',
                     'options': json.dumps(data, default=date_utils.
                                           json_default),
                     'output_format': 'xlsx',
    ...
    def get_lines(self, data, warehouse_ids):
        """Returns values based on warehouse ids"""
        lines = []
        ...
        sale_query = """
               SELECT sum(s_o_l.product_uom_qty) AS product_uom_qty, 
               s_o_l.product_id FROM sale_order_line AS s_o_l
        ...
        purchase_query = """
               ...
               INNER JOIN stock_picking_type AS s_p_t ON p_o.picking_type_id = 
               s_p_t.id WHERE p_o.state IN ('purchase','done')
               AND s_p_t.warehouse_id = %s AND p_o_l.product_id in %s group by 
               p_o_l.product_id"""
        ...
        for obj in categ_products:
            ...
            virtual_available = (obj.with_context({'warehouse': warehouse_ids}).
                                 virtual_available)
            outgoing_qty = (obj.with_context({'warehouse': warehouse_ids}).
                            outgoing_qty)
            incoming_qty = (obj.with_context({'warehouse': warehouse_ids}).
                            incoming_qty)
            ...
            vals = {
                ...
                'net_on_hand': obj.with_context({'warehouse': warehouse_ids}).
                qty_available,
                'total_value': value,
    ...
    def get_xlsx_report(self, data, response):
        """Write the Excel datas to sheet"""
        output = io.BytesIO()
        ...
        if d1:
            for category in d1:
                c.append(self.env['product.category'].browse(category).name)
            cat = cat.join(c)
        ...
        times = pytz.utc.localize(fields.datetime.now()).astimezone(tz)
        ...
        flag =0
        p_col_no1 = 0
        for item in get_warehouse[0]:
            w_col_no = w_col_no + 11
            sheet.merge_range(8, w_col_no1, 8, w_col_no, item, format11)
            ...
            sheet.write(9, p_col_no1 + 7, 'Available', format21)
            sheet.write(9, p_col_no1 + 8, 'Virtual', format21)
            sheet.write(9, p_col_no1 + 9, 'Incoming', format21)
            sheet.write(9, p_col_no1 + 10, 'Outgoing', format21)
            sheet.merge_range(9, p_col_no1 + 11, 9, p_col_no1 + 12, 'Net On Hand',
                              format21)
            sheet.merge_range(9, p_col_no1 + 13, 9, p_col_no1 + 14, 'Total Sold',
                              format21)
            sheet.merge_range(9, p_col_no1 + 15, 9, p_col_no1 + 16,
                              'Total Purchased', format21)
            sheet.write(9, p_col_no1 + 17, 'Valuation', format21)
            p_col_no1 += 11
            prod_row = 10
            prod_col = 0
            flag =1
        for item in get_warehouse[1]:
            get_line = self.get_lines(d, item)
