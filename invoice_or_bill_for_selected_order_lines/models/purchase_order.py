    ...
    def action_create_invoice(self):
        ...
        for order in self:
            ...
            for line in order.order_line:
                if (line.is_product_select is True) or \
                        True not in order.order_line.mapped(
                    'is_product_select'):
        ...
        for grouping_keys, invoices in groupby(invoice_vals_list, key=lambda
                x: (x.get('company_id'), x.get('partner_id'),
                    x.get('currency_id'))):
            origins = set()
            payment_refs = set()
            refs = set()
