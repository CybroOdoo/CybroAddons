...
class SaleTenderCreateAlternative(models.TransientModel):
    ...
    origin_so_id = fields.Many2one(
        'sale.order', help="The original PO that this alternative"
                           " PO is being created for."
    )
    partner_id = fields.Many2one(
        'res.partner', string='Partner', required=True,
        help="Choose a Partner for alternative SO")

    copy_products = fields.Boolean(
        "Copy Products", default=True,
        help="If this is checked, the product quantities of the original"
             " SO will be copied")
    ...
    def action_create_alternative(self):
        ...
        alt_so = self.env['sale.order'].with_context(
            origin_so_id=self.origin_so_id.id,
            default_tender_id=False
        ).create(vals)
        ...
        return {
            ...
            'res_id': alt_so.id,
            'context': {'active_id': alt_so.id},
        }

    def _get_alternative_values(self):
        """unction to return alternative values."""
        vals = {
