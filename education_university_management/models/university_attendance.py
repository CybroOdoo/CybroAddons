...
class UniversityAttendance(models.Model):
    ...
    def action_create_attendance_line(self):
        ...
        if self.search_count(
            ...
            raise ValidationError(
                self.env._('Attendance for this batch on this date already exists.'))
        self.name = str(self.date)
        if len(self.batch_id.batch_student_ids) < 1:
            raise UserError(self.env._('There are no students in this Batch'))
        ...
        for rec in self:
            ...
            for i in range(len(lines)):
                for j in range(i + 1, len(lines)):
                    ...
                    from1 = line1.period_id.time_from
                    to1 = line1.period_id.time_to

                    from2 = line2.period_id.time_from
                    to2 = line2.period_id.time_to

                    # Overlap condition
                    if (from1 < to2) and (to1 > from2):
                        raise ValidationError(
                            "Time overlap detected between periods."
                        )
    ...
    def action_attendance_done(self):
        ...
        if not self.attendance_line_ids:
            raise UserError(_(
                'You cannot complete the attendance action without any students.'))
        for records in self.attendance_line_ids:
