...
class UniversityExam(models.Model):
        ...
        for rec in self:
            if rec.start_date > rec.end_date:
                raise ValidationError(
                    self.env._("Start date must be Anterior to end date"))
    ...
    def action_confirm_exam(self):
        ...
        for rec in self.subject_line_ids:
            if rec.date < self.start_date or rec.date > self.end_date:
                raise ValidationError(
                    "The date must be between the exam start and end date."
                )
        if len(self.subject_line_ids) < 1:
            raise UserError(self.env._('Please Add Subjects'))
