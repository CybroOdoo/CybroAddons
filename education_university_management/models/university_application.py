...
class UniversityApplication(models.Model):
    ...
    def create(self, vals):
        ...
        for val in vals:
            if val.get('application_no', self.env._('New')) == self.env._('New'):
                val['application_no'] = self.env['ir.sequence'].next_by_code(
                    'university.application') or self.env._('New')
    ...
    application_no = fields.Char(string='Application  No',
                                 help="Application number of new admission", translate=True,
    ...
    def action_send_verification(self):
        ...
        for rec in self:
            if not self.env['university.document'].search(
                    [('application_ref_id', '=', rec.id)]):
                raise ValidationError(self.env._('No Documents provided'))
            rec.write({
                'state': 'verification'
            })
            ...
            if doc_status:
                if all(state in 'done' for state in doc_status):
                    ...
                    raise ValidationError(
                        self.env._('All Documents are not Verified Yet, '
    ...
    def _compute_department_ids(self):
        ...
        for rec in self:
            rec.department_ids = self.env['university.department'].search(
                [('course_id', '=',
                  rec.course_id.id)]).ids if rec.course_id else False
    ...
    def _compute_semester_ids(self):
        ...
        for rec in self:
            rec.semester_ids = self.env['university.semester'].search(
                [('department_id', '=',
                  rec.department_id.id)]).ids if rec.department_id else False
    ...
    def _compute_batch_ids(self):
        ...
        for rec in self:
            rec.batch_ids = self.env['university.batch'].search(
                [('semester_id', '=',
                  rec.semester_id.id)]).ids if rec.semester_id else False
    ...
    def _onchange_date_of_birth(self):
        ...
        if self.date_of_birth:
            if (fields.Datetime.today().year - self.date_of_birth.year) < 18:
                raise ValidationError(self.env._('Please provide valid date of birth'))
