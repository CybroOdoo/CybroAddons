...
class TestWmPrintableReports(TransactionCase):
    """Unit tests verifying QWeb PDF route sheet and collection summary report generation."""
    ...
    def setUpClass(cls):
        ...
        cls.point = cls.env['wm.collection.point'].create({'name': 'Print Point 1 TestColl'})
        ...
        brand = cls.env['fleet.vehicle.model.brand'].create({'name': 'Inspect Brand TestColl'})
        cls.vehicle = cls.env['fleet.vehicle'].create({
            'model_id': cls.env['fleet.vehicle.model'].create({'name': 'Inspect Model TestColl', 'brand_id': brand.id}).id,
            'license_plate': 'WM-INSPECT-01',
        })
        cls.item = cls.env['wm.checklist.item'].create({'name': 'Tire Pressure Check TestColl', 'checklist_type': 'pre_trip'})
        ...
        cls.category = cls.env['wm.waste.category'].create({'name': 'Print Category TestColl'})
        cls.pricing_rule = cls.env['wm.pricing.rule'].create({
            'name': 'Standard Tariff TestColl',
