...
class MapViewConfigWizard(models.TransientModel):
    """Wizard to generate map views and attach them to selected actions."""
    ...
    model_name = fields.Char(related='model_id.model', string='Model Technical Name', help='Provides information about model technical name')
    ...
    def _get_partner_field_selection(self):
        """
        Return the list of available address/partner Many2one fields on the
        target model for the map view configuration dropdown.
        """
        if self.model_id:
            partner_fields = self.env['ir.model.fields'].search([
    ...
    def _get_partner_field_domain(self):
        """
        Compute the field domain used to filter partner address fields suitable
        for geographic pin display in the collection point map view.
        """
        return
    ...
    def _onchange_model_id(self):
        """
        Refresh the available partner field selection list when the target
        model is changed in the map view configuration wizard.
        """
        self.action_ids = [(5, 0, 0)]  # Clear all selections
        self._get_partner_field_selection()
        self.partner_field_id = False

    def action_create_map_view(self):
        """
        Create or update the dynamic map view for the selected model using the
        configured partner field, enabling geographic visualisation of
        collection points.
        """
        self.ensure_one()
    ...
    def _build_map_view_arch(self):
        """
        Construct the XML architecture string for the Odoo map view, embedding
        the configured partner coordinate field and marker labels.
        """
        # Build schema-allowed map attributes.
        attrs = []
        if self.partner_field_id:
            attrs.append(f'res_partner="{self.partner_field_id.name}"')

        attrs_str = ' ' + ' '.join(attrs) if attrs else ''

        if self.partner_field_id:
            # For models like sale.order, coordinates are read from related partner;
            # latitude/longitude fields are not declared on the main model.
            arch = f'''<?xml version="1.0"?>
<map{attrs_str}>
    <field name="{self.partner_field_id.name}" string="Partner"/>
</map>'''
        else:
            # Direct coordinate mode: fields must exist on the selected model.
            arch = f'''<?xml version="1.0"?>
<map{attrs_str}>
    <field name="{self.lat_field}" string="Latitude"/>
    <field name="{self.lng_field}" string="Longitude"/>
    <field name="{self.address_field}" string="Address"/>
</map>'''
    ...
    def _update_action_view_mode(self, action):
        """
        Add 'map' to the view modes of the target model's primary window action
        so the map view option appears in the view switcher toolbar.
        """
        ...
        if self.view_position == 'after_list' and 'list' in view_modes:
