...
class LegalEvidence(models.Model):
    ...
    def create(self, vals):
        """Override create method to auto-generate evidence reference."""
        if isinstance(vals, list):
            for val in vals:
                if val.get('name', 'New') == 'New':
                    val['name'] = self.env['ir.sequence'].next_by_code('case_evidence') or 'New'
        else:
            if vals.get('name', 'New') == 'New':
                vals['name'] = self.env['ir.sequence'].next_by_code('case_evidence') or 'New'
    ...
    def action_get_evidence_attachments(self):
