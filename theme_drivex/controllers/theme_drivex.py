...
class DriveXController(http.Controller):
    ...
    def drivex_fleet(self, **kwargs):
        ...
        # Only count/show published vehicles — no fallback to unpublished
        ...
        # Categories only from currently published vehicles
        published_vehicles = request.env['fleet.vehicle'].sudo().search(
            [('is_available_on_website', '=', True)])
        category_ids = published_vehicles.mapped('category_id')
        categories = category_ids.filtered(lambda c: c.id)
    ...
    def drivex_booking_submit(self, **post):
        ...
        try:
            # If it's a demo fallback booking, return a fake success immediately
            if str(vehicle_id) == '0':
                return {'success': True, 'order_id': 0,
                        'order_name': 'DRX-DEMO'}
            ...
            vehicle = request.env['fleet.vehicle'].sudo().browse(
                int(vehicle_id))
            base_amount = round(vehicle.daily_rate * days)

            insurance_amount = 0.0
            if insurance_id and str(insurance_id).isdigit():
                ins = request.env['fleet.rental.insurance'].sudo().browse(
                    int(insurance_id))
                insurance_amount = round(ins.price_per_day * days)
            ...
            for addon_id in addon_ids:
                if str(addon_id).isdigit():
                    ...
                    if add.charge_type == 'per_day':
                        addons_amount += round(add.price * days)
                    else:
                        addons_amount += round(add.price)
            ...
            order_vals = {
                ...
                'pickup_location_id': int(pickup_loc_id) if str(
                    pickup_loc_id).isdigit() else False,
                'return_location_id': int(
                    return_loc_id) if return_loc_id and return_loc_id != 'same' and str(
                    return_loc_id).isdigit() else (int(pickup_loc_id) if str(
                    pickup_loc_id).isdigit() else False),
                ...
                'insurance_id': int(insurance_id) if insurance_id and str(
                    insurance_id).isdigit() else False,
                'addon_ids': [
                    (6, 0, [int(x) for x in addon_ids if str(x).isdigit()])],
            ...
            return {'success': True, 'order_id': order.id,
                    ...
                    'amount_insurance': order.amount_insurance,
                    'amount_addons': order.amount_addons,
    ...
    def drivex_homepage_data(self, **kwargs):
        ...
        Returns locations, categories, and featured vehicles to populate
        the homepage snippet and search box without using t- directives in
        snippet templates.  Categories are always derived from published
        vehicles so they stay in sync with backend changes.
        """
        locations = request.env['fleet.rental.location'].sudo().search(
            [('active', '=', True)])

        # Only consider published (website-available) vehicles
        published_vehicles = request.env['fleet.vehicle'].sudo().search(
            [('is_available_on_website', '=', True)])
        has_vehicles = bool(published_vehicles)

        # Categories derived exclusively from published vehicles
        category_records = published_vehicles.mapped('category_id').filtered(
            lambda c: c.id)

        vehicles = []
        if has_vehicles:
            # Prefer vehicles flagged for homepage; fall back to first 3 published
            featured = published_vehicles.filtered(
                lambda v: getattr(v, 'publish_on_homepage', False))
            if not featured:
                featured = published_vehicles[:3]
            for vehicle in featured[:3]:
                vehicles.append({
                    'id': vehicle.id,
                    'name': (
                        f"{vehicle.model_id.brand_id.name} {vehicle.model_id.name}"
                        if vehicle.model_id else vehicle.name
                    ),
                    'category': vehicle.category_id.name if vehicle.category_id else '',
                    'category_lower': (
                        vehicle.category_id.name.lower()
                        if vehicle.category_id else ''
                    ),
                    'image': f"/web/image/fleet.vehicle/{vehicle.id}/image_128",
                    'daily_rate': vehicle.daily_rate or 0,
                    'horsepower': vehicle.horsepower or 0,
                    'transmission': (
                        vehicle.transmission.capitalize()
        ...
        return {
            'locations': [{'id': loc.id, 'name': loc.name} for loc in
                          locations],
            'categories': [
                {'name': cat.name, 'name_lower': cat.name.lower()}
