...
class FleetRentalOrder(models.Model):
    ...
    pickup_location_id = fields.Many2one('fleet.rental.location',
                                         string='Pickup Location',
                                         required=True,
                                         help='Location for pickup.')
    return_location_id = fields.Many2one('fleet.rental.location',
                                         string='Return Location',
                                         required=True,
                                         help='Location for return.')
    ...
    driver_license = fields.Char(string='License Number',
                                 help='Driver license number.')
    driver_dob = fields.Date(string='Date of Birth',
                             help='Driver date of birth.')
    driver_license_expiry = fields.Date(string='License Expiry Date',
                                        help='Expiry date of the driving license.')
    driver_license_state = fields.Char(string='Issuing State',
                                       help='State that issued the driving license.')
    driver_nationality = fields.Char(string='Issuing Country',
                                     help='Country that issued the driving license.')
    driver_personal_nationality = fields.Char(string='Nationality',
                                              help='Personal nationality of the driver.')
    ...
    currency_id = fields.Many2one('res.currency', string="Currency",
                                  ...
                                  readonly=True,
                                  help='Currency for the booking.')
    insurance_id = fields.Many2one('fleet.rental.insurance',
                                   string='Insurance Plan',
                                   help='Selected insurance plan.')
    addon_ids = fields.Many2many('fleet.rental.addon', string='Add-ons',
                                 help='Selected add-ons.')

    amount_base = fields.Float(string='Base Rate', tracking=True,
                               help='Base rental rate.')
    amount_insurance = fields.Float(string='Insurance Cost', tracking=True,
                                    help='Cost of insurance.')
    amount_addons = fields.Float(string='Add-ons Cost', tracking=True,
                                 help='Cost of add-ons.')
