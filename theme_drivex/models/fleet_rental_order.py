...
class FleetRentalOrder(models.Model):
    ...
    vehicle_id = fields.Many2one('fleet.vehicle',
                                 string='Vehicle', required=True, tracking=1,
                                 help='Rented vehicle.')
    ...
    def _compute_amount_total(self):
        ...
        for order in self:
            order.amount_total = (order.amount_base + order.amount_insurance +
                                  order.amount_addons)

    @api.constrains('vehicle_id', 'pickup_datetime', 'return_datetime',
                    'state')
