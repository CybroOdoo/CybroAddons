...
class DatePicker {
    ...
    _renderGrid() {
        ...
        for (let d = 1; d <= daysInMon; d++) {
            ...
            // allowPast: past dates selectable (e.g. DOB)
            // blockFuture: future dates disabled (e.g. DOB)
...
// ── Global Helper: show a simple AlertDialog from anywhere the dialog service is bound ──
...
// Reuses the existing DatePicker class. The panel markup has no .dp-time
// section so time is absent. hiddenInput receives YYYY-MM-DD only.
...
// ── Widget: Fleet Filter (Homepage Snippet) ───────────────────────────────────
// Handles filter-btn clicks inside the Fleet Section snippet.
// The ThemeDriveXFleetSnippet widget populates the cards; this widget
// responds to user interactions after they are in the DOM.
...
publicWidget.registry.ThemeDriveXFleetPage = publicWidget.Widget.extend({
    ...
    events: {
        ...
        'input #carSearch': '_onSearch',
        'click .fv-btn': '_onViewToggle',
        'change #sortSelect': '_onSort',
        'click #resetFiltersBtn': '_onResetFilters',
    },
    start() {
        ...
        // Initialize original order index for default sort
        this.el.querySelectorAll(".fc-card").forEach((card, index) => {
            if (!card.dataset.index) card.dataset.index = index;
        });

        // Update counts from real rendered cards
        this._updateCount();

        // Update hero stats from rendered data
        this._updateHeroStats();

        // Hide category filter bar when no vehicles
    ...
    _updateCount() {
        ...
        if (!cnt) return;
        const visible = Array.from(this.el.querySelectorAll('.fc-card'))
            .filter(c => c.style.display !== 'none').length;
        cnt.innerHTML = `Showing <strong>${visible}</strong> vehicle${visible !== 1 ? 's' : ''}`;
    },
    _updateHeroStats() {
        // Derive counts from rendered fc-card elements (always live)
        const cards = this.el.querySelectorAll('.fc-card');
        ...
        cards.forEach(c => {
            ...
            if (cat && cat !== 'all') {
                cat.split(' ').forEach(part => { if (part) catSet.add(part); });
            }
        });
        const heroCount = document.getElementById('heroVehicleCount');
        const heroCats = document.getElementById('heroCategoryCount');
        if (heroCount) heroCount.textContent = total > 0 ? total + '+' : total;
        if (heroCats) heroCats.textContent = catSet.size;
    },
    _onCatFilter(e) {
        const btn = e.currentTarget;
        ...
        const cnt = document.getElementById('resultsCount');
        if (cnt) cnt.innerHTML = `Showing <strong>${visible}</strong> vehicle${visible !== 1 ? 's' : ''}`;
        const noRes = document.getElementById('noResults');
        const grid = document.getElementById('fleetGrid');
        if (noRes) noRes.style.display = (visible === 0 && cards.length > 0) ? '' : 'none';
        // Remove fc-grid when no cards visible (just no-results message)
        if (grid) grid.classList.toggle('fc-grid', visible > 0);
    },
    _onSearch(e) {
        const q = e.currentTarget.value.toLowerCase().trim();
        // Reset category filter to "all" when searching
        this.el.querySelectorAll('.fb-cat').forEach(b => b.classList.remove('active'));
        const allBtn = this.el.querySelector('.fb-cat[data-cat="all"]');
        ...
        if (curr) {
            ...
            for (let input of inputs) {
                if (!input.value || input.value.trim() === "") {
                    showAlert(this.dialog, _t("Please fill in all required fields."), );
                    input.focus();
                    return;
                }
            }
            ...
            .then(data => {
                if (data.result && data.result.success) {
                    ...
                    if (sumDays && data.result.days !== undefined) {
                        console.log(111, sumDays, data.result.days);
                    ...
                    showAlert(this.dialog, _t("Booking Error: %s", data.result ? data.result.error : _t("Unknown error")),);
                }
            })
            .catch(err => {
                console.error(err);
...
publicWidget.registry.ThemeDriveXHomepageLoader = publicWidget.Widget.extend({
    ...
    start: function () {
        ...
        fetch('/homepage/data', {
            ...
            .catch(function (err) {
                ...
                // On fetch error, populate empty states
                self._showUnavailableStates();
            });
    ...
    _populateCarTypes: function (categories, hasVehicles) {
        ...
        carTypeMenu.innerHTML = '<div class="cs-opt cs-opt--active" data-value="all"><span class="cs-opt-icon"><i class="fa-solid fa-car-side"></i></span>All Vehicles</div>';

        if (!hasVehicles || !categories.length) return;
    ...
    _populateFleetFilters: function (categories, hasVehicles) {
        ...
        fleetFilter.innerHTML = '<button class="filter-btn active" data-filter="all">All</button>';
        ...
        // Force flex centering in case Website Builder CSS overrides the SCSS rule
    ...
    _showUnavailableStates: function () {
...
// Uses /homepage/data so vehicle data is always live, never baked into the
// saved page arch.  Re-initialisation safe: clears containers before rebuild.
publicWidget.registry.ThemeDriveXFleetSnippet = publicWidget.Widget.extend({
    selector: '#snippetFleetGrid',

    start: function () {
        var self = this;
        var grid   = this.el;                                    // #snippetFleetGrid
        var filter = document.getElementById('snippetFleetFilter');

        // Guard: avoid double-init if widget fires twice on the same element
        if (grid.dataset.dxSnippetLoaded) {
            return this._super.apply(this, arguments);
        }
        grid.dataset.dxSnippetLoaded = '1';

        // Force flex centering on the filter bar — Website Builder can override SCSS
        if (filter) {
        ...
        fetch('/homepage/data', {
            ...
            .then(function (data) {
                if (data && data.result) {
                    self._buildFilters(filter, data.result.categories || [], data.result.has_vehicles);
                    self._buildCards(grid, data.result.vehicles || [], data.result.has_vehicles);
                } else {
                    self._buildFilters(filter, [], false);
                    self._showUnavailable(grid);
                }
            })
            .catch(function () {
                self._buildFilters(filter, [], false);
    ...
    _buildFilters: function (filter, categories, hasVehicles) {
        if (!filter) return;
        ...
        // Remove previously injected category buttons (safe re-init)
    ...
    _buildCards: function (grid, vehicles, hasVehicles) {
        grid.innerHTML = '';
        if (!hasVehicles || !vehicles.length) {
            this._showUnavailable(grid);
            return;
        }
        vehicles.forEach(function (v) {
            var card = document.createElement('div');
    ...
    _showUnavailable: function (grid) {
        grid.innerHTML =
