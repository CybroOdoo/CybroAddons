...
#    Author: Raneesha MK(<https://www.cybrosys.com>)
#
...
class ResUsers(models.Model):
    ...
    @api.model
    def menu_lock_search(self, args, action=False, action_type=False):
        """Method to returning values though args"""
        user = self.env['res.users'].browse(args)
        locked_models = False
        if action_type == 'ir.actions.act_window':
            action = self.env['ir.actions.act_window'].with_user(
                SUPERUSER_ID).browse(int(action))
            if action.res_model in user.models_to_lock_ids.mapped('model'):
                locked_models = True
            else:
                locked_models = False
        return {
    ...
    @api.onchange('multi_lock_ids')
    def _onchange_multi_lock_ids(self):
        """To update 'menus_to_lock_ids' while changing the One2many menus list
        """
        self.menus_to_lock_ids = [fields.Command.set((
            self.multi_lock_ids.mapped('menus_id').ids))]
