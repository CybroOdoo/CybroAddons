...
patch(DropdownItem.prototype, "menu_lock", {
    ...
    async onClick(ev) {
        ...
        await rpc.query({
            ...
            /** Check if current menu is lock menu **/
            if (menu.length != 0 && menu[0].id && menu[0].password) {
                ...
                dialog.opened().then(() => {
                    ...
                    /** dialog box confirm button function, if matches successfully logins to the menu,
                        otherwise show error warning. **/
                    dialog.buttons[0].click = function(event) {
                        var current_pwd = dialog.$el.find('#password').val()
                        /** if password matches close the modal and enter into the model **/
