...
publicWidget.registry.BestProduct = animations.Animation.extend({
    ...
    start: async function () {
        ...
        var data = await rpc('/top_deal_product_snippet', {})
            if (data) {
                console.log('data',data)
                console.log(self.$target)
                this.$target.empty().html(renderToElement('theme_shopping.best_deal_product_carousel_snippet', {products: data}))
                self._initCarousel();
            }
    ...
    _clearContent: function () {
        const $templateArea = this.$el.find('.best_deal_products_carousel');
