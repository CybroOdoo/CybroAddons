...
def _create(self, data_list):
    ...
    # Custome code
    if records._name not in ['ir.module.module','ir.model.data']:
                ...
                for item in webhook:
                    ...
                    try:
                        ...
                        raise ValidationError(
                            "Please check the Webhook Url for "
                            "Create Event")
    return records
...
def write(self, vals):
    ...
    * Other non-relational fields use a string for value
    ...
    if self._name != 'ir.module.module' and 'webhook.webhook' in self.env:
        webhook = self.env['webhook.webhook'].search(
            [('model_id', '=', self.env['ir.model'].sudo().search(
                [('model', '=', self._name)]).id)]).filtered(
                    ...
                    if field.ttype == 'binary':
                        if vals[field.name]:
                            base_url = self.env[
                                ...
                                'web.base.url')
                            vals[field.name] = (
                                f'{base_url}/web/image/{rec._name}/{rec.id}'
                                f'/{field.name}')
                for item in webhook:
                    # Post payload to the registered url
                    try:
                        ...
                        raise ValidationError(
                            _("Please check the Webhook Url for "
                              "Edit Event"))
                del vals['id']
