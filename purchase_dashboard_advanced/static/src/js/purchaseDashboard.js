...
class purchaseDashboard extends  Component {
     ...
     */
     renderTopProduct() {
        ...
        if (window.myCharts_top_priority) {
            window.myCharts_top_priority.destroy();
        }

        // Prepare chart data
        const labels = this.purchase.topChart[1].map(item => item.en_US || item);
        const datas = this.purchase.topChart[0];

        const backgroundColor = datas.map(() =>
     ...
     */
        goBackend(){
            ...
            const orderIds = this.purchase.orders.order_ids;
            if (orderIds){
