...
class PurchaseOrder(models.Model):
    ...
    def get_yearly_data(self):
        ...
        query_current_vendors = """
               ...
               array_agg(id) AS order_ids, array_agg(partner_id) AS vendor_ids
               FROM purchase_order
               WHERE company_id = %s
                   AND state IN ('purchase', 'done')
                   AND EXTRACT(YEAR from date_order) = EXTRACT(YEAR from now())
    ...
    def get_monthly_data(self):
        ...
        query = """
            SELECT DISTINCT partner_id, po.id
            FROM purchase_order po
            WHERE company_id = %s
                AND state IN ('purchase','done')
                AND EXTRACT(YEAR from date_order) = EXTRACT(YEAR FROM
        ...
        monthly = {
            ...
            'vendors': len(set([rec['partner_id'] for rec in vendors])),
            'vendor_id': [rec['partner_id'] for rec in vendors],
            'order_ids': [rec['id'] for rec in vendors],
        }
        return monthly
    ...
    def get_weekly_data(self):
        ...
        query = """
            ...
            THEN 1 ELSE NULL END), array_agg(partner_id) AS partner_ids, 
        ...
        self.env.cr.execute(query, [company])
        vendors = self.env.cr.dictfetchall()
        new_vendors = [rec['partner_id'] for rec in vendors]
        weekly = {
    ...
    def get_today_data(self):
        ...
        return {
            'order_ids': today_data['order_ids'],
    ...
    def purchase_vendor_details(self, args):
        ...
        partner_orders = self.env.cr.dictfetchall()
        query_draft = """
            SELECT count(po.id),SUM(po.amount_total), EXTRACT(MONTH from 
            po.date_order) as dates 
            FROM purchase_order po 
        ...
        draft_orders = self.env.cr.dictfetchall()
        approve_qry = """
            SELECT count(po.id),SUM(po.amount_total), EXTRACT(MONTH from 
            po.date_order) as dates 
            FROM purchase_order po 
        ...
        approve_orders = self.env.cr.dictfetchall()
        cancel_qry = """
            SELECT count(po.id),SUM(po.amount_total), EXTRACT(MONTH from 
            po.date_order) as dates 
            FROM purchase_order po 
        ...
        cancel_orders = self.env.cr.dictfetchall()
        all_orders = {
    ...
    def get_upcoming_purchase_data(self):
        ...
        value = {
            ...
            'amount': [rec['amount_total'] for rec in orders],
            'date': [rec['date_planned'] for rec in orders],
            'state': [STATE_LABELS.get(rec['state'], rec['state']) for rec in
                      orders],  # mapped labels
            'data': [list(val for val in rec.values()) for rec in orders],
        }
        return value
