...
publicWidget.registry.ProductDesigner = publicWidget.Widget.extend({
    ...
    async _onSaveAreaConfig(ev) {
        ...
        try {
            const result = await jsonrpc('/shop/designer/save_area_config', {
    ...
    _loadTemplates() {
        ...
        jsonrpc('/shop/designer/get_templates', {
    ...
    async _onVariantChange(ev) {
        ...
        try {
            const data = await jsonrpc('/shop/designer/get_variant_info', {
                product_id: parseInt(this.productId),
    ...
    _loadSavedDesign() {
        ...
        jsonrpc('/shop/designer/load_saved', {
            product_id: parseInt(this.productId),
            variant_id: this.variantId || 0,
        }).then((result) => {
    ...
    async _onAddToCart() {
        ...
        jsonrpc('/shop/designer/save', {
            ...
            if (saveResult.success && saveResult.customization_id) {
                ...
                jsonrpc('/shop/designer/add_to_cart', {
                    product_id: parseInt(this.productId),
    ...
    async _onSaveAsTemplate() {
        ...
        jsonrpc(route, params).then((result) => {
            if (btn) {
                btn.disabled = false;
            }
