...
class ProductProduct(models.Model):
    ...
    # Computed image fields that override the _inherits delegation.
    # Read-only computed: returns variant image when set, else template image.
    # No inverse — uploads go to design_base_image_variant (variant form)
    # or design_base_image on product.template (template form via _inherits).
    ...
    # -------------------------------------------------------------------------
    design_variant_override = fields.Boolean(
    ...
    variant_design_area_left = fields.Float("Variant Design Area Left (%)", default=0.0)
    variant_design_area_top = fields.Float("Variant Design Area Top (%)", default=0.0)
    variant_design_area_width = fields.Float("Variant Design Area Width (%)", default=100.0)
    variant_design_area_height = fields.Float("Variant Design Area Height (%)", default=100.0)

    # Variant-specific design area (back)
    variant_design_area_back_left = fields.Float("Variant Back Design Area Left (%)", default=0.0)
    variant_design_area_back_top = fields.Float("Variant Back Design Area Top (%)", default=0.0)
    variant_design_area_back_width = fields.Float("Variant Back Design Area Width (%)", default=100.0)
    variant_design_area_back_height = fields.Float("Variant Back Design Area Height (%)", default=100.0)

    # Variant-specific settings
    variant_has_front_back = fields.Boolean("Two-Sided Design")
    variant_design_instruction = fields.Html("Variant Design Instructions", translate=True)
    variant_design_max_texts = fields.Integer("MVariant ax Text Objects", default=0)
    variant_design_max_images = fields.Integer("Variant Max Image Objects", default=0)
    variant_design_max_characters = fields.Integer("Variant Max Characters per Text", default=25)
    variant_min_order_quantity = fields.Integer("Variant Minimum Order Quantity", default=1)
    variant_max_order_quantity = fields.Integer("Variant Maximum Order Quantity", default=10000)
    variant_production_time_days = fields.Integer("Variant Production Time (Days)", default=3)
    variant_shipping_weight_per_unit = fields.Float("Variant Shipping Weight per Unit (kg)", default=0.1)
    ...
    def _get_design_config(self):
        ...
        When ``design_variant_override`` is True, variant-specific values are
        ...
        if self.design_variant_override:
            ...
            return {
                'has_front_back': self.variant_has_front_back,
