...
class ProductDesignCustomization(models.Model):
    ...
    def action_approval(self):
        """State change to approved state"""
        self.write({'state' :'approved'})

    def action_production(self):
        """State change to in_production state"""
        self.write({'state': 'in_production'})

    def action_completed(self):
        """State change to completed state"""
        self.write({'state': 'completed'})

    def action_cancelled(self):
        """State change to canceled state"""
        self.write({'state': 'cancelled'})

    def action_draft(self):
        """State change to draft state"""
