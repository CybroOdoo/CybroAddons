...
class StockMoveReport(models.TransientModel):
    ...
    def _onchange_product_category_id(self):
        """ Function for dynamic domain in product filed """
        if self.product_category_id:
            domain = [('categ_id', '=', self.product_category_id.id)]
            return {'domain': {'product_id': domain}}

    def action_print_pdf_report(self):
        """Print PDF report with filtered data passed to the template."""
        if (self.from_date and self.to_date and self.from_date > self.to_date) or \
                (self.to_date and self.to_date > fields.datetime.now()):
            raise ValidationError("Please enter a valid To Date")
        ...
        query = f"""
            WITH RECURSIVE CategoryHierarchy AS (
                SELECT id, name, parent_id, complete_name FROM product_category WHERE id = '{self.product_category_id.id}'
                UNION ALL 
                SELECT c.id, c.name, c.parent_id, c.complete_name FROM product_category c JOIN CategoryHierarchy ch ON c.parent_id = ch.id
            )
            SELECT stock_move.date, stock_move.reference, product_template.name, stock_location.complete_name,
                   product_category.complete_name as category_name, stock_move.product_uom_qty, res_company.name, 
            ...
            JOIN res_company ON stock_move.company_id = res_company.id 
            WHERE product_category.id IN (SELECT id FROM CategoryHierarchy)
        """
        filters = []
        if self.product_id:
            filters.append(f"stock_move.product_id = '{self.product_id.id}'")
        if self.location_id:
            filters.append(f"stock_move.location_id = '{self.location_id.id}'")
        if self.company_id:
            filters.append(f"stock_move.company_id = '{self.company_id.id}'")
        if self.from_date:
            filters.append(f"stock_move.date >= '{self.from_date}'")
        if self.to_date:
            filters.append(f"stock_move.date < '{self.to_date}'")
        if filters:
        ...
        return self.env.ref(
            'warehouse_reports.stock_move_report').report_action(None,
                                                                 data=data)
