...
class StockValuationReport(models.TransientModel):
    ...
    def action_print_pdf_report(self):
        ...
        query = """ WITH RECURSIVE CategoryHierarchy AS ( SELECT id,name, 
                    ...
                    stock_move.date as create_date, product_template.name->>{}
                     as name, stock_move.reference as description, 
                     product_category.complete_name,res_company.name as 
                     company_name, 
                     CASE WHEN stock_move.is_out = true THEN -stock_move.quantity ELSE stock_move.quantity END as quantity, 
                     stock_move.price_unit as unit_cost, 
                     CASE WHEN stock_move.is_out = true THEN -stock_move.value ELSE stock_move.value END as value 
                     ...
                     product_template.id = product_product.product_tmpl_id JOIN
                      stock_move on product_product.id = 
                      stock_move.product_id JOIN res_company on 
                      stock_move.company_id = res_company.id AND stock_move.state = 'done'
        ...
        if self.product_id and self.company_id:
            self.env.cr.execute(
                """{}where product_product.id='{}' and stock_move.company_id
                 ='{}'""".format(
            ...
            self.env.cr.execute(
                """{} where stock_move.company_id='{}' """.format(
                    query, company_id))
        elif self.from_Date:
            self.env.cr.execute(
            """{} where stock_move.date >= '{}' """.format(query,from_date))
        elif self.to_date:
            self.env.cr.execute(
            """{} where stock_move.date <= '{}' """.format(query, to_date))
