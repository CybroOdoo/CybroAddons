...
class CarRentalContract(models.Model):
    """
    Main model for managing car rental contracts, including checklist tracking,
    recurring invoices, and vehicle availability.
    """
    ...
    reserved_fleet_id = fields.Many2one('rental.fleet.reserved', copy=False,
                                        help="Reserved fleet record")
    ...
    end_time = fields.Char(string="End By",
                           ...
                           tracking=True)
    is_rent_by_hour = fields.Boolean(string="Rent By Hour",
    ...
    state = fields.Selection(
        ...
        help="Current state of the contract")
    notes = fields.Text(string="Details & Notes", help="Notes")
    ...
    first_payment_inv = fields.Many2one('account.move', copy=False,
                                       help="First payment invoice")
    is_first_invoice_created = fields.Boolean(string="First Invoice Created",
                                              copy=False,
                                              help="Whether the first invoice is created")
    ...
    total = fields.Float(string="Total (Accessories/Tools)", readonly=True,
                         copy=False, help="Total amount of accessories/tools", compute="_compute_total", store=True)
    tools_missing_cost = fields.Float(string="Missing Cost", readonly=True,
                                      copy=False,
                                      help='This is the total amount of '
                                           'missing tools/accessories', compute="_compute_total", store=True)
    damage_cost = fields.Float(string="Damage Cost / Balance Amount",
                               copy=False, help="Cost of damage")
    damage_cost_sub = fields.Float(string="Damage Cost / Balance Amount",
                                   readonly=True,
                                   copy=False, help="Subtotal of damage cost", compute="_compute_total", store=True)
    total_cost = fields.Float(string="Total", readonly=True, copy=False,
                              help="Total damage cost", compute="_compute_total", store=True)
    invoice_count = fields.Integer(compute='_compute_invoice_count',
                                   string='# Invoice', copy=False,
                                   help="Number of invoices")
    is_check_verify = fields.Boolean(compute='_compute_is_check_verify',
                                     copy=False, help="Check verify")
    sales_person = fields.Many2one('res.users', string='Sales Person',
                                   default=lambda self: self.env.uid,
                                   tracking=True, help="Sales person responsible")
    is_read_only = fields.Boolean(string="Read Only",
                                  help="To make field read only")
    ...
    def action_run(self):
        ...
        """
        self.state = 'running'
        self.vehicle_id.state_id = self.env.ref('fleet_rental.vehicle_state_rent').id

    @api.depends('checklist_line.is_checklist_active')
    def _compute_is_check_verify(self):
        """
            Update the 'is_check_verify' field based on the value of
            'checklist_active' in 'checklist_line'.
        """
        for record in self:
            record.is_check_verify = all(
                line.is_checklist_active for line in record.checklist_line)
    ...
    @api.depends('checklist_line.price', 'checklist_line.is_checklist_active', 'damage_cost')
    def _compute_total(self):
       ...
       """
        for record in self:
            total = sum(record.checklist_line.mapped('price'))
            tools_missing_cost = sum(record.checklist_line.filtered(lambda l: not l.is_checklist_active).mapped('price'))
            record.total = total
            record.tools_missing_cost = tools_missing_cost
            record.damage_cost_sub = record.damage_cost
            record.total_cost = tools_missing_cost + record.damage_cost

    @api.model
    def _get_fleet_service_product(self):
        """
            Get the configured rental service product with a module default
            fallback.
        """
        config_parameter = self.env['ir.config_parameter'].sudo()
        product_id = config_parameter.get_param(
            'fleet_rental.fleet_service_product_id')
        product = self.env['product.product']
        if product_id:
            product = self.env['product.product'].browse(int(product_id)).exists()
        if not product:
            product = self.env.ref('fleet_rental.fleet_service_product')
        return product
    ...
    def fleet_scheduler1(self, rent_date):
        ...
        product_id = self._get_fleet_service_product()
        ...
        for records in self.search([]):
            ...
            if end_date >= date.today():
                ...
                if temp == 1 and records.cost_frequency != "no" and records.state == "running":
                    ...
                    product_id = records._get_fleet_service_product()
                    if product_id.property_account_income_id.id:
                        income_account = product_id.property_account_income_id
                    elif product_id.categ_id.property_account_income_categ_id.id:
                        income_account = product_id.categ_id.property_account_income_categ_id
                    ...
                    inv_data = {
                        'ref': supplier.name,
                        'partner_id': supplier.id,
                        'currency_id': records.company_id.currency_id.id,
                    ...
                    mail_content = _(
                        ...
                        '<tr/><tr><td>Due Date <td/><td> %s<td/><tr/>'
                        '<tr/><tr><td>Responsible Person <td/><td> %s, %s<td/><tr/><table/>') % \
                                   (records.customer_id.name, records.name,
                                    inv_id.amount_total,
                                    inv_id.invoice_date_due,
                                    inv_id.user_id.name,
                    ...
                    main_content = {
                        ...
                        'author_id': self.env.user.partner_id.id,
                        'body_html': mail_content,
                        'email_to': records.customer_id.email,
                    }
                    self.env['mail.mail'].create(main_content).send()
            else:
                if records.state == 'running':
    ...
    def action_verify(self):
        ...
        self.vehicle_id.is_rental_check_availability = True
        self.vehicle_id.state_id = self.env.ref('fleet_rental.vehicle_state_active').id
        product_id = self._get_fleet_service_product()
        if product_id.property_account_income_id.id:
            income_account = product_id.property_account_income_id
        elif product_id.categ_id.property_account_income_categ_id.id:
    ...
    def action_confirm(self):
        """
           ...
           confirmation email.
        """
        self.vehicle_id.is_rental_check_availability = False
    ...
    def action_force_checking(self):
        """
    ...
    def action_invoice_create(self):
        ...
        """
        for each in self:
            rent_date = each.rent_start_date
            if each.cost_frequency != 'no' and rent_date < date.today():
                ...
                for i in range(0, rental_days + 1):
        ...
        self.is_first_invoice_created = True
        inv_obj = self.env['account.move']
        supplier = self.customer_id
        inv_data = {
            'ref': supplier.name,
