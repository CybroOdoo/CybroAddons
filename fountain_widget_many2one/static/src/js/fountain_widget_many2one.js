...
export class FountainWidget extends Component {
    ...
    static components = { DropdownItem,Dropdown,FountainDropdown};
    //Extending component and creating a new widget
    setup() {
        this.orm = useService('orm');
        useEffect(() => {
            this.state.inputData = this.props.record.data[this.props.name]
            }, () => [this.props])
        this.state = useState({
            childMenu: {},
        ...
        onWillStart(async () => {
            this.state.category = await this.orm.searchRead(this.props.record.fields[this.props.name].relation, []);
        });
        this.index = 0
    }
   ...
   //    Click function of the options
    onClickDropDown(parent_id, index = 0) {
        ...
        let value = this.state.category.filter(item => item.id === parent_id)
        this.state.inputData[0] = parent_id
        this.state.inputData[1] = value[0].complete_name
        return obj
    }
}
