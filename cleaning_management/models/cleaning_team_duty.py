...
class CleaningTeamDuty(models.Model):
    ...
    inspection_boolean = fields.Boolean(string="Is Inspection", default=True
                                    ,compute="_compute_inspection_boolean",
    ...
    def _compute_inspection_count(self):
        """Compute the number of inspections linked to each cleaning team duty."""
        for record in self:
    ...
    def _compute_inspection_boolean(self):
        """Set a boolean indicating whether the record has any inspections."""
        for rec in self:
            rec.inspection_boolean = rec.inspection_count > 0
