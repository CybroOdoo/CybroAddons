...
class AccountAnalyticLine(models.Model):
    ...
    def write(self, values):
        """Generate work report on updating record."""
        result = super().write(values)
        params = self.env['ir.config_parameter'].sudo()
        is_generate_work_report = params.get_param(
            'work_report_from_timesheet.is_generate_work_report')
        report_method = params.get_param(
            'work_report_from_timesheet.report_method')
        if is_generate_work_report and report_method == 'task_report':
    ...
    def send_employee_daily_work_report(self):
        ...
        report_method = params.get_param(
            'work_report_from_timesheet.report_method')
        if is_generate_work_report and report_method == 'daily_report':
            manager_email = params.get_param(
                'work_report_from_timesheet.employee_id')
            cc_employees = literal_eval(
                params.get_param('work_report_from_timesheet.employee_ids'))
