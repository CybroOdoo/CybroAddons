...
class ImportImage(models.TransientModel):
    ...
    product_template_selection = fields.Selection([
        ('name', "Name"), ('internal_reference', "Internal Reference"),
        ('barcode', "Barcode")], string="Based On", default='name',
        help="Choose what type of reference the image files are named after", )
    res_partner_selection = fields.Selection([('name', "Name")], string="Based On", default='name',
                                             help="Choose what type of reference the "
                                                  "image files are named after")
    product_product_selection = fields.Selection([('internal_reference', "Internal Reference"), ('barcode', "Barcode")],
                                                 default='internal_reference',
                                                 help="Choose what type of reference the image files are named after",
                                                 string="Based on")
    hr_employee_selection = fields.Selection([('name', "Name"), ('identification_no', "Identification No")],
                                             default="name",
                                             help="Choose what type of reference the image files are named after",
                                             string="Based on")
    model_template = fields.Selection([('product.template', "Product Template"),
                                       ('product.product', "Product Product"), ('hr.employee', "Employee"),
                                       ('res.partner', "Partner")], default='product.template',
                                      help="Choose what type of reference the image files are named after",
        ...
        try:
            with zipfile.ZipFile(main_file, 'r') as zip_file:
                ...
                if self.model_template == "product.template":
                    reference = self.product_template_selection
