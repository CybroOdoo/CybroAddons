...
class CommissionReport(models.TransientModel):
    ...
    def action_print_xls_report(self):
        ...
        # If neither filter is selected, default to all sales teams
        ...
        for user, user_sale_orders in user_sale_orders_dict.items():
            ...
            filtered_order_lines = user_sale_orders.filtered(
                lambda l: (
                    (not self.date_from or l.date_order.date() >= self.date_from) and
                    (not self.date_to or l.date_order.date() <= self.date_to)
                )
            ).mapped('order_line')
            ...
            if commission_id.type == 'product':
                ...
                matched_rules = commission_id.product_comm_ids.filtered(
            ...
            if commission_id.type == 'revenue' and commission_id.revenue_type == 'graduated':
                for rule in commission_id.revenue_grd_comm_ids:
                    match = rule.amount_from <= filtered_order_lines_commission_total < rule.amount_to
                    if match:
            ...
            if commission_id.type == 'revenue' and commission_id.revenue_type == 'straight':
                ...
                user_commission_salesperson.append(user.name)
                total_list.append(filtered_order_lines_commission_total)
        # -- END SALESPERSON SECTION -----------------------------------------

        # -- SALES TEAM SECTION ----------------------------------------------
        team_wizard_persons = self.sales_team_ids.mapped('member_ids')
        ...
        for team_user in team_obj:
            team_sale_orders_dict.update({
                team_user: team_sale_orders.filtered(
                    lambda l, u=team_user: l.user_id == u)
            })

        # Build map: user -> wizard-selected team for correct commission fallback
        user_team_map = {}
        for team in self.sales_team_ids:
            for member in team.member_ids:
                if member not in user_team_map:
                    user_team_map[member] = team

        for team_user, team_sale_orders in team_sale_orders_dict.items():
            wizard_team = user_team_map.get(team_user)
            commissions_id = team_user.commission_id if team_user.commission_id \
                else (wizard_team.commission_id if wizard_team else False)
            ...
            if commissions_id:
                ...
                if commissions_id.type == 'product':
                    ...
                    prod_commission = filtered_order_lines.filtered(
                        lambda l: l.product_id in commission_products)
                    matched_rules = commissions_id.product_comm_ids.filtered(
                        lambda l: l.product_id in prod_commission.mapped('product_id'))
                    if not matched_rules:
                        ...
                        commission_sales_team.append(wizard_team.name if wizard_team else '')
                        commission.append(0.0)
                    else:
                        for rules in matched_rules:
                            product_order_line = prod_commission.filtered(
                ...
                if commissions_id.type == 'revenue' and commissions_id.revenue_type == 'graduated':
                    for rules in commissions_id.revenue_grd_comm_ids:
                        match = rules.amount_from <= filtered_order_lines_commission_total < rules.amount_to
                        if match:
                            graduated_commission = (filtered_order_lines_commission_total
                ...
                if commissions_id.type == 'revenue' and commissions_id.revenue_type == 'straight':
                    straight_commission = (filtered_order_lines_commission_total
    ...
    def get_xlsx_report(self, data, response):
        ...
        sheet.merge_range('A2:B2', "Printed Date: " + str(date), date_format)
        ...
        if user:
            ...
            sheet.write('D2', 'Date From: ' + str(data['date_from']), date_format)
            sheet.write('E2', 'Date To: ' + str(data['date_to']), date_format)
            ...
            for sp_name in user_commission_salesperson:
                sheet.write(row, col + 0, index, format2)
                sheet.write(row, col + 1, sp_name, format1)
                row += 1
                index += 1

            row = 5
            col = 0
            for comm_name in user_commission_name:
                sheet.write(row, col + 2, comm_name, format1)
                row += 1

            row = 5
            col = 0
            for total in total_list:
                sheet.write(row, col + 3, round(total, 2), format2)
                row += 1

            row = 5
            col = 0
            for comm in commission_list:
                sheet.write(row, col + 4, round(comm, 2), format2)
                row += 1
            ...
            sheet.write('E2', 'Date From: ' + str(data['date_from']), date_format)
            sheet.write('F2', 'Date To: ' + str(data['date_to']), date_format)
            ...
            row = 5
            col = 0
            for sp_name in commission_salesperson:
                sheet.write(row, col + 2, sp_name, format1)
                row += 1

            row = 5
            col = 0
            for comm_name in commission_name:
                sheet.write(row, col + 3, comm_name, format1)
                row += 1

            row = 5
            col = 0
            for comm_total in commission_total:
                sheet.write(row, col + 4, round(comm_total, 2), format2)
                row += 1

            row = 5
            col = 0
            for comm in commission:
                sheet.write(row, col + 5, round(comm, 2), format2)
                row += 1
