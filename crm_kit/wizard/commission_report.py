...
class CommissionReport(models.TransientModel):
    ...
    def action_print_xls_report(self):
        ...
        for user, orders in user_sale_orders_dict.items():
            ...
            # Initialize with 0 in case no rules match
            current_total = filtered_order_lines_commission_total
            ...
            if commission_id.type == 'product':
                ...
                continue # Skip the general append below for products as it handles multiple lines

            elif commission_id.type == 'revenue' and (
                    commission_id.revenue_type == 'graduated'):
                rules = commission_id.revenue_grd_comm_ids.sorted('sequence')
                matched = False
                for rule in rules:
                    if rule.amount_from <= current_total <= rule.amount_to:
                        current_commission = (current_total * rule.graduated_commission_rate) / 100
                        matched = True
                        break
                if not matched and rules:
                    # If total exceeds highest bracket, use the last rule's rate
                    if current_total > rules[-1].amount_to:
                        current_commission = (current_total * rules[-1].graduated_commission_rate) / 100

            elif commission_id.type == 'revenue' and (
                    commission_id.revenue_type == 'straight'):
                current_commission = (current_total * commission_id.straight_commission_rate) / 100
            # For revenue types, we always append something for the user
            commission_list.append(current_commission)
            user_commission_name.append(commission_id.name)
            user_commission_salesperson.append(user.name)
        ...
        for team_user, orders in team_sale_orders_dict.items():
            ...
            if commissions_id:
                ...
                if commissions_id.type == 'product':
                ...
                elif commissions_id.type == 'revenue' and (
                        commissions_id.revenue_type == 'graduated'):
                    rules = commissions_id.revenue_grd_comm_ids.sorted('amount_from')
                    matched = False
                    for rule in rules:
                        if rule.amount_from <= current_total <= rule.amount_to:
                            current_commission = (current_total * rule.graduated_commission_rate) / 100
                    ...
                    current_commission = (current_total * commissions_id.straight_commission_rate) / 100
                commission.append(current_commission)
                commission_name.append(commissions_id.name)
                commission_salesperson.append(team_user.name)
                commission_sales_team.append(team_user.sale_team_id.name)
                commission_total.append(current_total)
    ...
    def get_xlsx_report(self, data, response):
        ...
        sheet.write('A1', "Printed Date: " + date, date_format)
        sheet.set_column(0, 5, 20)
        sheet.set_row(0, 25)
        row = 5
        col = 0
        index = 1
        if user_commission_salesperson:
            sheet.merge_range('A3:E3', 'COMMISSION PLAN REPORT', head)
            sheet.write('D4', 'Date From: ' + data['date_from'], date_format)
            sheet.write('E4', 'Date To: ' + data['date_to'], date_format)

            sheet.write('B5', 'Sale Persons', heading)
            sheet.write('C5', 'Commission Plan Name', heading)
            sheet.write('D5', 'Total Revenue', heading)
            sheet.write('E5', 'Commission Amount', heading)

            row = 6
            for person_name in user_commission_salesperson:
                sheet.write(row, col + 0, index, format2)
                sheet.write(row, col + 1, person_name, format1)
                row += 1
                index += 1

            row_current = 6
            for plan_name in user_commission_name:
                sheet.write(row_current, col + 2, plan_name, format1)
                row_current += 1

            row_current = 6
            for total_val in total_list:
                sheet.write(row_current, col + 3, round(total_val, 2), format2)
                row_current += 1

            row_current = 6
            for comm_val in commission_list:
                sheet.write(row_current, col + 4, round(comm_val, 2), format2)
                row_current += 1

            sheet.write(row + 1, col + 2, 'Total', format3)
            sheet.write(row + 1, col + 3, round(sum(total_list) if total_list else 0, 2), format2)
            sheet.write(row + 1, col + 4, round(sum(commission_list) if commission_list else 0, 2),
                        format2)
            row += 3 # Move down for potential next section

        if commission_sales_team:
            sheet.merge_range(row, col + 0, row, col + 5, 'COMMISSION PLAN REPORT (TEAMS)', head)
            sheet.write(row + 1, col + 4, 'Date From: ' + data['date_from'], date_format)
            sheet.write(row + 1, col + 5, 'Date To: ' + data['date_to'], date_format)

            sheet.write(row + 3, col + 0, 'No.', heading)
            sheet.write(row + 3, col + 1, 'Sales Teams', heading)
            sheet.write(row + 3, col + 2, 'Sales Person', heading)
            sheet.write(row + 3, col + 3, 'Commission Plan Name', heading)
            sheet.write(row + 3, col + 4, 'Total Revenue', heading)
            sheet.write(row + 3, col + 5, 'Commission Amount', heading)

            row += 4
            start_row = row
            for team_name in commission_sales_team:
                sheet.write(row, col + 0, index, format2)
                sheet.write(row, col + 1, team_name, format1)
                row += 1
                index += 1

            team_row = start_row
            for person_name in commission_salesperson:
                sheet.write(team_row, col + 2, person_name, format1)
                team_row += 1

            team_row = start_row
            for plan_name in commission_name:
                sheet.write(team_row, col + 3, plan_name, format1)
                team_row += 1

            team_row = start_row
            for total_val in commission_total:
                sheet.write(team_row, col + 4, round(total_val, 2), format2)
                team_row += 1

            team_row = start_row
            for comm_val in commission:
                sheet.write(team_row, col + 5, round(comm_val, 2), format2)
                team_row += 1

            sheet.write(row + 1, col + 3, 'Total:', format3)
            sheet.write(row + 1, col + 4, round(sum(commission_total) if commission_total else 0, 2),
                        format2)
            sheet.write(row + 1, col + 5, round(sum(commission) if commission else 0, 2), format2)
