...
class PropertyAuction(models.Model):
    ...
    def check_start_time_end_time(self):
        """ Checks if the current time is in between the start and end time ,
        also the start time is less than the end time"""
        ...
        if not (self.start_time <= current_date_time <= self.end_time):
            raise ValidationError(_('Please provide a valid date and time; '
                                    'Start time and End time should be in between the current time.'))
