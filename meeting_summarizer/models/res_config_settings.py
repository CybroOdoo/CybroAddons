...
class ResConfigSettings(models.TransientModel):
    ...
    open_api_value = fields.Boolean(
        string="Open API Key",
        help="Enable to configure and use Open API key for meeting transcription summarization.",
        config_parameter="meeting_summarizer.open_api_value")
    open_api_key = fields.Char(
        string="Open API Value",
        help="Enter your OpenAI API key for generating meeting summaries.",
        config_parameter="meeting_summarizer.open_api_key")
    auto_mail_send = fields.Boolean(
        string="Automatically Send Mail",
        help="Enable this feature to automatically send transcription and summary emails.",
        config_parameter="meeting_summarizer.auto_mail_send")
    select_user = fields.Selection(
        selection=[
            ('host', 'Host'),
            ('all_attendees', 'All Attendees'),
        ], string="Select Recipients",
        help="Choose whether to send the meeting summary to the host only or to all attendees.",
        config_parameter="meeting_summarizer.select_user")
