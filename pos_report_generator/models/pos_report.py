...
#    Author: Vyshnav PP (odoo@cybrosys.com)
#
...
class PosReportGenerator(models.Model):
    ...
    def pos_report(self, option):
        ...
        data = {
            ...
            'report_type_name': dict(report_values._fields['report_type'].selection).get(report_values.report_type),
            'model': self,
        }
    ...
    def _get_report_sub_lines(self, data):
        ...
        if data.get('report_type') == 'report_by_order':
            query = '''
                    ...
                    l.general_customer_note,l.user_id,res_partner.name,l.name as shop,
            ...
            query += ("group by l.user_id,res_users.partner_id,"
                      ...
                      "l.name,l.amount_total,l.general_customer_note,l.id")
            self.env.cr.execute(query)
            report_by_order = self.env.cr.dictfetchall()
            report_sub_lines.append(report_by_order)
        elif data.get('report_type') == 'report_by_order_detail':
            query = '''
            select l.name,l.date_order,l.partner_id,l.amount_total,l.general_customer_note,
            ...
            query += ("group by l.user_id,res_users.partner_id,"
                      ...
                      "l.general_customer_note,pos_order_line.full_product_name,"
                      "pos_order_line.price_unit,pos_order_line.price_subtotal,"
                      "pos_order_line.price_subtotal_incl,"
                      "pos_order_line.product_id,product_product.default_code")
            self.env.cr.execute(query)
            report_by_order_details = self.env.cr.dictfetchall()
            ...
            self.env.cr.execute(query)
            report_by_product = self.env.cr.dictfetchall()
            ...
            self.env.cr.execute(query)
            report_by_categories = self.env.cr.dictfetchall()
           ...
           '''
            ...
            self.env.cr.execute(query)
            report_by_salesman = self.env.cr.dictfetchall()
           ...
           left join pos_config on pos_session.config_id = pos_config.id
            ...
            query += ("group by pos_payment_method.name,pos_session.name,"
                      "pos_config.name")
            self.env.cr.execute(query)
            report_by_payment = self.env.cr.dictfetchall()
    ...
    def _get_report_total_value(self, data, report):
        ...
        if data.get('report_type') == 'report_by_order':
            ...
            self.env.cr.execute('''
                        ...
                        ''')
            report_by_order_detail = self.env.cr.dictfetchall()
    ...
    def get_pos_xlsx_report(self, data, response, report_data):
        ...
        if filters.get('report_type') == 'report_by_order':
            sheet.merge_range('B5:D5', 'Report Type: ' +
                              filters.get('report_type_name'), txt_l)
            ...
            for rec_data in report_data_main:
                ...
                sheet.write(row, col + 7, rec_data['general_customer_note'], txt_l)
