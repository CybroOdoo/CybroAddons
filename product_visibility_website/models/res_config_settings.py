...
class ResConfigSettings(models.TransientModel):
    ...
    is_product_visibility_guest_user = fields.Boolean(
    ...
    is_product_visibility_portal_user = fields.Boolean(
    ...
    def set_values(self):
        ...
        self.env['ir.config_parameter'].sudo().set_param(
            ...
            self.is_product_visibility_guest_user)
        self.env['ir.config_parameter'].sudo().set_param(
            'is_product_visibility_portal_user',
            self.is_product_visibility_portal_user)
        self.env['ir.config_parameter'].sudo().set_param('filter_mode',
                                                         self.filter_mode)
        self.env['ir.config_parameter'].sudo().set_param(
            'filter_mode_portal', self.filter_mode_portal)
        if not self.is_product_visibility_guest_user:
            self.available_cat_for_guest_ids = None
            self.available_products_for_guest_ids = None
            self.env['ir.config_parameter'].sudo().set_param('filter_mode',
                                                             'product_only')
        if not self.is_product_visibility_portal_user:
    ...
    def get_values(self):
        ...
        is_product_visibility_guest_user = self.env[
            'ir.config_parameter'].sudo().get_param(
            'is_product_visibility_guest_user')
        is_product_visibility_portal_user = self.env[
            'ir.config_parameter'].sudo().get_param(
            'is_product_visibility_portal_user')
        ...
        available_cat_for_portal_ids = literal_eval(
            self.env['ir.config_parameter'].sudo().get_param(
                ...
                'False')) or []
        res.update(
            is_product_visibility_guest_user=is_product_visibility_guest_user,
            is_product_visibility_portal_user=is_product_visibility_portal_user,
