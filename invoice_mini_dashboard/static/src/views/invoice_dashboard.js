...
export class AccountDashboard extends Component {
    ...
    setSearchContext(ev) {
        ...
        // If the user is in a specific view (e.g. Invoices), ensure the Invoices filter is active 
        // when clicking dashboard buttons, even if they previously cleared it.
        ...
        if (defaultMoveType) {
            const typeFilter = this.env.searchModel.getSearchItems((item) => {
                ...
                // Look for domain that explicitly filters by the current default_move_type
