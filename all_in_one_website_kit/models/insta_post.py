...
class InstaPost(models.Model):
    ...
    def action_update_post(self, access_token):
        ...
        if access_token.startswith('IGAA'):
             # Basic Display API
            url = ('https://graph.instagram.com/%s?fields=id,caption,media_type,media_url,permalink,thumbnail_url,timestamp,username&access_token=%s' % (
                self.name, access_token))
        else:
            # Graph API
            url = ('https://graph.facebook.com/v19.0/%s?fields=id,caption,' +
