...
class WebsiteSale(WebsiteSale):
    ...
    def product_barcode(self, **kwargs):
        """ checking the is scanned or not  and passes the corresponding
            values"""
        _logger.info(f"Received barcode request: {kwargs.get('last_code')}")
        barcode_product = request.env['product.product'].search(
            [('barcode', '=', kwargs.get('last_code'))], limit=1)
        if barcode_product:
            return {
                'type': 'ir.actions.act_url',
                'url': '/shop/%s' % request.env['ir.http']._slug(barcode_product.product_tmpl_id)
            }
        else:
            return False
