...
class ReturnCustomerPortal(CustomerPortal):
    ...
    def geo_changer(self, coordinates):
        ...
        geolocator = Nominatim(user_agent="Odoo_AllInOneWebsiteKit_Cybrosys")
    ...
    def geo_location(self, address):
        ...
        """
        locator = Nominatim(user_agent="Odoo_AllInOneWebsiteKit_Cybrosys")
        try:
    ...
    @http.route('/portal/dashboard/data', type='json', auth="user", website=True)
    def portal_dashboard_data(self):
        """Returns data for the portal dashboard graphs"""
        user = request.env.user
        base_domain = [('user_id', '=', user.id)] if not user.has_group('base.group_portal') else [('message_partner_ids', 'child_of', [user.partner_id.id])]
