...
class MCPConnector:
    ...
    # Class-level cache: tracks (endpoint, api_key, platform) triplets that have
    # already completed the MCP initialize handshake — one session per platform.
    _initialized_endpoints: set = set()

    # Maps platform key → human-readable clientInfo name sent in initialize
    _PLATFORM_CLIENT_NAMES = {
        'telegram': 'Odoo Telegram Bot',
        'whatsapp': 'Odoo WhatsApp Bot',
        'discord':  'Odoo Discord Bot',
        'web':      'Odoo Web Chat',
        'bot':      'Odoo Bot Gateway',
    }

    def __init__(self, env, platform: str = 'bot'):
        """
        Initialise the connector.

        Args:
            env: Odoo Environment.
            platform: Source platform key — 'telegram', 'whatsapp', 'discord',
                      'web', or 'bot' (generic fallback). Used as the
                      X-Odoo-MCP-Source header so every platform gets its own
                      ai.session record and tool log entry.
        """
        self.env = env
        self.platform = platform
        base_url = env['ir.config_parameter'].sudo().get_str(
            'web.base.url', 'http://localhost:8069'
        ).rstrip('/')
        self.endpoint = f'{base_url}/mcp_gateway'
        self.api_key = env['ir.config_parameter'].sudo().get_str(
            'bot_gateway.mcp_api_key', ''
        )

    def _ensure_initialized(self) -> None:
        """
        Send the MCP initialize handshake on first use per (endpoint, api_key, platform)
        so each bot platform (Telegram, WhatsApp, Discord, Web) gets its own
        ai.session record in the dashboard — identifiable by mcp_source.
        """
        cache_key = (self.endpoint, self.api_key, self.platform)
        if cache_key in MCPConnector._initialized_endpoints:
            return
        if not self.api_key:
            return
        client_name = self._PLATFORM_CLIENT_NAMES.get(self.platform, 'Odoo Bot Gateway')
        payload = {
            'jsonrpc': '2.0',
            'method': 'initialize',
            'id': 0,
        ...
        try:
            resp = requests.post(
                ...
                headers={
                    ...
                    'X-Odoo-MCP-Source': self.platform,
                },
                timeout=15,
            )
            if resp.status_code == 200:
                MCPConnector._initialized_endpoints.add(cache_key)
                _logger.info(
                    'MCPConnector: session initialized | platform=%s endpoint=%s',
                    self.platform, self.endpoint,
                )
            else:
                _logger.warning(
                    'MCPConnector: initialize returned HTTP %s for platform=%s',
                    resp.status_code, self.platform,
                )
        except Exception as exc:
            _logger.warning(
                'MCPConnector: initialize handshake failed | platform=%s error=%s',
                self.platform, exc,
            )
    ...
    def call_tool(self, tool_name: str, arguments: dict, request_id: int = 1) -> str:
        ...
        # Ensure the bot has its own initialized MCP session before calling any tool
...
class BotGatewayService:
    ...
    def process(self, bot_message: dict) -> dict:
        ...
        raw_result = MCPConnector(self.env, platform=platform).call_tool(tool_name, tool_args)
