...
class MCPController(http.Controller):
    ...
    def _mcp_resources_read(self, user, params: dict) -> dict:
        ...
        # Coarse allow-list gate (same rules as the built-in read tool)
        ...
        if tool_model._allowlist_enforced() and not \
            ...
            raise Exception(
                f"Reading model '{model_name}' via MCP resources is not allowed. An "
                f"administrator can enable it under MCP Gateway → Configuration "
                f"→ Tool Access Rules."
            )
