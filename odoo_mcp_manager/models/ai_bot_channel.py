...
class AiBotChannel(models.Model):
    ...
    def _compute_urls(self) -> None:
        ...
        secret = params.get_str('bot_gateway.webhook_secret', '')
        for rec in self:
            base_url = (rec.custom_webhook_base or '').rstrip('/') or \
                       params.get_str('web.base.url', '').rstrip('/')
    ...
    def _ensure_webhook_secret(self) -> str:
        ...
        if not secret:
            ...
            params.set_str('bot_gateway.webhook_secret', secret)
    ...
    def _base_url(self, require_https: bool = False) -> str:
        ...
        url = (self.custom_webhook_base or '').rstrip('/') or \
              self.env['ir.config_parameter'].sudo().get_str('web.base.url', '').rstrip('/')
        if not url:
            raise UserError(_(
