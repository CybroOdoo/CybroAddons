                    ...
                    for file in response.json().get('value'):
                        if list(file.keys())[
                            ...
                            onedrive_dict[file['name']] = file[
                                ...
                                datetime.strptime(
                                    file['createdDateTime'].replace('Z', '.000000Z') if '.' not in file['createdDateTime'] else file['createdDateTime'],
                ...
                try:
                    ...
                    for file_data in response.json().get("files", []):
                        gdrive_dict[file_data.get("name")] = file_data.get(
                            ...
                            datetime.strptime(file_data.get("createdTime").replace('Z', '.000000Z') if '.' not in file_data.get("createdTime") else file_data.get("createdTime"),
                                              "%Y-%m-%dT%H:%M:%S.%fZ").strftime(
                                "%Y-%m-%d %H:%M:%S")
