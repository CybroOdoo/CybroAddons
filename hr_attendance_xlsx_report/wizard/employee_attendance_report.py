...
class HrAttendanceReport(models.TransientModel):
    ...
    def get_xlsx_report(self, data, response):
        ...
        if not data['employee_ids']:
            ...
            GROUP BY date(check_in), hr_e.name"""
            self.env.cr.execute(query)
            docs = self.env.cr.dictfetchall()
        else:
            query = """select hr_e.name,date(hr_at.check_in),
            SUM(hr_at.worked_hours) from hr_attendance hr_at LEFT JOIN
            hr_employee hr_e ON hr_at.employee_id = hr_e.id  WHERE
            hr_e.id in (%s) GROUP BY date(check_in), hr_e.name""" % (', '.join(
                str(empolyee_id) for empolyee_id in data['employee_ids']))
            self.env.cr.execute(query)
            docs = self.env.cr.dictfetchall()
        output = io.BytesIO()
        ...
        sheet.merge_range('B13:B14', 'Sl.No', border)
        sheet.merge_range('C13:C14', 'Employee', border)
        row = 12
        col = 2
