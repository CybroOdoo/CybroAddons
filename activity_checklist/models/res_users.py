...
class ResUsers(models.Model):
    ...
    def systray_get_activities(self):
        """Override to handle custom activity states (done, cancel)"""
        search_limit = int(self.env['ir.config_parameter'].sudo().get_param(
            'mail.activity.systray.limit', 1000))

        # CHANGE: Exclude done and cancelled activities from systray
        activities = self.env["mail.activity"].search(
            [("user_id", "=", self.env.uid), ('state', 'not in', ['done', 'cancel'])],
            order='id desc',
            limit=search_limit
        )
        ...
        for model_name, activities_by_record in activities_by_record_by_model_name.items():
            ...
            if has_model_access_right:
                allowed_records = existing._filter_access_rules('read')
            else:
        ...
        model_ids = [self.env["ir.model"]._get_id(name) for name in
                     activities_by_model_name]
        user_activities = {}
