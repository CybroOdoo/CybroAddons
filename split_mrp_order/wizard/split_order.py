...
class SplitOrder(models.TransientModel):
    ...
    is_works = fields.Boolean(string="Works",
                    ...
                    if value == 0:
                        ...
                        for val in range(int(split)):
                            ...
                            for rec in order.move_raw_ids:
                                by_product_split = (
                                    rec.product_uom_qty // no_split
                                )
                                by_product_reminder = (
                                    rec.product_uom_qty % no_split
                                )
                                by_product_value = (
                                    rec.product_uom_qty - by_product_reminder
                                )
                                by_product_split_qty = (
                                    by_product_value // by_product_split
                                )
                                move_raw_ids_list.append({
        ...
        if qty > 1 and manually:
            ...
            if total<qty:
                raise ValidationError(
                    "The total of split must be equal to the order quantity"
                )
            if validation:
