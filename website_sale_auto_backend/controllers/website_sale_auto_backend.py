...
class WebsiteSalePayment(WebsiteSale):
    ...
    def shop_payment_validate(self, sale_order_id=None, **post):
        ...
        if website_order_configuration == 'confirm_order':
            ...
            # Flush pending ORM recomputes (qty_invoiced → qty_to_invoice)
            # that were triggered by _create_invoices linking invoice lines
            # to SO lines, then re-apply the order-based qty_to_invoice
            # override.  Without this, delivery-policy products get a
            # negative qty_to_invoice (0 − qty_invoiced) and the stored
            # invoice_status stays "to invoice" instead of "invoiced".
            order.env.flush_all()
            order._force_lines_to_invoice_policy_order()
            ...
            for invoice in invoices:
                invoice._portal_ensure_token()
                invoice.action_post()
            # Flush pending ORM recomputes then re-apply order-based
            # qty_to_invoice so invoice_status correctly becomes "invoiced"
            order.env.flush_all()
            order._force_lines_to_invoice_policy_order()
