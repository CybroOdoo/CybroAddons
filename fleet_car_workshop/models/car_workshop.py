...
class CarWorkshop(models.Model):
    ...
    def _read_group_stage_ids(self, stages, domain):
        ...
        return stages.browse(stages.sudo()._search([]))
    ...
    vehicle_id = fields.Many2one('vehicle.details', string='Vehicle',
                                 index=True, tracking=True, change_default=True,store=True,
    ...
    kanban_state = fields.Selection(
        [('normal', 'In Progress'), ('done', 'Ready for next stage'),
         ('blocked', 'Blocked'), ('state_done', 'Done')], string='Kanban State',
    ...
    company_id = fields.Many2one('res.company', string='Company',
                                 help='Company relat to the workshop',
                                 required=True,
    ...
    @api.depends('planned_work_ids.is_invoiced', 'planned_work_ids.is_completed', 'materials_ids.is_invoiced')
    def _compute_state(self):
        for record in self:
            has_invoiced = any(w.is_invoiced for w in record.planned_work_ids) or any(m.is_invoiced for m in record.materials_ids)
            has_uninvoiced_ready = any(w.is_completed and not w.is_invoiced for w in record.planned_work_ids) or any(not m.is_invoiced for m in record.materials_ids)
            has_uninvoiced_uncompleted = any(not w.is_completed for w in record.planned_work_ids)
            
            if not has_invoiced and not has_uninvoiced_ready:
                record.state = 'toinvoice'
            elif has_invoiced and not has_uninvoiced_ready and not has_uninvoiced_uncompleted:
                record.state = 'invoiced'
            elif has_invoiced and (has_uninvoiced_ready or has_uninvoiced_uncompleted):
    ...
    def _onchange_vehicle_id(self):
        """Automatically fill the customer when a vehicle is selected."""
    ...
    def action_create_invoices(self):
        ...
        has_something_to_invoice = False
        for records in self.planned_work_ids:
            if records.planned_work_id:
                if records.is_completed and not records.is_invoiced:
                    inv_line_data = (0, 0, {
                        'name': records.planned_work_id,
                        'account_id': journal.default_account_id.id,
                        'price_unit': records.work_cost,
                        'quantity': 1,
                    })
                    invoice_line_ids.append(inv_line_data)
                    records.is_invoiced = True
                    has_something_to_invoice = True
        for records in self.materials_ids:
            if records.material_product_id.id and not records.is_invoiced:
                income_account = records.material_product_id.property_account_income_id.id
                inv_line_data = (0, 0, {
                    'name': records.material_product_id.name,
                    'account_id': income_account if income_account else journal.default_account_id.id,
        ...
        if not has_something_to_invoice:
            raise UserError(_('Nothing to invoice, all completed works are already invoiced.'))
        inv_data.update({
    ...
    def _compute_effective_hour(self):
        ...
        for hour in self:
            ...
            hour.effective_hour = effective_hour
    ...
    def _compute_remaining_hour(self):
        ...
        for hour in self:
            ...
            hour.remaining_hour = remaining_hour - hour.effective_hour
