...
class CarWorkshop(models.Model):
    ...
    kanban_state = fields.Selection(
        [('normal', 'In Progress'), ('done', 'Ready for next stage'),
         ('done_stage', 'Done'), ('blocked', 'Blocked')], string='Kanban State',
    ...
    @api.depends('planned_work_ids.is_completed', 'planned_work_ids.is_invoiced', 'materials_ids.is_invoiced')
    def _compute_state(self):
        for rec in self:
            if not rec.planned_work_ids and not rec.materials_ids:
                rec.state = 'toinvoice'
                continue
            
            uncompleted_works = any(not w.is_completed for w in rec.planned_work_ids)
            uninvoiced_completed_works = any(w.is_completed and not w.is_invoiced for w in rec.planned_work_ids)
            uninvoiced_materials = any(not m.is_invoiced for m in rec.materials_ids)
            
            invoiced_works = any(w.is_invoiced for w in rec.planned_work_ids)
            invoiced_materials = any(m.is_invoiced for m in rec.materials_ids)
            has_invoiced = invoiced_works or invoiced_materials
            
            if uninvoiced_completed_works or uninvoiced_materials:
                if has_invoiced:
                    rec.state = 'partially_invoiced'
                else:
                    rec.state = 'toinvoice'
            else:
    ...
    def action_create_invoices(self):
        ...
        for records in self.materials_ids:
            if records.material_product_id.id and not records.is_invoiced:
                ...
                invoice_line_ids.append(inv_line_data)
                material_records.append(records)
        
        if not invoice_line_ids:
            raise UserError(_('No completed works or materials to invoice.'))
        ...
        # Link invoice lines back to the worksheet
        created_lines = inv_id.invoice_line_ids.sorted(lambda l: l.id)
        line_idx = 0
        for records in work_records:
            records.invoice_line_ids = [(4, created_lines[line_idx].id)]
            line_idx += 1
            
        for records in material_records:
            records.invoice_line_ids = [(4, created_lines[line_idx].id)]
            line_idx += 1
