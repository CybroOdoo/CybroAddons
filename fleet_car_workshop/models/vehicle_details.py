...
class VehicleDetails(models.Model):
    ...
    archive = fields.Boolean('Archive', compute='_compute_archive',
                             inverse='_inverse_archive', store=True,
                             default=False, help='Enables will archive this record')

    @api.depends('active')
    def _compute_archive(self):
        for rec in self:
            rec.archive = not rec.active

    def _inverse_archive(self):
        for rec in self:
            rec.active = not rec.archive
    ...
    def _get_visibility_selection_id(self):
