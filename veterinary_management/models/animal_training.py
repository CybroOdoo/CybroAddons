...
class AnimalTraining(models.Model):
    """
    Model for storing training details in veterinary management.
    """
    ...
    training_no = fields.Char(
        string="Training No",
        default=lambda self: self.env['ir.sequence'].next_by_code('training.sequence') or _('New'),
        copy=False,
        readonly=True,
        tracking=True,
    ...
    def create(self, vals_list):
        ...
        if vals_list.get('training_no', 'New') == 'New':
            vals_list['training_no'] = self.env['ir.sequence'].next_by_code('training.sequence') or 'New'
        return super().create(vals_list)

    def action_create_invoice(self):
        """Create an invoice for the training appointment."""
        for record in self:
            product = self.env['product.product'].search([('name', '=', 'Training')])
