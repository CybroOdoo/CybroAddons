...
class MrpUnbuildLine(models.Model):
    """Component lines to be managed manually in each Unbuild Orders"""
    _name = 'mrp.unbuild.line'
    _description = 'Unbuild Line'

    unbuild_id = fields.Many2one('mrp.unbuild', string='Unbuild Order', ondelete='cascade', help='Unbuild Order')
    ...
    qty_on_hand = fields.Float(string='On Hand', related='product_id.qty_available',
                               help='On Hand quantity for the current product')
