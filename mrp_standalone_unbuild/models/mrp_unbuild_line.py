...
class MrpUnbuildLine(models.Model):
    """Component lines manageable in Unbuild Orders"""
    ...
    product_qty = fields.Float(string='Quantity', default=1.0, digits='Product Unit')
    ...
    qty_on_hand = fields.Float(string='On Hand', related='product_id.qty_available', readonly=True,
                               help="Current onhand stock of the current product in line")
    ...
    def _compute_uom_id(self):
        """Computing the UOM in component lines"""
        self.uom_id = False
        for line in self:
            if line.product_id:
