...
class ResPartner(models.Model):
    ...
    # @api.depends('partner_relation_ids.reverse_relation_type_id')
    # def _compute_relation_info(self):
    #     for partner in self:
    #         # Get unique names from all outgoing relationships
    #         relation_types = partner.partner_relation_ids.mapped(
    #             'reverse_relation_type_id.name')
    #
    #         # 1. Set the aggregated names
    #         partner.relation_type_names = ', '.join(
    #             relation_types) if relation_types else False
    #
    #         # 2. Set the boolean flag
    #         partner.is_related = bool(relation_types)
