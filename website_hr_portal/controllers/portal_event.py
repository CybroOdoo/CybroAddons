...
class WebsiteHrPortalEvents(http.Controller):
    ...
    def portal_my_events(self, sortby=None, search="", search_in="All"):
        ...
        event = request.env['event.event'].sudo().search(search_domain,
                                                         order=order)
        return http.request.render('website_hr_portal.portal_employee_events',
                                   {'event': event,
