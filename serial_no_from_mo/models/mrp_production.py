...
class MrpProduction(models.Model):
    ...
    def action_confirm(self):
        ...
        if parms.get_param('serial_no_from_mo.is_serial_selection') == 'True':
            for rec in self:
                if rec.product_id.tracking in ['serial', 'lot']:
                    if parms.get_param(
                            'serial_no_from_mo.serial_selection') == 'global':
                        digit = int(parms.get_param('serial_no_from_mo.digit') or 0)
                        prefix = parms.get_param('serial_no_from_mo.prefix') or ''
                        ...
                        if seq:
                            seq.write({
                                'prefix': rec.product_id.product_tmpl_id.prefix or '',
                                'padding': rec.product_id.product_tmpl_id.digit or 0,
                            })
                        if not seq:
                            self.env['ir.sequence'].create({
                                'name': 'Mrp Production',
                    ...
                    rec.lot_producing_ids = serial_id
