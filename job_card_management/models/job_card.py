...
class JobCard(models.Model):
    """Model for Job card to store the details of job and related project"""
    _name = "job.card"
    ...
    card_name = fields.Char('Card Name', required=True, store=True, help='Card name')
    sequence = fields.Char('Name', store=True, help='Sequence')
    name = fields.Char('Name', store=True, help='Name')

    start_date = fields.Date('Starting Date', required=True, help='Start date')
    end_date = fields.Date('Ending Date', required=False, help='End date')
    project_id = fields.Many2one('project.project',
                                 string='Project', required=True, help='Project')
    user_id = fields.Many2one('res.users', string='Assigned To', required=True, help='User')

    quality_checklist_id = fields.Many2many('quality.check.list',
                                            string='Quality check list',help='Quality check list')
    deadline = fields.Date(string='Deadline', help='deadline')

    team_id = fields.Many2one('workshop.team', string='Team', help='Team')
    ...
    instruction_ids = fields.One2many('job.card.instruction', 'job_card_id', string='Instruction IDs', help='Instruction IDs')
    instruction_count = fields.Integer(default=1, string='Instruction count', help='deadline')
    job_cost_sheet_ids = fields.One2many('job.cost.sheet', 'job_card_id', string='Job Cost sheet IDs', help='Job Cost sheet IDs')
    cost_sheet_amount = fields.Monetary(compute='_compute_cost_sheet_amount',
                                        store=True, string='Cost sheet amount', help='Cost sheet amount')
    job_cost_sheet_untaxed_amount = fields.Monetary(
        compute='_compute_cost_sheet_amount', store=True, string='cost sheet untaxed amount', help='cost sheet untaxed amount')
    job_card_timesheet_ids = fields.One2many('job.card.timesheet',
                                             'job_card_id', string='job card time sheet ids', help='job card time sheet ids')
    ...
    total_hours = fields.Float('Total Working Hour', store=True,
                               compute="_compute_hour", help='Total Working Hour')
    ...
    mpr_count = fields.Integer(compute='compute_count', string='MRP Count', help='MRP Count')
    progress = fields.Float(string='Progress', help="progress of this task")
...
class JobCardInstruction(models.Model):
    ...
    job_card_id = fields.Many2one('job.card', string='Job Card', help='Job Card')
    name = fields.Char('Name', compute='_compute_name', store=True, help='Name')
    start_date = fields.Datetime('Starting Date', required=True, help='Starting Date')
    end_date = fields.Datetime('Ending Date', required=False, help='Ending Date')
    user_id = fields.Many2one('res.users', string='Assigned To', required=True, help='Assigned To')
    instruction = fields.Char(help='Instructions', required=True, string='Instruction')
    notes = fields.Char(string='Notes', help='notes for this instruction')
    state = fields.Selection(
        [('to_do', 'To Do'), ('in_progress', 'In progress'), ('done', 'Done')],
        default='to_do', string='State')
...
class CostSheet(models.Model):
    ...
    type = fields.Selection([('material', 'Material'), ('labour', 'Labour'),
                            ...
                            help='Type of product', string='Type')
    job_card_id = fields.Many2one('job.card', string='Job Card', help='Job card')
    product_id = fields.Many2one('product.product', required=True, string='Product', help='Product')
    quantity = fields.Float('Quantity', default=1, help='Quantity')
    ...
    def onchange_price(self):
        for rec in self:
            if rec.quantity or rec.unit_price:
                rec.untaxed_mount = rec.quantity * rec.unit_price

class JobCardTimesheet(models.Model):
    _name = 'job.card.timesheet'
    _description = 'Job Card TimeSheet'

    job_card_id = fields.Many2one('job.card',string='Job Card', default=lambda
        self: self._default_job_card_id(), help='Job Card')
    name = fields.Char('Instruction Name', store=True, help='Name')
    instruction_id = fields.Many2one('job.card.instruction',string='Instruction', required=True, help='Instruction ID')
    description = fields.Char('Description', help='Description')
    leader_id = fields.Many2one('hr.employee', required=True,string='Leader',
    ...
    worker_id = fields.Many2one('hr.employee', required=True,
                                ...
                                string="Workers")
    date = fields.Date('Date', default=fields.Date.today(), help='date')
    time = fields.Float('Time', hep='Time')
    ...
    def create(self, vals_list):
        """Create details in account.analytic.line model"""
