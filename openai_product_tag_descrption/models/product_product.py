...
class ProductProduct(models.Model):
    """ Inherit product model to add tag field and function create tag
        and description"""
    _inherit = 'product.product'

    product_tag_ids = fields.Many2many('product.tag',
                                       related='product_tmpl_id.product_tag_ids',
                                       string="Product Tags", readonly=False,
                                       help="Tags for the product")
    ...
    def action_create_tag_internal_note(self):
        ...
        if not api_key:
            raise ValidationError(
                _("Provide an API key in the settings"))
