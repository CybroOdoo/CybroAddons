...
publicWidget.registry.OdoNovaTheme = publicWidget.Widget.extend({
    ...
    /** ── 7. Hide Contact Us when OdoNova theme is active ────────────────── */
    _initHideContactUs() {
        if (this.editableMode) return; // Keep standard DOM for editor

        const isOdoNovaTheme = this.el.querySelector('[data-odonova-header]');
        if (!isOdoNovaTheme) return;

        const links = this.el.querySelectorAll('.navbar a[href="/contactus"]:not(.odonova-book-cta), .nav-item a[href="/contactus"]:not(.odonova-book-cta)');
        links.forEach(link => {
