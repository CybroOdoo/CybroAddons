...
class PetSittingSchedule(models.TransientModel):
    ...
    service_ids = fields.One2many('pet.service', 'pet_service_id',
                                  string='Service', help='Pet services')

    def assign_sittings(self):
        """ Assigning pet sittings from sale order """
        sale_order = self.env['sale.order'].browse(self.reference_id.id)
        self.env['sitting.schedule'].create({
            'name': 'Meeting',
            'number_id': sale_order.id,
...
class AttendeesPet(models.TransientModel):
    ...
    attendee_id = fields.Many2one('pet.sitting.schedule',
                                  string='Sitting Schedule',
                                  help='Pet Sitting Schedule')
    attendees_id = fields.Many2one('res.partner', required=True,
                                   string='Attendees',
                                   help='Attendees of the schedule')
...
class PetInfo(models.TransientModel):
    ...
    pet_num = fields.Char(related='num.pet_seq', string='Reference',
                          help='Pet reference')
    name = fields.Char(related='num.name', string='Name', help='Name of the Pet')
...
class PetService(models.TransientModel):
    ...
    cost = fields.Float(related='name.standard_price', string='cost',
                        help='Cost of the service')
    sale_price = fields.Float(related='name.list_price', string='Price',
