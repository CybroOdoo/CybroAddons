...
export class BankBook extends Component {
    static template = 'bnk_b_template_new';
    ...
    async load_data() {
        ...
        let currency;
        var action_title = this.props?.action?.display_name || "Bank Book";
        try {
            this.state.data = await this.orm.call("bank.book.report", "view_report", []);
            if (this.state.data) {
            ...
            if (move_line_list && this.state.data) {
                for (const key of move_line_list) {
                    if (Array.isArray(this.state.data[key])) {
                        for (const line of this.state.data[key]) {
    ...
    async applyFilter(val, ev, is_delete = false) {
        ...
        let filtered_data = await this.orm.call("bank.book.report", "get_filter_values", [this.state.selected_partner, this.state.date_range, this.state.options, this.state.selected_account_list]);
