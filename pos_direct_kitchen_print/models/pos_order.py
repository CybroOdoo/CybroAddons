...
class PosOrder(models.Model):
    ...
    def _prepare_kitchen_tickets(self, order_data, printers):
        ...
        for printer in printers:
            ...
            for line in order_data.get("lines", []):
                ...
                # Check if any printer category matches the product's POS categories
                if any(cat.id in product.pos_categ_ids.ids for cat in printer.category_ids):
    ...
    def _send_to_printnode(self, printer, lines, order_name, order_data=None):
        ...
        try:
            ...
            # Use the mapped PrintNode printer ID
            printer_id = int(printer.printer_id.id_of_printer)

            _logger.info("Sending kitchen job to PrintNode printer %s", printer_id)
            gateway.PrintJob(
                printer=printer_id,
                job_type='raw',
                title="Kitchen Order",
            ...
            _logger.error("Failed to send print job to PrintNode: %s", str(e))
