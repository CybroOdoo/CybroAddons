...
class ProductTemplate(models.Model):
    ...
    def _onchange_digits_prefix(self):
    ...
    def _number_next_actual(self):
        ...
        if self.is_auto_generate:
            ...
            # Use product specific settings if set, otherwise fallback to global config
            prefix = self.prefix or self.env['ir.config_parameter'].sudo().get_param('auto_generate_lot_number.prefix', default='')
            digits = self.digits or int(self.env['ir.config_parameter'].sudo().get_param('auto_generate_lot_number.digits', default=0))
            if (len(str(digits)) == len(str(self.number_next)) and
                    self.check_string_for_nine(str(self.number_next))):
                digits = digits + 1
                if self.digits > 0:
                    self.digits = digits
            fill_len = digits - len(str(number_next))
            if fill_len <= 0:
                value = prefix
            else:
                padding = '0' * fill_len
                value = prefix + padding
            return str(value) + str(number_next)
    ...
    def _compute_is_auto_generate(self):
        ...
        settings page"""
        product_type = self.env['ir.config_parameter'].sudo().get_param(
        ...
        is_auto = auto_generate in ['True', '1', True]
        for rec in self:
            rec.is_auto_generate = (product_type == 'product' and is_auto)
