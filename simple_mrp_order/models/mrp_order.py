...
class MRPOrder(models.Model):
    ...
    uom_id = fields.Many2one('uom.uom', string='Product Unit of Measure',
                             ...
                             help='Unit of Measurement of the product to '
                                  'manufacture')
    uom_categ_id = fields.Many2one(related='product_id.uom_id.relative_uom_id')
    ...
    def _onchange_uom_id(self):
        """ Update the Uom for the product """
        multiple = 1
        if self.uom_id.factor < 1:
            multiple = 1
        elif self.uom_id.factor == 1:
        ...
        for line in self.bom_id.line_ids:
            val = {
                ...
                'reference': line.product_id.name,
                'company_id': self.company_id.id,
                'location_id': self.location_src_id.id,
                'location_dest_id': destination.id,
                'product_uom': line.uom_id.id,
                'state': 'draft',
                'allowed_uom_ids': line.uom_id.relative_uom_id,
            }
            self.stock_line_ids = [fields.Command.create(val)]

    @api.model
    def create(self, vals):
        """Override create to add sequence and stock line defaults for MRP Orders."""
        # Ensure vals_list is always a list of dicts
        vals_list = vals if isinstance(vals, list) else [vals]

        # Get common values (company-dependent)
        ...
        for val in vals_list:
            # Handle stock_line_ids
            stock_lines = val.get('stock_line_ids')
            if stock_lines:
                for item in stock_lines:
                    # item format: [command, id, values_dict]
                    # ensure values_dict exists
                    if isinstance(item, (list, tuple)) and len(item) >= 3 and item[2]:
                        product_id = item[2].get('product_id')
                        if product_id:
                            product = self.env['product.product'].browse(product_id)
                            item[2]['reference'] = product.name
            ...
            if not val.get('name') or val.get('name') == _("New"):
                val['name'] = self.env['ir.sequence'].next_by_code('mrp.order') or _("New")

        # Create the records
        records = super().create(vals_list)

        # Return created records (single or multiple)
        return records if isinstance(vals, list) else records[0]
    ...
    def _onchange_bom_id(self):
        ...
        self.line_ids = vals
        if self.bom_id:
            self.uom_id.relative_uom_id = self.uom_categ_id.id
    ...
    def action_done(self):
        ...
        move_id = self.env['stock.move'].create({
            ...
            'state': 'draft',
            'allowed_uom_ids': self.uom_id.relative_uom_id,
            'origin': self.name,
            'quantity': self.product_qty,
            'picked': True
        })
