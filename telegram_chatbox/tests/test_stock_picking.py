...
class TestStockPickingTelegram(TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.bot = cls.env["telegram.bot"].create({
            "name": "Picking Test Bot",
            "token": "picking-token"
        })
        cls.partner = cls.env["res.partner"].create({
            "name": "Picking Partner",
            "telegram_chat_id": "778899",
            "telegram_username": "picking_partner_username",
            "telegram_opt_in": True,
            "telegram_bot_id": cls.bot.id,
        })
        # Find warehouse/picking type to avoid constraints
        picking_type = cls.env["stock.picking.type"].search([], limit=1)
        location_src = cls.env["stock.location"].search([("usage", "=", "internal")], limit=1)
        location_dest = cls.env["stock.location"].search([("usage", "=", "customer")], limit=1)
        cls.picking = cls.env["stock.picking"].create({
            "partner_id": cls.partner.id,
            "picking_type_id": picking_type.id,
            "location_id": location_src.id or cls.env.ref("stock.stock_location_stock").id,
            "location_dest_id": location_dest.id or cls.env.ref("stock.stock_location_customers").id,
        })
    ...
    def test_send_telegram_message_validations(self):
        ...
        with self.assertRaises(ValidationError):
            self.picking.send_telegram_message("Test message", False, bot=self.bot)
    ...
    def test_send_telegram_message_success(self):
        ...
        with patch.object(type(self.env["telegram.message"]), "send_via_bot", return_value=True) as mock_send:
            res = self.picking.send_telegram_message("Valid Picking Message", False, bot=self.bot)
            
        self.assertTrue(res)
        mock_send.assert_called_once()
        
        # Verify chatter log
        messages = self.picking.message_ids
        chatter_bodies = [m.body for m in messages]
        self.assertTrue(any("Valid Picking Message" in body for body in chatter_bodies))

    def test_action_send_telegram_wizard_returns(self):
        """Test action method returns valid action and context dictionary."""
        action = self.picking.action_send_telegram()
