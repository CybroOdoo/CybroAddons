...
class TelegramTest(models.TransientModel):
    """
    Transient wizard model that allows users to draft, preview, and dispatch
    Telegram messages (with or without templates) on various documents.
    """
    _name = "telegram.test"
    ...
    def default_get(self, fields_list):
        """
        Populate default values for the Telegram message wizard.
        """
    ...
    def _compute_current_model(self):
        """
        Dynamically calculates and assigns the ir.model based on whichever active
        record (Sale, Purchase, Invoice, Picking) is linked to this wizard.
        Used to filter the relevant Telegram templates.
        """
        for rec in self:
    ...
    def _onchange_current_model(self):
        """
        Refreshes the template domain visually in the UI whenever the referenced document changes.
        Filters templates dynamically based on the current model.
        """
    ...
    def action_send(self):
        """
           Send a Telegram message from the wizard.
           This method determines the source record associated with the wizard
           (Sale Order, Purchase Order, Invoice, Delivery Order, or Partner)
           and delegates the message sending process to the corresponding
           model's `send_telegram_message()` method.
        """
        self.ensure_one()
    ...
    def _onchange_telegram_template_id(self):
        """
        Update the message text when the Telegram template changes.
        """
