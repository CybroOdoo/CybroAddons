...
class MrpProduction(models.Model):
    ...
    def action_cancel_mo(self):
        """Cancels the mo"""
        _logger.info("action_cancel_mo triggered for MO ids: %s", self.ids)
        for production in self:
            if production.cancel_inventory_moves:
                production.state = 'cancel'
                (production.move_raw_ids | production.move_finished_ids).move_line_ids.write({'state': 'cancel'})

            if production.cancel_workorder:
                production.state = 'cancel'
                production.workorder_ids.action_cancel()

    def _action_cancel(self):
        """Overriding the cancel method"""
        _logger.info("_action_cancel triggered for MO ids: %s", self.ids)
        documents_by_production = {}
        for production in self:
            documents = defaultdict(list)
            for move_raw_id in production.move_raw_ids.filtered(
                    lambda m: m.state not in ('done', 'cancel')):
                iterate_key = production._get_document_iterate_key(move_raw_id)
                if iterate_key:
                    document = self.env[
