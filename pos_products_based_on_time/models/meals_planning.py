...
class MealsPlanning(models.Model):
    ...
    def _load_pos_data_domain(self, data, config):
        session_id = False
        if isinstance(data, dict) and 'pos.session' in data and data['pos.session']:
            session_id = data['pos.session'][0].get('id')
        if session_id:
            return [('pos_ids', 'in', [session_id])]
        return [] 

    @api.model
    def _load_pos_data_fields(self, config):
        """To load fields in pos session"""
        return ['name', 'menu_product_ids', 'time_from', 'time_to', 'state', 'pos_ids']
