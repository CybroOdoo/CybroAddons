...
class QuickPurchaseOrder(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        for rec in products:
            line = (0, 0, {
               ...
               'tax_id': [Command.set(rec.taxes_id.ids)],  # tax_id is the field in quick.purchase.line
            })
    ...
    def action_create(self):
        ...
        order = self.env['purchase.order'].sudo().create({
            ...
            'picking_type_id': self.env['stock.warehouse']._get_partner_locations()[0].id,  # Get default picking type
            'order_line': [(0, 0, {
        ...
        for order in self.purchase_ids:
            order.update({
                'order_line': [(0, 0, {
                    ...
                    'product_qty': rec.product_qty,
                    'price_unit': rec.price_unit,
                    'tax_ids': [(6, 0, rec.tax_id.ids)],  # tax_ids is the field in purchase.order.line
                }) for rec in self.line_ids]
            })
