...
class ChequeTypes(models.TransientModel):
    ...
    def action_print_selected_cheque(self):
        ...
        cheque_pages = []
        if len(payments) <= 1:
            if self.cheque_format_id.is_date_remove_slashes:
            ...
            # Batch printing: Use data from payments
            for pay in payments:
                p_date = False
                if pay.payment_method_line_id.payment_method_id.name == 'Checks':
                    p_date = pay.date
                elif pay.payment_method_line_id.payment_method_id.name == 'PDC':
                    p_date = pay.effective_date
                ...
                cheque_pages.append({
                    ...
                    'amount_in_words': pay.check_amount_in_words,
                    'amount_in_digit': pay.amount,
        ...
        data = {
            ...
            'is_print_currency': self.cheque_format_id.is_print_currency,
            'currency_symbol': self.env.company.currency_id.symbol,
            'amount_digit_size': self.cheque_format_id.amount_digit_size,
            'is_print_cheque_number': self.cheque_format_id.is_print_cheque_number,
