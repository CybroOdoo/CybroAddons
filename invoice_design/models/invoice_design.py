...
class InvoiceDesign(models.Model):
    ...
    def create(self, vals_list):
        """Create invoice.design and its QWeb view with proper error handling."""
        for vals in vals_list:
            try:
                ...
                vals.update({'view_id': view.id})
            except etree.XMLSyntaxError:
                raise UserError(_('Add the template (Invoice XML) in proper format.'))
            except UserError:
                raise
