...
class TestPaymentPayPlugAcquirer(TransactionCase):
    ...
    @patch('odoo.addons.payment_payplug_acquirer.controllers.payment_payplug_acquirer.payplug.set_secret_key')
    @patch('odoo.addons.payment_payplug_acquirer.controllers.payment_payplug_acquirer.payplug.Payment.retrieve')
    def test_payplug_return(self, mock_payplug_retrieve, mock_set_secret_key):
        ...
        with patch.object(controller_module, 'request', fake_request), patch(
            ...
            mock_set_secret_key.assert_called_once_with('test_secret_key')
