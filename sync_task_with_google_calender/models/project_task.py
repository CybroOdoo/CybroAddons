...
class ProjectTask(models.Model):
    ...
    def create(self, vals):
        ...
        if task.is_add_in_gcalendar:
            task.sync_task_to_google_calendar(task)
        return task
    ...
    def sync_task_to_google_calendar(self, task):
        ...
        for task in self:
            if task.is_add_in_gcalendar and (
                    not task.partner_id or not task.date_deadline):
                required_fields = []
            ...
            event = {
                ...
                "start": {"date": self._get_google_calendar_date(
                    task.date_deadline)},
                "end": {"date": self._get_google_calendar_date(
                    task.date_deadline)},
                "extendedProperties": {
                    "private": {
    ...
    def _get_google_calendar_date(self, date_value):
        """Return a Google Calendar date string from date or datetime values."""
        if not date_value:
            return ""
