    ...
    def action_import_payment(self):
        ...
        if items:
            for item in items:
                ...
                amount = self.get_val(item, 'Amount')
                if amount:
                    vals['amount'] = amount
                date_val = self.get_val(item, 'Date')
                if date_val:
                    vals['date'] = date_val
                journal_name = self.get_val(item, 'Journal')
                if journal_name:
                    journal = self.env['account.journal'].search(
                        [('name', '=', journal_name)])
                    if journal:
                        vals['journal_id'] = journal.id
                partner_name = self.get_val(item, 'Customer/Vendor', 'Customer', 'Vendor', 'Partner')
                if partner_name:
                    ...
                    if not partner:
                        ...
                        info_msg += f"\nCreated new partner with name :{partner_name}"
                    vals['partner_id'] = partner.id
                reference = self.get_val(item, 'Reference')
                if reference:
                    vals['ref'] = reference
                payment_type = self.get_val(item, 'Payment Type')
