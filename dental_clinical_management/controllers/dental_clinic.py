...
class DentalClinic(Controller):
    ...
    def create_appointment(self, **kw):
        ...
        if len(kw.get('time_shift')) == 0:
            raise UserError(_('No appointments available for this doctor.'))
        else:
            ...
            return request.redirect(f'/success_appointment?appointment={patient_appointment.id}')
    ...
    def success_appointment(self, **kwargs):
        ...
        appointment = request.env['dental.appointment'].sudo().browse(int(kwargs.get('appointment')))
    ...
    @http.route(
        '/dental_clinic/appointment_card/<int:appointment_id>',
        type='http', auth="public", website=True
    )
    def appointment_card(self, appointment_id):
        appointment = request.env['dental.appointment'].sudo().browse(int(appointment_id))
        if not appointment.exists():
            return request.not_found()

        pdf_content, _ = request.env['ir.actions.report'].sudo()._render_qweb_pdf(
            'dental_clinical_management.action_appointment_card',
            res_ids=appointment.ids
        )
