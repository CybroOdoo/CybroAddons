...
class DentalTimeShift(models.Model):
    ...
    name = fields.Char(string='Name', readonly=True,
                       help="name of the time shifts", required=True)
    shift_type = fields.Selection(
        selection=[('morning', 'Morning'), ('day', 'Day'),
                   ('evening', 'Evening'), ('night', 'Night')],
        string="Shift Type", help="Selection field for the shift type", required=True)
    start_time = fields.Float(string="Start Time", help="start time of time slot", required=True)
