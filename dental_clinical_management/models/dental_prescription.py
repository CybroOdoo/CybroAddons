...
class DentalPrescription(models.Model):
    ...
    state = fields.Selection([('new', 'New'),
                             ...
                             default="new",
                             string="State",
                             help="state of the prescription")
    ...
    def create(self, vals_list):
        ...
        res = super(DentalPrescription, self).create(vals_list)
        return res
    ...
    def _compute_grand_total(self):
        ...
        for rec in self:
            rec.grand_total = rec.cost
            for med in rec.medicine_ids:
                rec.grand_total += med.total
...
class DentalPrescriptionLines(models.Model):
    ...
    quantity = fields.Integer(string="Quantity",
                              required=True, help="Quantity of medicine", default=1)
    frequency_id = fields.Many2one('medicine.frequency',
                                   string="Frequency",
                                   required=True, help="Frequency of medicine")
    price = fields.Float(related='medicament_id.list_price',
                         string="Price", help="Cost of medicine")
    total = fields.Float(string="Total Price", help="Total price of medicine", compute="_compute_total", store=True)
