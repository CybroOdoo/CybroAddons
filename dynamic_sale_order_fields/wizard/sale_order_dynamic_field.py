...
class SaleOrderDynamicFields(models.TransientModel):
    ...
    def get_possible_field_types(self):
        ...
        field_list = sorted(
            ...
            for name, cls in vars(fields).items()
            if isinstance(cls, type)
            and issubclass(cls, fields.Field)
            and cls is not fields.Field
        )

        excluded_fields = {'one2many', 'reference'}
        field_list = [ft for ft in field_list if ft[0] not in excluded_fields]
