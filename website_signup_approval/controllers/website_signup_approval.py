...
#    Author: Afra MP @ Cybrosys, (odoo@cybrosys.com)
#
...
class AuthSignupHome(Home):
    ...
    def web_auth_signup(self, *args, **kw):
        ...
        if request.httprequest.method == 'POST' and values and signup_approval:
            if values.get('password') != request.params.get('confirm_password'):
                qcontext['error'] = _("Passwords do not match")
                return request.render('auth_signup.signup', qcontext)
            if request.env['res.users'].sudo().search([('login', '=', request.params.get('login'))], limit=1):
                qcontext['error'] = _("Email already registered")
                return request.render('auth_signup.signup', qcontext)
...
class SignUpApproveController(http.Controller):
    ...
    def create_attachment(self, **dat):
        ...
        data_list = []
        approve_id = request.env['res.users.approve']
        if dat.get('password') != dat.get('confirm_password'):
        ...
        if approve_id.sudo().search(
                [('email', '=', dat['email'])]):
            pass
        else:
            attach = approve_id.sudo().create(
                {'name': dat['username'],
                 'email': dat['email'],
