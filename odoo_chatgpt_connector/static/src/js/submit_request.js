odoo.define('odoo_chatgpt_connector.chatgpt_search', function(require) {
    ...
    publicWidget.registry.QuestionInput = publicWidget.Widget.extend({
        ...
        get_answer: async function(event) {
            this.$el.find(this).prop("disabled", true);
            this.$el.find('#result_area').val('');
            this.$el.find('#result_area')[0].placeholder = "Loading.....";
            if (!this.api_key) {
            ...
            /* For showing the answer in the result area */
            fetch("https://api.openai.com/v1/completions", requestOptions)
                .then(response => response.text())
                .then(result => {
                    const obj = JSON.parse(result);
                    let area = obj.choices[0].text;
                    this.$el.find('#result_area').val(obj.choices[0].text.trim());
                })
                .catch(error => console.log('error', error));
        },
        /*For copying the Answer to the clipboard*/
        copy: function(event) {
            document.querySelector('#result_area').select();
