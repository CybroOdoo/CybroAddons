...
class ResPartner(models.Model):
    ...
    Methods:
        load_views(self, views, options=None):
            Super load_views function to write into related user when opening the view.
    """
    _inherit = 'res.partner'

    related_user_id = fields.Many2one('res.users', string='Related User',
                                      readonly=True, help='This field contains'
                                                          'the related user of'
                                                          'the partner if there'
                                                          'is any')
    is_have_user = fields.Boolean(string='Have User', help='This field helps to'
                                                           'check if there is'
                                                           'any user related to'
                                                           'the partner')
