...
class MailThread(models.AbstractModel):
    ...
    def _message_auto_subscribe(self, updated_values,
        ...
        """ Handle auto subscription. Auto subscription is done based on two
             ...
             * calling _message_auto_subscribe_notify that returns
             ...
             subtypes and notification to send. Base behavior is to subscribe
              responsible and notify them; Adding application-specific auto
              subscription should be done by overriding
            ``_message_auto_subscribe_followers``. It should return structured
            data for new partner to subscribe, with subtypes and eventual
            notification to perform. See that method for more details.
    ...
    def _message_auto_subscribe_notify(self, partner_ids, template):
        ...
        if self.no_subscription():
            for rec in self:
                if rec._name not in self.no_subscription():
