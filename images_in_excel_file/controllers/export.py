...
def _is_image_field(field_name: str) -> bool:
    """Check whether the field is an image-related field."""
...
def _to_png_stream(raw: bytes):
    """Convert image data to a PNG stream with suitable scaling."""
...
def _patch_base_export():
    """Patch binary field export to preserve image data."""
...
def _patch_xlsx_writer():
    """Patch XLSX export to embed image fields as thumbnails."""
    ...
    def _new_init(self, fields, columns_headers, row_count=0, **kwargs):
        _orig_init(self, fields, columns_headers, row_count, **kwargs)
