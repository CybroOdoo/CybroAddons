...
class CustomerRegistration(http.Controller):
    @http.route('/sale_return', type='jsonrpc', csrf=False, auth="public",
                website=True)

    @http.route('/sale_return', type='jsonrpc', csrf=False, auth="public", website=True)
    def sale_return(self, **kwargs):
        """Controller to create return order with debug logs"""
        _logger.info("sale_return called with kwargs: %s", kwargs)
        ...
        if kwargs.get('vals'):
            ...
            if so_id:
                ...
                moves = stock_picks.mapped('move_ids').sudo().filtered(
